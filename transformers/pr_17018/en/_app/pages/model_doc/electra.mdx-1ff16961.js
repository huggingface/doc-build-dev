...
<p>Indices can be obtained using <a href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17018/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
...
elements depending on the configuration (<a
  href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraConfig"
...
</ul>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1064",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.models.electra.modeling_electra.ElectraForPreTrainingOutput"
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1549",returnDescription:`
<p>A <a
...
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1178",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L967",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1453",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1265",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_electra.py#L1347",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L931",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17018/en/main_classes/output#transformers.modeling_tf_outputs.TFBaseModelOutputWithPastAndCrossAttentions"
...
Whether or not to use the model in training mode (some modules like dropout modules have different
behaviors between training and evaluation).`,name:"training"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1029",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.models.electra.modeling_tf_electra.TFElectraForPreTrainingOutput"
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1159",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1271",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1365",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1482",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_tf_electra.py#L1564",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17018/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_flax_electra.py#L674"}}),zi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraModel.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraModel.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17018/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17018/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Whether or not to return a <a href="/docs/transformers/pr_17018/en/main_classes/output#transformers.utils.ModelOutput">ModelOutput</a> instead of a plain tuple.`,name:"return_dict"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_flax_electra.py#L576",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17018/en/main_classes/output#transformers.modeling_flax_outputs.FlaxBaseModelOutput"
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17018/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_flax_electra.py#L823"}}),Si=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForPreTraining.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForPreTraining.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17018/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17018/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17018/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_flax_electra.py#L760"}}),Gi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForMaskedLM.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForMaskedLM.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17018/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17018/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17018/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17018/src/transformers/models/electra/modeling_flax_electra.py#L1133"}}),Ll=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForQuestionAnswering.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForQuestionAnswering.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17018/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17018/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17018/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
pooled output) e.g. for GLUE tasks.`),Xk=c(),cr=n("p"),Yk=r("This model inherits from "),ud=n("a"),Zk=r("PreTrainedModel"),e1=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),t1=c(),pr=n("p"),o1=r("This model is also a PyTorch "),hr=n("a"),n1=r("torch.nn.Module"),s1=r(` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),a1=c(),Re=n("div"),b(mr.$$.fragment),r1=c(),jo=n("p"),i1=r("The "),fd=n("a"),l1=r("ElectraForSequenceClassification"),d1=r(" forward method, overrides the "),Qc=n("code"),c1=r("__call__"),p1=r(" special method."),h1=c(),b(In.$$.fragment),m1=c(),b(Sn.$$.fragment),u1=c(),b(Dn.$$.fragment),f1=c(),b(Wn.$$.fragment),g1=c(),b(Un.$$.fragment),du=c(),qo=n("h2"),Rn=n("a"),Bc=n("span"),b(ur.$$.fragment),_1=c(),Vc=n("span"),k1=r("ElectraForMultipleChoice"),cu=c(),ot=n("div"),b(fr.$$.fragment),v1=c(),Kc=n("p"),T1=r(`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),b1=c(),gr=n("p"),y1=r("This model inherits from "),gd=n("a"),E1=r("PreTrainedModel"),w1=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),$1=c(),_r=n("p"),F1=r("This model is also a PyTorch "),kr=n("a"),x1=r("torch.nn.Module"),M1=r(` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),z1=c(),Ft=n("div"),b(vr.$$.fragment),C1=c(),Po=n("p"),j1=r("The "),_d=n("a"),q1=r("ElectraForMultipleChoice"),P1=r(" forward method, overrides the "),Jc=n("code"),A1=r("__call__"),O1=r(" special method."),L1=c(),b(Hn.$$.fragment),N1=c(),b(Qn.$$.fragment),pu=c(),Ao=n("h2"),Bn=n("a"),Gc=n("span"),b(Tr.$$.fragment),I1=c(),Xc=n("span"),S1=r("ElectraForTokenClassification"),hu=c(),Be=n("div"),b(br.$$.fragment),D1=c(),Yc=n("p"),W1=r("Electra model with a token classification head on top."),U1=c(),Zc=n("p"),R1=r("Both the discriminator and generator may be loaded into this model."),H1=c(),yr=n("p"),Q1=r("This model inherits from "),kd=n("a"),B1=r("PreTrainedModel"),V1=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),K1=c(),Er=n("p"),J1=r("This model is also a PyTorch "),wr=n("a"),G1=r("torch.nn.Module"),X1=r(` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),Y1=c(),at=n("div"),b($r.$$.fragment),Z1=c(),Oo=n("p"),ev=r("The "),vd=n("a"),tv=r("ElectraForTokenClassification"),ov=r(" forward method, overrides the "),ep=n("code"),nv=r("__call__"),sv=r(" special method."),av=c(),b(Vn.$$.fragment),rv=c(),b(Kn.$$.fragment),iv=c(),b(Jn.$$.fragment),mu=c(),Lo=n("h2"),Gn=n("a"),tp=n("span"),b(Fr.$$.fragment),lv=c(),op=n("span"),dv=r("ElectraForQuestionAnswering"),uu=c(),nt=n("div"),b(xr.$$.fragment),cv=c(),No=n("p"),pv=r(`ELECTRA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),np=n("code"),hv=r("span start logits"),mv=r(" and "),sp=n("code"),uv=r("span end logits"),fv=r(")."),gv=c(),Mr=n("p"),_v=r("This model inherits from "),Td=n("a"),kv=r("PreTrainedModel"),vv=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Tv=c(),zr=n("p"),bv=r("This model is also a PyTorch "),Cr=n("a"),yv=r("torch.nn.Module"),Ev=r(` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
...
behavior.`),Bv=c(),b(ts.$$.fragment),Vv=c(),xt=n("div"),b(Nr.$$.fragment),Kv=c(),Do=n("p"),Jv=r("The "),Ed=n("a"),Gv=r("TFElectraModel"),Xv=r(" forward method, overrides the "),dp=n("code"),Yv=r("__call__"),Zv=r(" special method."),eT=c(),b(os.$$.fragment),tT=c(),b(ns.$$.fragment),_u=c(),Wo=n("h2"),ss=n("a"),cp=n("span"),b(Ir.$$.fragment),oT=c(),pp=n("span"),nT=r("TFElectraForPreTraining"),ku=c(),Oe=n("div"),b(Sr.$$.fragment),sT=c(),hp=n("p"),aT=r("Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),rT=c(),mp=n("p"),iT=r(`Even though both the discriminator and generator may be loaded into this model, the discriminator is the only model
of the two to have the correct classification head to be used for this model.`),lT=c(),Dr=n("p"),dT=r("This model inherits from "),wd=n("a"),cT=r("TFPreTrainedModel"),pT=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),hT=c(),Wr=n("p"),mT=r("This model is also a "),Ur=n("a"),uT=r("tf.keras.Model"),fT=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),gT=c(),b(as.$$.fragment),_T=c(),Mt=n("div"),b(Rr.$$.fragment),kT=c(),Uo=n("p"),vT=r("The "),$d=n("a"),TT=r("TFElectraForPreTraining"),bT=r(" forward method, overrides the "),up=n("code"),yT=r("__call__"),ET=r(" special method."),wT=c(),b(rs.$$.fragment),$T=c(),b(is.$$.fragment),vu=c(),Ro=n("h2"),ls=n("a"),fp=n("span"),b(Hr.$$.fragment),FT=c(),gp=n("span"),xT=r("TFElectraForMaskedLM"),Tu=c(),Le=n("div"),b(Qr.$$.fragment),MT=c(),_p=n("p"),zT=r("Electra model with a language modeling head on top."),CT=c(),kp=n("p"),jT=r(`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),qT=c(),Br=n("p"),PT=r("This model inherits from "),Fd=n("a"),AT=r("TFPreTrainedModel"),OT=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),LT=c(),Vr=n("p"),NT=r("This model is also a "),Kr=n("a"),IT=r("tf.keras.Model"),ST=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),DT=c(),b(ds.$$.fragment),WT=c(),it=n("div"),b(Jr.$$.fragment),UT=c(),Ho=n("p"),RT=r("The "),xd=n("a"),HT=r("TFElectraForMaskedLM"),QT=r(" forward method, overrides the "),vp=n("code"),BT=r("__call__"),VT=r(" special method."),KT=c(),b(cs.$$.fragment),JT=c(),b(ps.$$.fragment),GT=c(),b(hs.$$.fragment),bu=c(),Qo=n("h2"),ms=n("a"),Tp=n("span"),b(Gr.$$.fragment),XT=c(),bp=n("span"),YT=r("TFElectraForSequenceClassification"),yu=c(),Ke=n("div"),b(Xr.$$.fragment),ZT=c(),yp=n("p"),eb=r(`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
...
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),db=c(),b(us.$$.fragment),cb=c(),lt=n("div"),b(ti.$$.fragment),pb=c(),Bo=n("p"),hb=r("The "),zd=n("a"),mb=r("TFElectraForSequenceClassification"),ub=r(" forward method, overrides the "),Ep=n("code"),fb=r("__call__"),gb=r(" special method."),_b=c(),b(fs.$$.fragment),kb=c(),b(gs.$$.fragment),vb=c(),b(_s.$$.fragment),Eu=c(),Vo=n("h2"),ks=n("a"),wp=n("span"),b(oi.$$.fragment),Tb=c(),$p=n("span"),bb=r("TFElectraForMultipleChoice"),wu=c(),Je=n("div"),b(ni.$$.fragment),yb=c(),Fp=n("p"),Eb=r(`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
...
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Mb=c(),ai=n("p"),zb=r("This model is also a "),ri=n("a"),Cb=r("tf.keras.Model"),jb=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),qb=c(),b(vs.$$.fragment),Pb=c(),zt=n("div"),b(ii.$$.fragment),Ab=c(),Ko=n("p"),Ob=r("The "),jd=n("a"),Lb=r("TFElectraForMultipleChoice"),Nb=r(" forward method, overrides the "),xp=n("code"),Ib=r("__call__"),Sb=r(" special method."),Db=c(),b(Ts.$$.fragment),Wb=c(),b(bs.$$.fragment),$u=c(),Jo=n("h2"),ys=n("a"),Mp=n("span"),b(li.$$.fragment),Ub=c(),zp=n("span"),Rb=r("TFElectraForTokenClassification"),Fu=c(),Ne=n("div"),b(di.$$.fragment),Hb=c(),Cp=n("p"),Qb=r("Electra model with a token classification head on top."),Bb=c(),jp=n("p"),Vb=r("Both the discriminator and generator may be loaded into this model."),Kb=c(),ci=n("p"),Jb=r("This model inherits from "),qd=n("a"),Gb=r("TFPreTrainedModel"),Xb=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Yb=c(),pi=n("p"),Zb=r("This model is also a "),hi=n("a"),ey=r("tf.keras.Model"),ty=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),oy=c(),b(Es.$$.fragment),ny=c(),dt=n("div"),b(mi.$$.fragment),sy=c(),Go=n("p"),ay=r("The "),Pd=n("a"),ry=r("TFElectraForTokenClassification"),iy=r(" forward method, overrides the "),qp=n("code"),ly=r("__call__"),dy=r(" special method."),cy=c(),b(ws.$$.fragment),py=c(),b($s.$$.fragment),hy=c(),b(Fs.$$.fragment),xu=c(),Xo=n("h2"),xs=n("a"),Pp=n("span"),b(ui.$$.fragment),my=c(),Ap=n("span"),uy=r("TFElectraForQuestionAnswering"),Mu=c(),Ge=n("div"),b(fi.$$.fragment),fy=c(),Yo=n("p"),gy=r(`Electra Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),Op=n("code"),_y=r("span start logits"),ky=r(" and "),Lp=n("code"),vy=r("span end logits"),Ty=r(")."),by=c(),gi=n("p"),yy=r("This model inherits from "),Ad=n("a"),Ey=r("TFPreTrainedModel"),wy=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),$y=c(),_i=n("p"),Fy=r("This model is also a "),ki=n("a"),xy=r("tf.keras.Model"),My=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),zy=c(),b(Ms.$$.fragment),Cy=c(),ct=n("div"),b(vi.$$.fragment),jy=c(),Zo=n("p"),qy=r("The "),Od=n("a"),Py=r("TFElectraForQuestionAnswering"),Ay=r(" forward method, overrides the "),Np=n("code"),Oy=r("__call__"),Ly=r(" special method."),Ny=c(),b(zs.$$.fragment),Iy=c(),b(Cs.$$.fragment),Sy=c(),b(js.$$.fragment),zu=c(),en=n("h2"),qs=n("a"),Ip=n("span"),b(Ti.$$.fragment),Dy=c(),Sp=n("span"),Wy=r("FlaxElectraModel"),Cu=c(),Ie=n("div"),b(bi.$$.fragment),Uy=c(),Dp=n("p"),Ry=r("The bare Electra Model transformer outputting raw hidden-states without any specific head on top."),Hy=c(),yi=n("p"),Qy=r("This model inherits from "),Ld=n("a"),By=r("FlaxPreTrainedModel"),Vy=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Ky=c(),Ei=n("p"),Jy=r(`This model is also a Flax Linen
`),wi=n("a"),Gy=r("flax.nn.Module"),Xy=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Yy=c(),Wp=n("p"),Zy=r("Finally, this model supports inherent JAX features such as:"),eE=c(),Qt=n("ul"),Up=n("li"),$i=n("a"),tE=r("Just-In-Time (JIT) compilation"),oE=c(),Rp=n("li"),Fi=n("a"),nE=r("Automatic Differentiation"),sE=c(),Hp=n("li"),xi=n("a"),aE=r("Vectorization"),rE=c(),Qp=n("li"),Mi=n("a"),iE=r("Parallelization"),lE=c(),Ct=n("div"),b(zi.$$.fragment),dE=c(),tn=n("p"),cE=r("The "),Bp=n("code"),pE=r("FlaxElectraPreTrainedModel"),hE=r(" forward method, overrides the "),Vp=n("code"),mE=r("__call__"),uE=r(" special method."),fE=c(),b(Ps.$$.fragment),gE=c(),b(As.$$.fragment),ju=c(),on=n("h2"),Os=n("a"),Kp=n("span"),b(Ci.$$.fragment),_E=c(),Jp=n("span"),kE=r("FlaxElectraForPreTraining"),qu=c(),Pe=n("div"),b(ji.$$.fragment),vE=c(),Gp=n("p"),TE=r("Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),bE=c(),Xp=n("p"),yE=r("It is recommended to load the discriminator checkpoint into that model."),EE=c(),qi=n("p"),wE=r("This model inherits from "),Nd=n("a"),$E=r("FlaxPreTrainedModel"),FE=r(`. Check the superclass documentation for the generic methods the
...
pooled output) e.g. for GLUE tasks.`),L0.forEach(t),Xk=p(ro),cr=s(ro,"P",{});var df=a(cr);Yk=i(df,"This model inherits from "),ud=s(df,"A",{href:!0});var N0=a(ud);Zk=i(N0,"PreTrainedModel"),N0.forEach(t),e1=i(df,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),df.forEach(t),t1=p(ro),pr=s(ro,"P",{});var cf=a(pr);o1=i(cf,"This model is also a PyTorch "),hr=s(cf,"A",{href:!0,rel:!0});var I0=a(hr);n1=i(I0,"torch.nn.Module"),I0.forEach(t),s1=i(cf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),cf.forEach(t),a1=p(ro),Re=s(ro,"DIV",{class:!0});var pt=a(Re);y(mr.$$.fragment,pt),r1=p(pt),jo=s(pt,"P",{});var Vd=a(jo);i1=i(Vd,"The "),fd=s(Vd,"A",{href:!0});var S0=a(fd);l1=i(S0,"ElectraForSequenceClassification"),S0.forEach(t),d1=i(Vd," forward method, overrides the "),Qc=s(Vd,"CODE",{});var D0=a(Qc);c1=i(D0,"__call__"),D0.forEach(t),p1=i(Vd," special method."),Vd.forEach(t),h1=p(pt),y(In.$$.fragment,pt),m1=p(pt),y(Sn.$$.fragment,pt),u1=p(pt),y(Dn.$$.fragment,pt),f1=p(pt),y(Wn.$$.fragment,pt),g1=p(pt),y(Un.$$.fragment,pt),pt.forEach(t),ro.forEach(t),du=p(o),qo=s(o,"H2",{class:!0});var pf=a(qo);Rn=s(pf,"A",{id:!0,class:!0,href:!0});var W0=a(Rn);Bc=s(W0,"SPAN",{});var U0=a(Bc);y(ur.$$.fragment,U0),U0.forEach(t),W0.forEach(t),_1=p(pf),Vc=s(pf,"SPAN",{});var R0=a(Vc);k1=i(R0,"ElectraForMultipleChoice"),R0.forEach(t),pf.forEach(t),cu=p(o),ot=s(o,"DIV",{class:!0});var io=a(ot);y(fr.$$.fragment,io),v1=p(io),Kc=s(io,"P",{});var H0=a(Kc);T1=i(H0,`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),H0.forEach(t),b1=p(io),gr=s(io,"P",{});var hf=a(gr);y1=i(hf,"This model inherits from "),gd=s(hf,"A",{href:!0});var Q0=a(gd);E1=i(Q0,"PreTrainedModel"),Q0.forEach(t),w1=i(hf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),hf.forEach(t),$1=p(io),_r=s(io,"P",{});var mf=a(_r);F1=i(mf,"This model is also a PyTorch "),kr=s(mf,"A",{href:!0,rel:!0});var B0=a(kr);x1=i(B0,"torch.nn.Module"),B0.forEach(t),M1=i(mf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),mf.forEach(t),z1=p(io),Ft=s(io,"DIV",{class:!0});var sa=a(Ft);y(vr.$$.fragment,sa),C1=p(sa),Po=s(sa,"P",{});var Kd=a(Po);j1=i(Kd,"The "),_d=s(Kd,"A",{href:!0});var V0=a(_d);q1=i(V0,"ElectraForMultipleChoice"),V0.forEach(t),P1=i(Kd," forward method, overrides the "),Jc=s(Kd,"CODE",{});var K0=a(Jc);A1=i(K0,"__call__"),K0.forEach(t),O1=i(Kd," special method."),Kd.forEach(t),L1=p(sa),y(Hn.$$.fragment,sa),N1=p(sa),y(Qn.$$.fragment,sa),sa.forEach(t),io.forEach(t),pu=p(o),Ao=s(o,"H2",{class:!0});var uf=a(Ao);Bn=s(uf,"A",{id:!0,class:!0,href:!0});var J0=a(Bn);Gc=s(J0,"SPAN",{});var G0=a(Gc);y(Tr.$$.fragment,G0),G0.forEach(t),J0.forEach(t),I1=p(uf),Xc=s(uf,"SPAN",{});var X0=a(Xc);S1=i(X0,"ElectraForTokenClassification"),X0.forEach(t),uf.forEach(t),hu=p(o),Be=s(o,"DIV",{class:!0});var St=a(Be);y(br.$$.fragment,St),D1=p(St),Yc=s(St,"P",{});var Y0=a(Yc);W1=i(Y0,"Electra model with a token classification head on top."),Y0.forEach(t),U1=p(St),Zc=s(St,"P",{});var Z0=a(Zc);R1=i(Z0,"Both the discriminator and generator may be loaded into this model."),Z0.forEach(t),H1=p(St),yr=s(St,"P",{});var ff=a(yr);Q1=i(ff,"This model inherits from "),kd=s(ff,"A",{href:!0});var eF=a(kd);B1=i(eF,"PreTrainedModel"),eF.forEach(t),V1=i(ff,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),ff.forEach(t),K1=p(St),Er=s(St,"P",{});var gf=a(Er);J1=i(gf,"This model is also a PyTorch "),wr=s(gf,"A",{href:!0,rel:!0});var tF=a(wr);G1=i(tF,"torch.nn.Module"),tF.forEach(t),X1=i(gf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),gf.forEach(t),Y1=p(St),at=s(St,"DIV",{class:!0});var lo=a(at);y($r.$$.fragment,lo),Z1=p(lo),Oo=s(lo,"P",{});var Jd=a(Oo);ev=i(Jd,"The "),vd=s(Jd,"A",{href:!0});var oF=a(vd);tv=i(oF,"ElectraForTokenClassification"),oF.forEach(t),ov=i(Jd," forward method, overrides the "),ep=s(Jd,"CODE",{});var nF=a(ep);nv=i(nF,"__call__"),nF.forEach(t),sv=i(Jd," special method."),Jd.forEach(t),av=p(lo),y(Vn.$$.fragment,lo),rv=p(lo),y(Kn.$$.fragment,lo),iv=p(lo),y(Jn.$$.fragment,lo),lo.forEach(t),St.forEach(t),mu=p(o),Lo=s(o,"H2",{class:!0});var _f=a(Lo);Gn=s(_f,"A",{id:!0,class:!0,href:!0});var sF=a(Gn);tp=s(sF,"SPAN",{});var aF=a(tp);y(Fr.$$.fragment,aF),aF.forEach(t),sF.forEach(t),lv=p(_f),op=s(_f,"SPAN",{});var rF=a(op);dv=i(rF,"ElectraForQuestionAnswering"),rF.forEach(t),_f.forEach(t),uu=p(o),nt=s(o,"DIV",{class:!0});var co=a(nt);y(xr.$$.fragment,co),cv=p(co),No=s(co,"P",{});var Gd=a(No);pv=i(Gd,`ELECTRA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),np=s(Gd,"CODE",{});var iF=a(np);hv=i(iF,"span start logits"),iF.forEach(t),mv=i(Gd," and "),sp=s(Gd,"CODE",{});var lF=a(sp);uv=i(lF,"span end logits"),lF.forEach(t),fv=i(Gd,")."),Gd.forEach(t),gv=p(co),Mr=s(co,"P",{});var kf=a(Mr);_v=i(kf,"This model inherits from "),Td=s(kf,"A",{href:!0});var dF=a(Td);kv=i(dF,"PreTrainedModel"),dF.forEach(t),vv=i(kf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),kf.forEach(t),Tv=p(co),zr=s(co,"P",{});var vf=a(zr);bv=i(vf,"This model is also a PyTorch "),Cr=s(vf,"A",{href:!0,rel:!0});var cF=a(Cr);yv=i(cF,"torch.nn.Module"),cF.forEach(t),Ev=i(vf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),vf.forEach(t),wv=p(co),rt=s(co,"DIV",{class:!0});var po=a(rt);y(jr.$$.fragment,po),$v=p(po),Io=s(po,"P",{});var Xd=a(Io);Fv=i(Xd,"The "),bd=s(Xd,"A",{href:!0});var pF=a(bd);xv=i(pF,"ElectraForQuestionAnswering"),pF.forEach(t),Mv=i(Xd," forward method, overrides the "),ap=s(Xd,"CODE",{});var hF=a(ap);zv=i(hF,"__call__"),hF.forEach(t),Cv=i(Xd," special method."),Xd.forEach(t),jv=p(po),y(Xn.$$.fragment,po),qv=p(po),y(Yn.$$.fragment,po),Pv=p(po),y(Zn.$$.fragment,po),po.forEach(t),co.forEach(t),fu=p(o),So=s(o,"H2",{class:!0});var Tf=a(So);es=s(Tf,"A",{id:!0,class:!0,href:!0});var mF=a(es);rp=s(mF,"SPAN",{});var uF=a(rp);y(qr.$$.fragment,uF),uF.forEach(t),mF.forEach(t),Av=p(Tf),ip=s(Tf,"SPAN",{});var fF=a(ip);Ov=i(fF,"TFElectraModel"),fF.forEach(t),Tf.forEach(t),gu=p(o),Ve=s(o,"DIV",{class:!0});var Dt=a(Ve);y(Pr.$$.fragment,Dt),Lv=p(Dt),lp=s(Dt,"P",{});var gF=a(lp);Nv=i(gF,"The bare Electra Model transformer outputting raw hidden-states without any specific head on top. Identical to the BERT model except that it uses an additional linear layer between the embedding layer and the encoder if the hidden size and embedding size are different. Both the generator and discriminator checkpoints may be loaded into this model."),gF.forEach(t),Iv=p(Dt),Ar=s(Dt,"P",{});var bf=a(Ar);Sv=i(bf,"This model inherits from "),yd=s(bf,"A",{href:!0});var _F=a(yd);Dv=i(_F,"TFPreTrainedModel"),_F.forEach(t),Wv=i(bf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),bf.forEach(t),Uv=p(Dt),Or=s(Dt,"P",{});var yf=a(Or);Rv=i(yf,"This model is also a "),Lr=s(yf,"A",{href:!0,rel:!0});var kF=a(Lr);Hv=i(kF,"tf.keras.Model"),kF.forEach(t),Qv=i(yf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),yf.forEach(t),Bv=p(Dt),y(ts.$$.fragment,Dt),Vv=p(Dt),xt=s(Dt,"DIV",{class:!0});var aa=a(xt);y(Nr.$$.fragment,aa),Kv=p(aa),Do=s(aa,"P",{});var Yd=a(Do);Jv=i(Yd,"The "),Ed=s(Yd,"A",{href:!0});var vF=a(Ed);Gv=i(vF,"TFElectraModel"),vF.forEach(t),Xv=i(Yd," forward method, overrides the "),dp=s(Yd,"CODE",{});var TF=a(dp);Yv=i(TF,"__call__"),TF.forEach(t),Zv=i(Yd," special method."),Yd.forEach(t),eT=p(aa),y(os.$$.fragment,aa),tT=p(aa),y(ns.$$.fragment,aa),aa.forEach(t),Dt.forEach(t),_u=p(o),Wo=s(o,"H2",{class:!0});var Ef=a(Wo);ss=s(Ef,"A",{id:!0,class:!0,href:!0});var bF=a(ss);cp=s(bF,"SPAN",{});var yF=a(cp);y(Ir.$$.fragment,yF),yF.forEach(t),bF.forEach(t),oT=p(Ef),pp=s(Ef,"SPAN",{});var EF=a(pp);nT=i(EF,"TFElectraForPreTraining"),EF.forEach(t),Ef.forEach(t),ku=p(o),Oe=s(o,"DIV",{class:!0});var ht=a(Oe);y(Sr.$$.fragment,ht),sT=p(ht),hp=s(ht,"P",{});var wF=a(hp);aT=i(wF,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),wF.forEach(t),rT=p(ht),mp=s(ht,"P",{});var $F=a(mp);iT=i($F,`Even though both the discriminator and generator may be loaded into this model, the discriminator is the only model
of the two to have the correct classification head to be used for this model.`),$F.forEach(t),lT=p(ht),Dr=s(ht,"P",{});var wf=a(Dr);dT=i(wf,"This model inherits from "),wd=s(wf,"A",{href:!0});var FF=a(wd);cT=i(FF,"TFPreTrainedModel"),FF.forEach(t),pT=i(wf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),wf.forEach(t),hT=p(ht),Wr=s(ht,"P",{});var $f=a(Wr);mT=i($f,"This model is also a "),Ur=s($f,"A",{href:!0,rel:!0});var xF=a(Ur);uT=i(xF,"tf.keras.Model"),xF.forEach(t),fT=i($f,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),$f.forEach(t),gT=p(ht),y(as.$$.fragment,ht),_T=p(ht),Mt=s(ht,"DIV",{class:!0});var ra=a(Mt);y(Rr.$$.fragment,ra),kT=p(ra),Uo=s(ra,"P",{});var Zd=a(Uo);vT=i(Zd,"The "),$d=s(Zd,"A",{href:!0});var MF=a($d);TT=i(MF,"TFElectraForPreTraining"),MF.forEach(t),bT=i(Zd," forward method, overrides the "),up=s(Zd,"CODE",{});var zF=a(up);yT=i(zF,"__call__"),zF.forEach(t),ET=i(Zd," special method."),Zd.forEach(t),wT=p(ra),y(rs.$$.fragment,ra),$T=p(ra),y(is.$$.fragment,ra),ra.forEach(t),ht.forEach(t),vu=p(o),Ro=s(o,"H2",{class:!0});var Ff=a(Ro);ls=s(Ff,"A",{id:!0,class:!0,href:!0});var CF=a(ls);fp=s(CF,"SPAN",{});var jF=a(fp);y(Hr.$$.fragment,jF),jF.forEach(t),CF.forEach(t),FT=p(Ff),gp=s(Ff,"SPAN",{});var qF=a(gp);xT=i(qF,"TFElectraForMaskedLM"),qF.forEach(t),Ff.forEach(t),Tu=p(o),Le=s(o,"DIV",{class:!0});var mt=a(Le);y(Qr.$$.fragment,mt),MT=p(mt),_p=s(mt,"P",{});var PF=a(_p);zT=i(PF,"Electra model with a language modeling head on top."),PF.forEach(t),CT=p(mt),kp=s(mt,"P",{});var AF=a(kp);jT=i(AF,`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),AF.forEach(t),qT=p(mt),Br=s(mt,"P",{});var xf=a(Br);PT=i(xf,"This model inherits from "),Fd=s(xf,"A",{href:!0});var OF=a(Fd);AT=i(OF,"TFPreTrainedModel"),OF.forEach(t),OT=i(xf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),xf.forEach(t),LT=p(mt),Vr=s(mt,"P",{});var Mf=a(Vr);NT=i(Mf,"This model is also a "),Kr=s(Mf,"A",{href:!0,rel:!0});var LF=a(Kr);IT=i(LF,"tf.keras.Model"),LF.forEach(t),ST=i(Mf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Mf.forEach(t),DT=p(mt),y(ds.$$.fragment,mt),WT=p(mt),it=s(mt,"DIV",{class:!0});var ho=a(it);y(Jr.$$.fragment,ho),UT=p(ho),Ho=s(ho,"P",{});var ec=a(Ho);RT=i(ec,"The "),xd=s(ec,"A",{href:!0});var NF=a(xd);HT=i(NF,"TFElectraForMaskedLM"),NF.forEach(t),QT=i(ec," forward method, overrides the "),vp=s(ec,"CODE",{});var IF=a(vp);BT=i(IF,"__call__"),IF.forEach(t),VT=i(ec," special method."),ec.forEach(t),KT=p(ho),y(cs.$$.fragment,ho),JT=p(ho),y(ps.$$.fragment,ho),GT=p(ho),y(hs.$$.fragment,ho),ho.forEach(t),mt.forEach(t),bu=p(o),Qo=s(o,"H2",{class:!0});var zf=a(Qo);ms=s(zf,"A",{id:!0,class:!0,href:!0});var SF=a(ms);Tp=s(SF,"SPAN",{});var DF=a(Tp);y(Gr.$$.fragment,DF),DF.forEach(t),SF.forEach(t),XT=p(zf),bp=s(zf,"SPAN",{});var WF=a(bp);YT=i(WF,"TFElectraForSequenceClassification"),WF.forEach(t),zf.forEach(t),yu=p(o),Ke=s(o,"DIV",{class:!0});var Wt=a(Ke);y(Xr.$$.fragment,Wt),ZT=p(Wt),yp=s(Wt,"P",{});var UF=a(yp);eb=i(UF,`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),UF.forEach(t),tb=p(Wt),Yr=s(Wt,"P",{});var Cf=a(Yr);ob=i(Cf,"This model inherits from "),Md=s(Cf,"A",{href:!0});var RF=a(Md);nb=i(RF,"TFPreTrainedModel"),RF.forEach(t),sb=i(Cf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Cf.forEach(t),ab=p(Wt),Zr=s(Wt,"P",{});var jf=a(Zr);rb=i(jf,"This model is also a "),ei=s(jf,"A",{href:!0,rel:!0});var HF=a(ei);ib=i(HF,"tf.keras.Model"),HF.forEach(t),lb=i(jf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),jf.forEach(t),db=p(Wt),y(us.$$.fragment,Wt),cb=p(Wt),lt=s(Wt,"DIV",{class:!0});var mo=a(lt);y(ti.$$.fragment,mo),pb=p(mo),Bo=s(mo,"P",{});var tc=a(Bo);hb=i(tc,"The "),zd=s(tc,"A",{href:!0});var QF=a(zd);mb=i(QF,"TFElectraForSequenceClassification"),QF.forEach(t),ub=i(tc," forward method, overrides the "),Ep=s(tc,"CODE",{});var BF=a(Ep);fb=i(BF,"__call__"),BF.forEach(t),gb=i(tc," special method."),tc.forEach(t),_b=p(mo),y(fs.$$.fragment,mo),kb=p(mo),y(gs.$$.fragment,mo),vb=p(mo),y(_s.$$.fragment,mo),mo.forEach(t),Wt.forEach(t),Eu=p(o),Vo=s(o,"H2",{class:!0});var qf=a(Vo);ks=s(qf,"A",{id:!0,class:!0,href:!0});var VF=a(ks);wp=s(VF,"SPAN",{});var KF=a(wp);y(oi.$$.fragment,KF),KF.forEach(t),VF.forEach(t),Tb=p(qf),$p=s(qf,"SPAN",{});var JF=a($p);bb=i(JF,"TFElectraForMultipleChoice"),JF.forEach(t),qf.forEach(t),wu=p(o),Je=s(o,"DIV",{class:!0});var Ut=a(Je);y(ni.$$.fragment,Ut),yb=p(Ut),Fp=s(Ut,"P",{});var GF=a(Fp);Eb=i(GF,`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),GF.forEach(t),wb=p(Ut),si=s(Ut,"P",{});var Pf=a(si);$b=i(Pf,"This model inherits from "),Cd=s(Pf,"A",{href:!0});var XF=a(Cd);Fb=i(XF,"TFPreTrainedModel"),XF.forEach(t),xb=i(Pf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Pf.forEach(t),Mb=p(Ut),ai=s(Ut,"P",{});var Af=a(ai);zb=i(Af,"This model is also a "),ri=s(Af,"A",{href:!0,rel:!0});var YF=a(ri);Cb=i(YF,"tf.keras.Model"),YF.forEach(t),jb=i(Af,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Af.forEach(t),qb=p(Ut),y(vs.$$.fragment,Ut),Pb=p(Ut),zt=s(Ut,"DIV",{class:!0});var ia=a(zt);y(ii.$$.fragment,ia),Ab=p(ia),Ko=s(ia,"P",{});var oc=a(Ko);Ob=i(oc,"The "),jd=s(oc,"A",{href:!0});var ZF=a(jd);Lb=i(ZF,"TFElectraForMultipleChoice"),ZF.forEach(t),Nb=i(oc," forward method, overrides the "),xp=s(oc,"CODE",{});var ex=a(xp);Ib=i(ex,"__call__"),ex.forEach(t),Sb=i(oc," special method."),oc.forEach(t),Db=p(ia),y(Ts.$$.fragment,ia),Wb=p(ia),y(bs.$$.fragment,ia),ia.forEach(t),Ut.forEach(t),$u=p(o),Jo=s(o,"H2",{class:!0});var Of=a(Jo);ys=s(Of,"A",{id:!0,class:!0,href:!0});var tx=a(ys);Mp=s(tx,"SPAN",{});var ox=a(Mp);y(li.$$.fragment,ox),ox.forEach(t),tx.forEach(t),Ub=p(Of),zp=s(Of,"SPAN",{});var nx=a(zp);Rb=i(nx,"TFElectraForTokenClassification"),nx.forEach(t),Of.forEach(t),Fu=p(o),Ne=s(o,"DIV",{class:!0});var ut=a(Ne);y(di.$$.fragment,ut),Hb=p(ut),Cp=s(ut,"P",{});var sx=a(Cp);Qb=i(sx,"Electra model with a token classification head on top."),sx.forEach(t),Bb=p(ut),jp=s(ut,"P",{});var ax=a(jp);Vb=i(ax,"Both the discriminator and generator may be loaded into this model."),ax.forEach(t),Kb=p(ut),ci=s(ut,"P",{});var Lf=a(ci);Jb=i(Lf,"This model inherits from "),qd=s(Lf,"A",{href:!0});var rx=a(qd);Gb=i(rx,"TFPreTrainedModel"),rx.forEach(t),Xb=i(Lf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Lf.forEach(t),Yb=p(ut),pi=s(ut,"P",{});var Nf=a(pi);Zb=i(Nf,"This model is also a "),hi=s(Nf,"A",{href:!0,rel:!0});var ix=a(hi);ey=i(ix,"tf.keras.Model"),ix.forEach(t),ty=i(Nf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Nf.forEach(t),oy=p(ut),y(Es.$$.fragment,ut),ny=p(ut),dt=s(ut,"DIV",{class:!0});var uo=a(dt);y(mi.$$.fragment,uo),sy=p(uo),Go=s(uo,"P",{});var nc=a(Go);ay=i(nc,"The "),Pd=s(nc,"A",{href:!0});var lx=a(Pd);ry=i(lx,"TFElectraForTokenClassification"),lx.forEach(t),iy=i(nc," forward method, overrides the "),qp=s(nc,"CODE",{});var dx=a(qp);ly=i(dx,"__call__"),dx.forEach(t),dy=i(nc," special method."),nc.forEach(t),cy=p(uo),y(ws.$$.fragment,uo),py=p(uo),y($s.$$.fragment,uo),hy=p(uo),y(Fs.$$.fragment,uo),uo.forEach(t),ut.forEach(t),xu=p(o),Xo=s(o,"H2",{class:!0});var If=a(Xo);xs=s(If,"A",{id:!0,class:!0,href:!0});var cx=a(xs);Pp=s(cx,"SPAN",{});var px=a(Pp);y(ui.$$.fragment,px),px.forEach(t),cx.forEach(t),my=p(If),Ap=s(If,"SPAN",{});var hx=a(Ap);uy=i(hx,"TFElectraForQuestionAnswering"),hx.forEach(t),If.forEach(t),Mu=p(o),Ge=s(o,"DIV",{class:!0});var Rt=a(Ge);y(fi.$$.fragment,Rt),fy=p(Rt),Yo=s(Rt,"P",{});var sc=a(Yo);gy=i(sc,`Electra Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),Op=s(sc,"CODE",{});var mx=a(Op);_y=i(mx,"span start logits"),mx.forEach(t),ky=i(sc," and "),Lp=s(sc,"CODE",{});var ux=a(Lp);vy=i(ux,"span end logits"),ux.forEach(t),Ty=i(sc,")."),sc.forEach(t),by=p(Rt),gi=s(Rt,"P",{});var Sf=a(gi);yy=i(Sf,"This model inherits from "),Ad=s(Sf,"A",{href:!0});var fx=a(Ad);Ey=i(fx,"TFPreTrainedModel"),fx.forEach(t),wy=i(Sf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Sf.forEach(t),$y=p(Rt),_i=s(Rt,"P",{});var Df=a(_i);Fy=i(Df,"This model is also a "),ki=s(Df,"A",{href:!0,rel:!0});var gx=a(ki);xy=i(gx,"tf.keras.Model"),gx.forEach(t),My=i(Df,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Df.forEach(t),zy=p(Rt),y(Ms.$$.fragment,Rt),Cy=p(Rt),ct=s(Rt,"DIV",{class:!0});var fo=a(ct);y(vi.$$.fragment,fo),jy=p(fo),Zo=s(fo,"P",{});var ac=a(Zo);qy=i(ac,"The "),Od=s(ac,"A",{href:!0});var _x=a(Od);Py=i(_x,"TFElectraForQuestionAnswering"),_x.forEach(t),Ay=i(ac," forward method, overrides the "),Np=s(ac,"CODE",{});var kx=a(Np);Oy=i(kx,"__call__"),kx.forEach(t),Ly=i(ac," special method."),ac.forEach(t),Ny=p(fo),y(zs.$$.fragment,fo),Iy=p(fo),y(Cs.$$.fragment,fo),Sy=p(fo),y(js.$$.fragment,fo),fo.forEach(t),Rt.forEach(t),zu=p(o),en=s(o,"H2",{class:!0});var Wf=a(en);qs=s(Wf,"A",{id:!0,class:!0,href:!0});var vx=a(qs);Ip=s(vx,"SPAN",{});var Tx=a(Ip);y(Ti.$$.fragment,Tx),Tx.forEach(t),vx.forEach(t),Dy=p(Wf),Sp=s(Wf,"SPAN",{});var bx=a(Sp);Wy=i(bx,"FlaxElectraModel"),bx.forEach(t),Wf.forEach(t),Cu=p(o),Ie=s(o,"DIV",{class:!0});var ft=a(Ie);y(bi.$$.fragment,ft),Uy=p(ft),Dp=s(ft,"P",{});var yx=a(Dp);Ry=i(yx,"The bare Electra Model transformer outputting raw hidden-states without any specific head on top."),yx.forEach(t),Hy=p(ft),yi=s(ft,"P",{});var Uf=a(yi);Qy=i(Uf,"This model inherits from "),Ld=s(Uf,"A",{href:!0});var Ex=a(Ld);By=i(Ex,"FlaxPreTrainedModel"),Ex.forEach(t),Vy=i(Uf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Uf.forEach(t),Ky=p(ft),Ei=s(ft,"P",{});var Rf=a(Ei);Jy=i(Rf,`This model is also a Flax Linen
`),wi=s(Rf,"A",{href:!0,rel:!0});var wx=a(wi);Gy=i(wx,"flax.nn.Module"),wx.forEach(t),Xy=i(Rf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Rf.forEach(t),Yy=p(ft),Wp=s(ft,"P",{});var $x=a(Wp);Zy=i($x,"Finally, this model supports inherent JAX features such as:"),$x.forEach(t),eE=p(ft),Qt=s(ft,"UL",{});var la=a(Qt);Up=s(la,"LI",{});var Fx=a(Up);$i=s(Fx,"A",{href:!0,rel:!0});var xx=a($i);tE=i(xx,"Just-In-Time (JIT) compilation"),xx.forEach(t),Fx.forEach(t),oE=p(la),Rp=s(la,"LI",{});var Mx=a(Rp);Fi=s(Mx,"A",{href:!0,rel:!0});var zx=a(Fi);nE=i(zx,"Automatic Differentiation"),zx.forEach(t),Mx.forEach(t),sE=p(la),Hp=s(la,"LI",{});var Cx=a(Hp);xi=s(Cx,"A",{href:!0,rel:!0});var jx=a(xi);aE=i(jx,"Vectorization"),jx.forEach(t),Cx.forEach(t),rE=p(la),Qp=s(la,"LI",{});var qx=a(Qp);Mi=s(qx,"A",{href:!0,rel:!0});var Px=a(Mi);iE=i(Px,"Parallelization"),Px.forEach(t),qx.forEach(t),la.forEach(t),lE=p(ft),Ct=s(ft,"DIV",{class:!0});var da=a(Ct);y(zi.$$.fragment,da),dE=p(da),tn=s(da,"P",{});var rc=a(tn);cE=i(rc,"The "),Bp=s(rc,"CODE",{});var Ax=a(Bp);pE=i(Ax,"FlaxElectraPreTrainedModel"),Ax.forEach(t),hE=i(rc," forward method, overrides the "),Vp=s(rc,"CODE",{});var Ox=a(Vp);mE=i(Ox,"__call__"),Ox.forEach(t),uE=i(rc," special method."),rc.forEach(t),fE=p(da),y(Ps.$$.fragment,da),gE=p(da),y(As.$$.fragment,da),da.forEach(t),ft.forEach(t),ju=p(o),on=s(o,"H2",{class:!0});var Hf=a(on);Os=s(Hf,"A",{id:!0,class:!0,href:!0});var Lx=a(Os);Kp=s(Lx,"SPAN",{});var Nx=a(Kp);y(Ci.$$.fragment,Nx),Nx.forEach(t),Lx.forEach(t),_E=p(Hf),Jp=s(Hf,"SPAN",{});var Ix=a(Jp);kE=i(Ix,"FlaxElectraForPreTraining"),Ix.forEach(t),Hf.forEach(t),qu=p(o),Pe=s(o,"DIV",{class:!0});var Xe=a(Pe);y(ji.$$.fragment,Xe),vE=p(Xe),Gp=s(Xe,"P",{});var Sx=a(Gp);TE=i(Sx,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),Sx.forEach(t),bE=p(Xe),Xp=s(Xe,"P",{});var Dx=a(Xp);yE=i(Dx,"It is recommended to load the discriminator checkpoint into that model."),Dx.forEach(t),EE=p(Xe),qi=s(Xe,"P",{});var Qf=a(qi);wE=i(Qf,"This model inherits from "),Nd=s(Qf,"A",{href:!0});var Wx=a(Nd);$E=i(Wx,"FlaxPreTrainedModel"),Wx.forEach(t),FE=i(Qf,`. Check the superclass documentation for the generic methods the
