...
- SegFormer consists of a hierarchical Transformer encoder, and a lightweight all-MLP decode head.
  [SegformerModel](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerModel) is the hierarchical Transformer encoder (which in the paper is also referred to
  as Mix Transformer or MiT). [SegformerForSemanticSegmentation](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerForSemanticSegmentation) adds the all-MLP decode head on
  top to perform semantic segmentation of images. In addition, there's
  [SegformerForImageClassification](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerForImageClassification) which can be used to - you guessed it - classify images. The
...
- One can use [SegformerFeatureExtractor](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerFeatureExtractor) to prepare images and corresponding segmentation maps
...
- One additional thing to keep in mind is that one can initialize [SegformerFeatureExtractor](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerFeatureExtractor) with
  ...
  used by [SegformerForSemanticSegmentation](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerForSemanticSegmentation)). However, other datasets use the 0 index as
...
This is the configuration class to store the configuration of a [SegformerModel](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerModel). It is used to instantiate an
...
- **return_tensors** (`str` or [TensorType](/docs/transformers/pr_14880/en/internal/file_utils#transformers.TensorType), *optional*, defaults to `'np'`) --
...
<docstring><name>forward</name><anchor>transformers.SegformerModel.forward</anchor><source>https://github.com/huggingface/transformers/blob/pr_14880/src/transformers/models/segformer/modeling_segformer.py#L480</source><parameters>[{"name": "pixel_values", "val": ""}, {"name": "output_attentions", "val": " = None"}, {"name": "output_hidden_states", "val": " = None"}, {"name": "return_dict", "val": " = None"}]</parameters><paramsdesc>- **pixel_values** (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`) --
  ...
  [SegformerFeatureExtractor](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerFeatureExtractor). See [SegformerFeatureExtractor.__call__()](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerFeatureExtractor.__call__) for details.
...
elements depending on the configuration ([SegformerConfig](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerConfig)) and inputs.
...
The [SegformerModel](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerModel) forward method, overrides the `__call__` special method.
...
- **attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
  heads.</retdesc></docstring>
The [SegformerForImageClassification](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerForImageClassification) forward method, overrides the `__call__` special method.
...
- **attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
  heads.</retdesc></docstring>
The [SegformerForSemanticSegmentation](/docs/transformers/pr_14880/en/model_doc/segformer#transformers.SegformerForSemanticSegmentation) forward method, overrides the `__call__` special method.
