...
1. Integration of the core DeepSpeed features via [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer). This is an everything-done-for-you type
...
2. If you don't use [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) and want to use your own Trainer where you integrated DeepSpeed
...
To deploy this feature with multiple GPUs adjust the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) command line arguments as
following:
...
To deploy DeepSpeed with one GPU adjust the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) command line arguments as follows:
...
When you execute the program, DeepSpeed will log the configuration it received from the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer)
...
[Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) via [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) then for the `deepspeed` argument you can
pass a nested `dict`. This allows you to create the configuration on the fly and doesn't require you to write it to
the file system before passing it to [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments).
...
Some configuration values are required by both the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) and DeepSpeed to function correctly,
therefore, to prevent conflicting definitions, which could lead to hard to detect errors, we chose to configure those
via the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) command line arguments.

Additionally, some configuration values are derived automatically based on the model's configuration, so instead of
remembering to manually adjust multiple values, it's the best to let the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) do the majority
...
[Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) arguments and DeepSpeed configurations agree. For example, are you using the same
...
and configure [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) based on that. The steps are:

1. Create or load the DeepSpeed configuration to be used as a master configuration
2. Create the [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) object based on these values

Do note that some values, such as `scheduler.params.total_num_steps` are calculated by
[Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) during `train`, but you can of course do the math yourself.
...
This section has to be configured exclusively via DeepSpeed configuration - the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) provides
...
therefore set these values to `auto` and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically assign the recommended
...
If you don't configure the `optimizer` entry in the configuration file, the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will
...
But then you're on your own synchronizing the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) command line arguments and the DeepSpeed
configuration.
...
If you don't configure the `scheduler` entry in the configuration file, the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will use
...
Since *"auto"* is used the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) arguments will set the correct values in the configuration
...
and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically enable or disable it based on the value of
...
and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically configure it based on the values of `args.fp16_backend` and
`args.fp16_opt_level`.
...
and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically set `train_micro_batch_size_per_gpu` to the value of
...
and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically set it to the value of `args.gradient_accumulation_steps`.
...
and the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) will automatically set it to the value of `args.max_grad_norm`.
...
[TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) object if the passed DeepSpeed configuration file contains ZeRO-3 config
section. Thus you must create the [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) object **before** calling
...
Otherwise you just need to pass the usual [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) arguments. For example:
...
2. either the command line arguments if you were using the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) or
   [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) arguments if you were scripting the Trainer setup yourself. Please do not
   dump the [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) as it has dozens of entries that are irrelevant.
...
- DeepSpeed works with the PyTorch [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) but not TF `TFTrainer`
...
- You don't have to use the [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) to use DeepSpeed with 🤗 Transformers - you can use any model
...
The [HfDeepSpeedConfig](/docs/transformers/pr_10638/en/main_classes/deepspeed#transformers.deepspeed.HfDeepSpeedConfig) is used to integrate Deepspeed into the 🤗 Transformers core
functionality, when [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) is not used. The only thing that it does is handling Deepspeed ZeRO 3 param gathering and automatically splitting the model onto multiple gpus during `from_pretrained` call. Everything else you have to do by yourself.

When using [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) everything is automatically taken care of.

When not using [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer), to efficiently deploy DeepSpeed stage 3, you must instantiate the
[HfDeepSpeedConfig](/docs/transformers/pr_10638/en/main_classes/deepspeed#transformers.deepspeed.HfDeepSpeedConfig) object before instantiating the model.
...
[Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) uses the `HfTrainerDeepSpeedConfig` subclass instead. That subclass has logic to sync the configuration
with values of [TrainingArguments](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.TrainingArguments) by replacing special placeholder values: `"auto"`. Without this special logic
...
<div class="docstring">
<docstring><name>del\_config\_sub\_tree</name><anchor>transformers.deepspeed.HfDeepSpeedConfig.del_config_sub_tree</anchor><source>https://github.com/huggingface/transformers/blob/pr_10638/src/transformers/deepspeed.py#L114</source><parameters>[{"name": "ds_key_long", "val": ""}, {"name": "must_exist", "val": " = False"}]</parameters></docstring>
...
<div class="docstring">
<docstring><name>get\_value</name><anchor>transformers.deepspeed.HfDeepSpeedConfig.get_value</anchor><source>https://github.com/huggingface/transformers/blob/pr_10638/src/transformers/deepspeed.py#L105</source><parameters>[{"name": "ds_key_long", "val": ""}, {"name": "default", "val": " = None"}]</parameters></docstring>
...
<div class="docstring">
<docstring><name>is\_false</name><anchor>transformers.deepspeed.HfDeepSpeedConfig.is_false</anchor><source>https://github.com/huggingface/transformers/blob/pr_10638/src/transformers/deepspeed.py#L146</source><parameters>[{"name": "ds_key_long", "val": ""}]</parameters></docstring>
...
<div class="docstring">
<docstring><name>is\_true</name><anchor>transformers.deepspeed.HfDeepSpeedConfig.is_true</anchor><source>https://github.com/huggingface/transformers/blob/pr_10638/src/transformers/deepspeed.py#L137</source><parameters>[{"name": "ds_key_long", "val": ""}]</parameters></docstring>
...
Finally, please, remember that, HuggingFace [Trainer](/docs/transformers/pr_10638/en/main_classes/trainer#transformers.Trainer) only integrates DeepSpeed, therefore if you
