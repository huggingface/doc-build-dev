...
debug_overflow = DebugUnderflowOverflow(model)`}}),je=new P({props:{codee:`Detected inf/nan during batch_number=0
...
<span class="hljs-attribute">0</span>.<span class="hljs-number">00</span>e+<span class="hljs-number">00</span>      inf output`}}),_e=new P({props:{codee:`                  encoder.block.2.layer.1.layer_norm T5LayerNorm
8.69e-02 4.18e-01 weight
2.65e-04 3.42e+03 input[0]
...
<span class="hljs-attribute">8</span>.<span class="hljs-number">69</span>e-<span class="hljs-number">02</span> <span class="hljs-number">4</span>.<span class="hljs-number">18</span>e-<span class="hljs-number">01</span> weight
<span class="hljs-attribute">2</span>.<span class="hljs-number">65</span>e-<span class="hljs-number">04</span> <span class="hljs-number">3</span>.<span class="hljs-number">42</span>e+<span class="hljs-number">03</span> input[<span class="hljs-number">0</span>]
<span class="hljs-attribute">1</span>.<span class="hljs-number">79</span>e-<span class="hljs-number">06</span> <span class="hljs-number">4</span>.<span class="hljs-number">65</span>e+<span class="hljs-number">00</span> output`}}),ve=new P({props:{codee:`Detected inf/nan during batch_number=0
Last 21 forward frames:
abs min  abs max  metadata
[...]
                  encoder.block.2.layer.1.DenseReluDense.wi_0 Linear
...
<span class="hljs-attribute">3</span>.<span class="hljs-number">18</span>e-<span class="hljs-number">04</span> <span class="hljs-number">6</span>.<span class="hljs-number">27</span>e+<span class="hljs-number">04</span> output
                  <span class="hljs-attribute">encoder</span>.block.<span class="hljs-number">2</span>.layer.<span class="hljs-number">1</span>.dropout Dropout
<span class="hljs-attribute">3</span>.<span class="hljs-number">18</span>e-<span class="hljs-number">04</span> <span class="hljs-number">6</span>.<span class="hljs-number">27</span>e+<span class="hljs-number">04</span> input[<span class="hljs-number">0</span>]
<span class="hljs-attribute">0</span>.<span class="hljs-number">00</span>e+<span class="hljs-number">00</span>      inf output`}}),ye=new P({props:{codee:`class T5DenseGatedGeluDense(nn.Module):
    ...
    <span class="hljs-keyword">def</span> <span class="hljs-title function_">forward</span>(<span class="hljs-params">self, hidden_states</span>):
        ...
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.wo(hidden_states)
        <span class="hljs-keyword">return</span> hidden_states`}}),ge=new P({props:{codee:`def _forward(self, hidden_states):
    hidden_gelu = self.gelu_act(self.wi_0(hidden_states))
    hidden_linear = self.wi_1(hidden_states)
    hidden_states = hidden_gelu * hidden_linear
    hidden_states = self.dropout(hidden_states)
...
<span class="hljs-keyword">def</span> <span class="hljs-title function_">forward</span>(<span class="hljs-params">self, hidden_states</span>):
    ...
    <span class="hljs-keyword">else</span>:
        <span class="hljs-keyword">return</span> self._forward(hidden_states)`}}),$e=new P({props:{codee:`from debug_utils import detect_overflow
...
<span class="hljs-keyword">class</span> <span class="hljs-title class_">T5LayerFF</span>(nn.Module):
    ...
    <span class="hljs-keyword">def</span> <span class="hljs-title function_">forward</span>(<span class="hljs-params">self, hidden_states</span>):
        ...
        <span class="hljs-keyword">return</span> hidden_states + self.dropout(forwarded_states)`}}),De=new P({props:{codee:`from .debug_utils import DebugUnderflowOverflow
...
debug_overflow = DebugUnderflowOverflow(model, max_frames_to_save=<span class="hljs-number">100</span>)`}}),Ee=new it({}),ke=new P({props:{codee:"debug_overflow = DebugUnderflowOverflow(model, trace_batch_nums=[1, 3]),",highlighted:'debug_overflow = DebugUnderflowOverflow(model, trace_batch_nums=[<span class="hljs-number">1</span>, <span class="hljs-number">3</span>])'}}),Oe=new P({props:{codee:`                  *** Starting batch number=1 ***
abs min  abs max  metadata
                  shared Embedding
...
for. If we look just at this frame:`),va=u(),m(_e.$$.fragment),ya=u(),L=r("p"),wn=t("Here, "),_s=r("code"),jn=t("encoder.block.2.layer.1.layer_norm"),_n=t(` indicates that it was a layer norm for the first layer, of the second
block of the encoder. And the specific calls of the `),vs=r("code"),vn=t("forward"),yn=t(" is "),ys=r("code"),gn=t("T5LayerNorm"),$n=t("."),ga=u(),Me=r("p"),Dn=t("Let\u2019s look at the last few frames of that report:"),$a=u(),m(ve.$$.fragment),Da=u(),k=r("p"),En=t("The last frame reports for "),gs=r("code"),kn=t("Dropout.forward"),On=t(` function with the first entry for the only input and the second for the
...
input elements was `),Es=r("code"),Ln=t("6.27e+04"),An=t(" and same for the output was "),ks=r("code"),Nn=t("inf"),Gn=t("."),Ea=u(),A=r("p"),Un=t("You can see here, that "),Os=r("code"),Fn=t("T5DenseGatedGeluDense.forward"),Sn=t(` resulted in output activations, whose absolute max value was
...
numbers.`),Oa=u(),ee=r("p"),Yn=t("Let\u2019s match the report to the code from "),xs=r("code"),Bn=t("models/t5/modeling_t5.py"),Kn=t(":"),Pa=u(),m(ye.$$.fragment),Ca=u(),se=r("p"),zn=t("Now it\u2019s easy to see the "),Ts=r("code"),Wn=t("dropout"),Jn=t(" call, and all the previous calls as well."),xa=u(),ae=r("p"),Qn=t("Since the detection is happening in a forward hook, these reports are printed immediately after each "),Ls=r("code"),Vn=t("forward"),Xn=t(`
...
enabled, after moving the original `),Gs=r("code"),nl=t("forward"),ll=t(" into a helper wrapper, like so:"),La=u(),m(ge.$$.fragment),Aa=u(),R=r("p"),rl=t(`Since the automatic detector only reports on inputs and outputs of full frames, once you know where to look, you may
...
`),Fs=r("code"),il=t("detect_overflow"),ul=t(" helper function to inject the detector where you want it, for example:"),Na=u(),m($e.$$.fragment),Ga=u(),G=r("p"),cl=t("You can see that we added 2 of these and now we track if "),Ss=r("code"),hl=t("inf"),dl=t(" or "),Rs=r("code"),fl=t("nan"),ml=t(" for "),Is=r("code"),bl=t("forwarded_states"),wl=t(` was detected
...
right to that area. Here is a sample truncated output for such configuration:`),Ka=u(),m(Oe.$$.fragment),za=u(),We=r("p"),Tl=t(`Here you will get a huge number of frames dumped - as many as there were forward calls in your model, so it may or may
...
`),Ue=o(Qe,"A",{href:!0});var Zl=i(Ue);Rt=n(Zl,"TrainingArguments"),Zl.forEach(s),It=n(Qe," object."),Qe.forEach(s),ha=c(e),Fe=o(e,"P",{});var er=i(Fe);Mt=n(er,"If you\u2019re using your own training loop or another Trainer you can accomplish the same with:"),er.forEach(s),da=c(e),b(we.$$.fragment,e),fa=c(e),x=o(e,"P",{});var Y=i(x);Se=o(Y,"A",{href:!0});var sr=i(Se);qt=n(sr,"DebugUnderflowOverflow"),sr.forEach(s),Ht=n(Y,` inserts hooks into the model that immediately after each
forward call will test input and output variables and also the corresponding module\u2019s weights. As soon as `),ps=o(Y,"CODE",{});var ar=i(ps);Yt=n(ar,"inf"),ar.forEach(s),Bt=n(Y,` or
`),is=o(Y,"CODE",{});var tr=i(is);Kt=n(tr,"nan"),tr.forEach(s),zt=n(Y,` is detected in at least one element of the activations or weights, the program will assert and print a report
...
for. If we look just at this frame:`),fr.forEach(s),va=c(e),b(_e.$$.fragment,e),ya=c(e),L=o(e,"P",{});var oe=i(L);wn=n(oe,"Here, "),_s=o(oe,"CODE",{});var mr=i(_s);jn=n(mr,"encoder.block.2.layer.1.layer_norm"),mr.forEach(s),_n=n(oe,` indicates that it was a layer norm for the first layer, of the second
