...
the 🤗 Transformers library provides an API with the class [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) to let you fine-tune or train
...
Since PyTorch does not provide a training loop, the 🤗 Transformers library provides a [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer)
...
Then, to define our [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer), we will need to instantiate a
[TrainingArguments](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.TrainingArguments). This class contains all the hyperparameters we can tune for the
[Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) or the flags to activate the different training options it supports. Let's begin by
...
Then we can instantiate a [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) like this:
...
[Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) to compute any metrics. Let's have a look on how to do that now!

To have the [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) compute and report metrics, we need to give it a `compute_metrics`
function that takes predictions and labels (grouped in a namedtuple called [EvalPrediction](/docs/transformers/pr_15335/en/internal/trainer_utils#transformers.EvalPrediction)) and
...
To check if this works on practice, let's create a new [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) with our fine-tuned model:
...
See the documentation of [TrainingArguments](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.TrainingArguments) for more options.
...
scheduler. The default optimizer used by the [Trainer](/docs/transformers/pr_15335/en/main_classes/trainer#transformers.Trainer) is [AdamW](/docs/transformers/pr_15335/en/main_classes/optimizer_schedules#transformers.AdamW):
