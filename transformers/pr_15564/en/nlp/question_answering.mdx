...
Question answering tasks return an answer given a question. This type of NLP task can be extractive where the model identifies an answer from the given context. The task can also be open-ended for more of a challenge where a model generates an answer from the given context. 
...
The `answers` field gives you the starting position of the answer and the `text` of the answer.
...
Use 🤗 Datasets [`map`](https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.map) function to apply the preprocessing function over the entire dataset. You can also set `batched=True` to process multiple elements of the dataset at once for faster preprocessing. Remove the columns you don't need:

```py
>>> tokenized_squad = squad.map(preprocess_function, batched=True, remove_columns=squad["train"].column_names)
```

Use [DefaultDataCollator](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DefaultDataCollator) to batch the examples together:
...
Load DistilBERT with [AutoModelForQuestionAnswering](/docs/transformers/pr_15564/en/model_doc/auto#transformers.AutoModelForQuestionAnswering):
...
Convert your datasets to the `tf.data.Dataset` format with [`to_tf_dataset`](https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.to_tf_dataset). Specify inputs and the start and end positions of an answer in `columns`:
