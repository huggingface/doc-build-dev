...
Use [DataCollatorForTokenClassification](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DataCollatorForTokenClassification) to batch the examples together. It will also pad your text and labels, so they are a uniform length. While it is possible to pad your text and labels in the `tokenizer` function by setting `padding=True`, it is more efficient to only pad the text and labels to the length of the longest element in its batch. This is known as *dynamic padding* and is enabled by [DataCollatorForTokenClassification](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DataCollatorForTokenClassification).
