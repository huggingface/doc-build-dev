...
<h2 id="load-imdb-dataset">Load IMDb dataset</h2>
...
Create a preprocessing function to tokenize `text` and add a truncation strategy to truncate sequences to be no longer than DistilBERT's maximum input length:
...
Use 🤗 Datasets [`map`](https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.map) function to apply the preprocessing function to the entire dataset. You can speed up the preprocessing by setting `batched=True` to process multiple elements of the dataset at once:

```py
tokenized_imdb = imdb.map(preprocess_function, batched=True)
```

Use [DataCollatorWithPadding](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DataCollatorWithPadding) to batch the examples together. It will also pad your text, so they are a uniform length. While it is possible to pad your text in the `tokenizer` function by setting `padding=True`, it is more efficient to only pad the text to the length of the longest element in its batch. This is known as *dynamic padding* and is enabled by [DataCollatorWithPadding](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DataCollatorWithPadding).
...
Load DistilBERT with [AutoModelForSequenceClassification](/docs/transformers/pr_15564/en/model_doc/auto#transformers.AutoModelForSequenceClassification) along with the number of expected labels:
...
[Trainer](/docs/transformers/pr_15564/en/main_classes/trainer#transformers.Trainer) will apply dynamic padding by default when you pass `tokenizer` to it. You don't need to specify a data collator explicitly.
...
Start by batching the processed examples together with dynamic padding using [DataCollatorWithPadding](/docs/transformers/pr_15564/en/main_classes/data_collator#transformers.DataCollatorWithPadding). Dynamic padding is more efficient because it only pads text to the longest element in its batch. Make sure you set `return_tensors="tf"` to return `tf.Tensor`'s instead of PyTorch tensors!
...
Load DistilBERT with [TFAutoModelForSequenceClassification](/docs/transformers/pr_15564/en/model_doc/auto#transformers.TFAutoModelForSequenceClassification) along with the number of expected labels:
