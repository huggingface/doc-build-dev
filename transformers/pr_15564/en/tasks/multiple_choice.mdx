...
A multiple choice task selects the most correct input given a question and multiple possible inputs. This makes it similar to a question answering task, both tasks require a model to return the most probable answer. 
...
1. Make four copies of the `sent1` field so you can combine each of them with `sent2` to create how a sentence starts.
...
`DataCollatorForMultipleChoice` will flatten all the model inputs, apply padding, and then unflatten the results. When you create this data collator, instruct it to return `tf.Tensor`'s instead of PyTorch tensors:
...
Convert your datasets to the `tf.data.Dataset` format with [`to_tf_dataset`](https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.to_tf_dataset). Specify inputs in `columns`, target in `label_cols`, whether to shuffle the dataset order, batch size, and the data collator:
