...
Summarization creates a shorter version of a long document that captures all the important information. Along with translation, it is another example of a task that can be formulated as a sequence-to-sequence task. Summarization can be:
...
2. Use a context manager to parallelize the tokenization with the `as_target_tokenizer()` function. This will tokenize the `text` field first, followed by the `summary`.
