...
[timm library](https://github.com/rwightman/pytorch-image-models/tree/master/timm) into a [PreTrainedModel](/docs/transformers/pr_15612/en/main_classes/model#transformers.PreTrainedModel).
...
You can also use any other method of the [PretrainedConfig](/docs/transformers/pr_15612/en/main_classes/configuration#transformers.PretrainedConfig) class, like [push_to_hub()](/docs/transformers/pr_15612/en/main_classes/model#transformers.file_utils.PushToHubMixin.push_to_hub) to
...
extracts the hidden features from a batch of images (like [BertModel](/docs/transformers/pr_15612/en/model_doc/bert#transformers.BertModel)) and one that is suitable for image
...
usable inside the [Trainer](/docs/transformers/pr_15612/en/main_classes/trainer#transformers.Trainer) class. Using another output format is fine as long as you are planning on using your own
...
Again, you can use any of the methods of [PreTrainedModel](/docs/transformers/pr_15612/en/main_classes/model#transformers.PreTrainedModel), like [save_pretrained()](/docs/transformers/pr_15612/en/main_classes/model#transformers.PreTrainedModel.save_pretrained) or
[push_to_hub()](/docs/transformers/pr_15612/en/main_classes/model#transformers.file_utils.PushToHubMixin.push_to_hub). We will use the second in the next section, and see how to push the model weights
...
Now let's see how to make sure that when we do [save_pretrained()](/docs/transformers/pr_15612/en/main_classes/model#transformers.PreTrainedModel.save_pretrained) or [push_to_hub()](/docs/transformers/pr_15612/en/main_classes/model#transformers.file_utils.PushToHubMixin.push_to_hub), the
...
[AutoConfig](/docs/transformers/pr_15612/en/model_doc/auto#transformers.AutoConfig)) but it's different for models. Your custom model could be suitable for many different tasks, so you
...
Note that the first argument used when registering your custom config to [AutoConfig](/docs/transformers/pr_15612/en/model_doc/auto#transformers.AutoConfig) needs to match the `model_type`
