...
<p>Indices can be obtained using <a href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17022/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
...
elements depending on the configuration (<a
  href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraConfig"
...
</ul>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1064",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.models.electra.modeling_electra.ElectraForPreTrainingOutput"
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1549",returnDescription:`
<p>A <a
...
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1178",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L967",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1453",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1265",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_electra.py#L1347",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L931",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17022/en/main_classes/output#transformers.modeling_tf_outputs.TFBaseModelOutputWithPastAndCrossAttentions"
...
Whether or not to use the model in training mode (some modules like dropout modules have different
behaviors between training and evaluation).`,name:"training"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1029",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.models.electra.modeling_tf_electra.TFElectraForPreTrainingOutput"
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1159",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1271",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1365",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1482",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_tf_electra.py#L1564",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17022/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_flax_electra.py#L674"}}),zi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraModel.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraModel.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17022/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17022/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Whether or not to return a <a href="/docs/transformers/pr_17022/en/main_classes/output#transformers.utils.ModelOutput">ModelOutput</a> instead of a plain tuple.`,name:"return_dict"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_flax_electra.py#L576",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17022/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_flax_electra.py#L823"}}),Si=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForPreTraining.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForPreTraining.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17022/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17022/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17022/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_flax_electra.py#L760"}}),Gi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForMaskedLM.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForMaskedLM.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17022/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17022/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17022/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17022/src/transformers/models/electra/modeling_flax_electra.py#L1133"}}),Ll=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForQuestionAnswering.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForQuestionAnswering.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17022/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17022/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17022/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
behavior.`),BT=c(),b(ts.$$.fragment),VT=c(),xt=n("div"),b(Nr.$$.fragment),KT=c(),Do=n("p"),JT=r("The "),Ed=n("a"),GT=r("TFElectraModel"),XT=r(" forward method, overrides the "),dp=n("code"),YT=r("__call__"),ZT=r(" special method."),e2=c(),b(os.$$.fragment),t2=c(),b(ns.$$.fragment),_u=c(),Wo=n("h2"),ss=n("a"),cp=n("span"),b(Ir.$$.fragment),o2=c(),pp=n("span"),n2=r("TFElectraForPreTraining"),ku=c(),Oe=n("div"),b(Sr.$$.fragment),s2=c(),hp=n("p"),a2=r("Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),r2=c(),mp=n("p"),i2=r(`Even though both the discriminator and generator may be loaded into this model, the discriminator is the only model
of the two to have the correct classification head to be used for this model.`),l2=c(),Dr=n("p"),d2=r("This model inherits from "),wd=n("a"),c2=r("TFPreTrainedModel"),p2=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),h2=c(),Wr=n("p"),m2=r("This model is also a "),Ur=n("a"),u2=r("tf.keras.Model"),f2=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),g2=c(),b(as.$$.fragment),_2=c(),Mt=n("div"),b(Rr.$$.fragment),k2=c(),Uo=n("p"),v2=r("The "),$d=n("a"),T2=r("TFElectraForPreTraining"),b2=r(" forward method, overrides the "),up=n("code"),y2=r("__call__"),E2=r(" special method."),w2=c(),b(rs.$$.fragment),$2=c(),b(is.$$.fragment),vu=c(),Ro=n("h2"),ls=n("a"),fp=n("span"),b(Hr.$$.fragment),F2=c(),gp=n("span"),x2=r("TFElectraForMaskedLM"),Tu=c(),Le=n("div"),b(Qr.$$.fragment),M2=c(),_p=n("p"),z2=r("Electra model with a language modeling head on top."),C2=c(),kp=n("p"),j2=r(`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),q2=c(),Br=n("p"),P2=r("This model inherits from "),Fd=n("a"),A2=r("TFPreTrainedModel"),O2=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),L2=c(),Vr=n("p"),N2=r("This model is also a "),Kr=n("a"),I2=r("tf.keras.Model"),S2=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),D2=c(),b(ds.$$.fragment),W2=c(),it=n("div"),b(Jr.$$.fragment),U2=c(),Ho=n("p"),R2=r("The "),xd=n("a"),H2=r("TFElectraForMaskedLM"),Q2=r(" forward method, overrides the "),vp=n("code"),B2=r("__call__"),V2=r(" special method."),K2=c(),b(cs.$$.fragment),J2=c(),b(ps.$$.fragment),G2=c(),b(hs.$$.fragment),bu=c(),Qo=n("h2"),ms=n("a"),Tp=n("span"),b(Gr.$$.fragment),X2=c(),bp=n("span"),Y2=r("TFElectraForSequenceClassification"),yu=c(),Ke=n("div"),b(Xr.$$.fragment),Z2=c(),yp=n("p"),eb=r(`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
...
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Yy=c(),Wp=n("p"),Zy=r("Finally, this model supports inherent JAX features such as:"),e1=c(),Qt=n("ul"),Up=n("li"),$i=n("a"),t1=r("Just-In-Time (JIT) compilation"),o1=c(),Rp=n("li"),Fi=n("a"),n1=r("Automatic Differentiation"),s1=c(),Hp=n("li"),xi=n("a"),a1=r("Vectorization"),r1=c(),Qp=n("li"),Mi=n("a"),i1=r("Parallelization"),l1=c(),Ct=n("div"),b(zi.$$.fragment),d1=c(),tn=n("p"),c1=r("The "),Bp=n("code"),p1=r("FlaxElectraPreTrainedModel"),h1=r(" forward method, overrides the "),Vp=n("code"),m1=r("__call__"),u1=r(" special method."),f1=c(),b(Ps.$$.fragment),g1=c(),b(As.$$.fragment),ju=c(),on=n("h2"),Os=n("a"),Kp=n("span"),b(Ci.$$.fragment),_1=c(),Jp=n("span"),k1=r("FlaxElectraForPreTraining"),qu=c(),Pe=n("div"),b(ji.$$.fragment),v1=c(),Gp=n("p"),T1=r("Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),b1=c(),Xp=n("p"),y1=r("It is recommended to load the discriminator checkpoint into that model."),E1=c(),qi=n("p"),w1=r("This model inherits from "),Nd=n("a"),$1=r("FlaxPreTrainedModel"),F1=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),x1=c(),Pi=n("p"),M1=r(`This model is also a Flax Linen
`),Ai=n("a"),z1=r("flax.nn.Module"),C1=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),j1=c(),Yp=n("p"),q1=r("Finally, this model supports inherent JAX features such as:"),P1=c(),Bt=n("ul"),Zp=n("li"),Oi=n("a"),A1=r("Just-In-Time (JIT) compilation"),O1=c(),eh=n("li"),Li=n("a"),L1=r("Automatic Differentiation"),N1=c(),th=n("li"),Ni=n("a"),I1=r("Vectorization"),S1=c(),oh=n("li"),Ii=n("a"),D1=r("Parallelization"),W1=c(),jt=n("div"),b(Si.$$.fragment),U1=c(),nn=n("p"),R1=r("The "),nh=n("code"),H1=r("FlaxElectraPreTrainedModel"),Q1=r(" forward method, overrides the "),sh=n("code"),B1=r("__call__"),V1=r(" special method."),K1=c(),b(Ls.$$.fragment),J1=c(),b(Ns.$$.fragment),Pu=c(),sn=n("h2"),Is=n("a"),ah=n("span"),b(Di.$$.fragment),G1=c(),rh=n("span"),X1=r("FlaxElectraForMaskedLM"),Au=c(),Se=n("div"),b(Wi.$$.fragment),Y1=c(),Ui=n("p"),Z1=r("Electra Model with a "),ih=n("code"),eE=r("language modeling"),tE=r(" head on top."),oE=c(),Ri=n("p"),nE=r("This model inherits from "),Id=n("a"),sE=r("FlaxPreTrainedModel"),aE=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),rE=c(),Hi=n("p"),iE=r(`This model is also a Flax Linen
`),Qi=n("a"),lE=r("flax.nn.Module"),dE=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),cE=c(),lh=n("p"),pE=r("Finally, this model supports inherent JAX features such as:"),hE=c(),Vt=n("ul"),dh=n("li"),Bi=n("a"),mE=r("Just-In-Time (JIT) compilation"),uE=c(),ch=n("li"),Vi=n("a"),fE=r("Automatic Differentiation"),gE=c(),ph=n("li"),Ki=n("a"),_E=r("Vectorization"),kE=c(),hh=n("li"),Ji=n("a"),vE=r("Parallelization"),TE=c(),qt=n("div"),b(Gi.$$.fragment),bE=c(),an=n("p"),yE=r("The "),mh=n("code"),EE=r("FlaxElectraPreTrainedModel"),wE=r(" forward method, overrides the "),uh=n("code"),$E=r("__call__"),FE=r(" special method."),xE=c(),b(Ss.$$.fragment),ME=c(),b(Ds.$$.fragment),Ou=c(),rn=n("h2"),Ws=n("a"),fh=n("span"),b(Xi.$$.fragment),zE=c(),gh=n("span"),CE=r("FlaxElectraForSequenceClassification"),Lu=c(),De=n("div"),b(Yi.$$.fragment),jE=c(),_h=n("p"),qE=r(`Electra Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),PE=c(),Zi=n("p"),AE=r("This model inherits from "),Sd=n("a"),OE=r("FlaxPreTrainedModel"),LE=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),NE=c(),el=n("p"),IE=r(`This model is also a Flax Linen
`),tl=n("a"),SE=r("flax.nn.Module"),DE=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),WE=c(),kh=n("p"),UE=r("Finally, this model supports inherent JAX features such as:"),RE=c(),Kt=n("ul"),vh=n("li"),ol=n("a"),HE=r("Just-In-Time (JIT) compilation"),QE=c(),Th=n("li"),nl=n("a"),BE=r("Automatic Differentiation"),VE=c(),bh=n("li"),sl=n("a"),KE=r("Vectorization"),JE=c(),yh=n("li"),al=n("a"),GE=r("Parallelization"),XE=c(),Pt=n("div"),b(rl.$$.fragment),YE=c(),ln=n("p"),ZE=r("The "),Eh=n("code"),ew=r("FlaxElectraPreTrainedModel"),tw=r(" forward method, overrides the "),wh=n("code"),ow=r("__call__"),nw=r(" special method."),sw=c(),b(Us.$$.fragment),aw=c(),b(Rs.$$.fragment),Nu=c(),dn=n("h2"),Hs=n("a"),$h=n("span"),b(il.$$.fragment),rw=c(),Fh=n("span"),iw=r("FlaxElectraForMultipleChoice"),Iu=c(),We=n("div"),b(ll.$$.fragment),lw=c(),xh=n("p"),dw=r(`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),cw=c(),dl=n("p"),pw=r("This model inherits from "),Dd=n("a"),hw=r("FlaxPreTrainedModel"),mw=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),uw=c(),cl=n("p"),fw=r(`This model is also a Flax Linen
`),pl=n("a"),gw=r("flax.nn.Module"),_w=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),kw=c(),Mh=n("p"),vw=r("Finally, this model supports inherent JAX features such as:"),Tw=c(),Jt=n("ul"),zh=n("li"),hl=n("a"),bw=r("Just-In-Time (JIT) compilation"),yw=c(),Ch=n("li"),ml=n("a"),Ew=r("Automatic Differentiation"),ww=c(),jh=n("li"),ul=n("a"),$w=r("Vectorization"),Fw=c(),qh=n("li"),fl=n("a"),xw=r("Parallelization"),Mw=c(),At=n("div"),b(gl.$$.fragment),zw=c(),cn=n("p"),Cw=r("The "),Ph=n("code"),jw=r("FlaxElectraPreTrainedModel"),qw=r(" forward method, overrides the "),Ah=n("code"),Pw=r("__call__"),Aw=r(" special method."),Ow=c(),b(Qs.$$.fragment),Lw=c(),b(Bs.$$.fragment),Su=c(),pn=n("h2"),Vs=n("a"),Oh=n("span"),b(_l.$$.fragment),Nw=c(),Lh=n("span"),Iw=r("FlaxElectraForTokenClassification"),Du=c(),Ae=n("div"),b(kl.$$.fragment),Sw=c(),Nh=n("p"),Dw=r("Electra model with a token classification head on top."),Ww=c(),Ih=n("p"),Uw=r("Both the discriminator and generator may be loaded into this model."),Rw=c(),vl=n("p"),Hw=r("This model inherits from "),Wd=n("a"),Qw=r("FlaxPreTrainedModel"),Bw=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Vw=c(),Tl=n("p"),Kw=r(`This model is also a Flax Linen
`),bl=n("a"),Jw=r("flax.nn.Module"),Gw=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Xw=c(),Sh=n("p"),Yw=r("Finally, this model supports inherent JAX features such as:"),Zw=c(),Gt=n("ul"),Dh=n("li"),yl=n("a"),e$=r("Just-In-Time (JIT) compilation"),t$=c(),Wh=n("li"),El=n("a"),o$=r("Automatic Differentiation"),n$=c(),Uh=n("li"),wl=n("a"),s$=r("Vectorization"),a$=c(),Rh=n("li"),$l=n("a"),r$=r("Parallelization"),i$=c(),Ot=n("div"),b(Fl.$$.fragment),l$=c(),hn=n("p"),d$=r("The "),Hh=n("code"),c$=r("FlaxElectraPreTrainedModel"),p$=r(" forward method, overrides the "),Qh=n("code"),h$=r("__call__"),m$=r(" special method."),u$=c(),b(Ks.$$.fragment),f$=c(),b(Js.$$.fragment),Wu=c(),mn=n("h2"),Gs=n("a"),Bh=n("span"),b(xl.$$.fragment),g$=c(),Vh=n("span"),_$=r("FlaxElectraForQuestionAnswering"),Uu=c(),Ue=n("div"),b(Ml.$$.fragment),k$=c(),un=n("p"),v$=r(`ELECTRA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
...
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),yf.forEach(t),BT=p(Dt),y(ts.$$.fragment,Dt),VT=p(Dt),xt=s(Dt,"DIV",{class:!0});var aa=a(xt);y(Nr.$$.fragment,aa),KT=p(aa),Do=s(aa,"P",{});var Yd=a(Do);JT=i(Yd,"The "),Ed=s(Yd,"A",{href:!0});var vF=a(Ed);GT=i(vF,"TFElectraModel"),vF.forEach(t),XT=i(Yd," forward method, overrides the "),dp=s(Yd,"CODE",{});var TF=a(dp);YT=i(TF,"__call__"),TF.forEach(t),ZT=i(Yd," special method."),Yd.forEach(t),e2=p(aa),y(os.$$.fragment,aa),t2=p(aa),y(ns.$$.fragment,aa),aa.forEach(t),Dt.forEach(t),_u=p(o),Wo=s(o,"H2",{class:!0});var Ef=a(Wo);ss=s(Ef,"A",{id:!0,class:!0,href:!0});var bF=a(ss);cp=s(bF,"SPAN",{});var yF=a(cp);y(Ir.$$.fragment,yF),yF.forEach(t),bF.forEach(t),o2=p(Ef),pp=s(Ef,"SPAN",{});var EF=a(pp);n2=i(EF,"TFElectraForPreTraining"),EF.forEach(t),Ef.forEach(t),ku=p(o),Oe=s(o,"DIV",{class:!0});var ht=a(Oe);y(Sr.$$.fragment,ht),s2=p(ht),hp=s(ht,"P",{});var wF=a(hp);a2=i(wF,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),wF.forEach(t),r2=p(ht),mp=s(ht,"P",{});var $F=a(mp);i2=i($F,`Even though both the discriminator and generator may be loaded into this model, the discriminator is the only model
of the two to have the correct classification head to be used for this model.`),$F.forEach(t),l2=p(ht),Dr=s(ht,"P",{});var wf=a(Dr);d2=i(wf,"This model inherits from "),wd=s(wf,"A",{href:!0});var FF=a(wd);c2=i(FF,"TFPreTrainedModel"),FF.forEach(t),p2=i(wf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),wf.forEach(t),h2=p(ht),Wr=s(ht,"P",{});var $f=a(Wr);m2=i($f,"This model is also a "),Ur=s($f,"A",{href:!0,rel:!0});var xF=a(Ur);u2=i(xF,"tf.keras.Model"),xF.forEach(t),f2=i($f,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),$f.forEach(t),g2=p(ht),y(as.$$.fragment,ht),_2=p(ht),Mt=s(ht,"DIV",{class:!0});var ra=a(Mt);y(Rr.$$.fragment,ra),k2=p(ra),Uo=s(ra,"P",{});var Zd=a(Uo);v2=i(Zd,"The "),$d=s(Zd,"A",{href:!0});var MF=a($d);T2=i(MF,"TFElectraForPreTraining"),MF.forEach(t),b2=i(Zd," forward method, overrides the "),up=s(Zd,"CODE",{});var zF=a(up);y2=i(zF,"__call__"),zF.forEach(t),E2=i(Zd," special method."),Zd.forEach(t),w2=p(ra),y(rs.$$.fragment,ra),$2=p(ra),y(is.$$.fragment,ra),ra.forEach(t),ht.forEach(t),vu=p(o),Ro=s(o,"H2",{class:!0});var Ff=a(Ro);ls=s(Ff,"A",{id:!0,class:!0,href:!0});var CF=a(ls);fp=s(CF,"SPAN",{});var jF=a(fp);y(Hr.$$.fragment,jF),jF.forEach(t),CF.forEach(t),F2=p(Ff),gp=s(Ff,"SPAN",{});var qF=a(gp);x2=i(qF,"TFElectraForMaskedLM"),qF.forEach(t),Ff.forEach(t),Tu=p(o),Le=s(o,"DIV",{class:!0});var mt=a(Le);y(Qr.$$.fragment,mt),M2=p(mt),_p=s(mt,"P",{});var PF=a(_p);z2=i(PF,"Electra model with a language modeling head on top."),PF.forEach(t),C2=p(mt),kp=s(mt,"P",{});var AF=a(kp);j2=i(AF,`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),AF.forEach(t),q2=p(mt),Br=s(mt,"P",{});var xf=a(Br);P2=i(xf,"This model inherits from "),Fd=s(xf,"A",{href:!0});var OF=a(Fd);A2=i(OF,"TFPreTrainedModel"),OF.forEach(t),O2=i(xf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),xf.forEach(t),L2=p(mt),Vr=s(mt,"P",{});var Mf=a(Vr);N2=i(Mf,"This model is also a "),Kr=s(Mf,"A",{href:!0,rel:!0});var LF=a(Kr);I2=i(LF,"tf.keras.Model"),LF.forEach(t),S2=i(Mf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Mf.forEach(t),D2=p(mt),y(ds.$$.fragment,mt),W2=p(mt),it=s(mt,"DIV",{class:!0});var ho=a(it);y(Jr.$$.fragment,ho),U2=p(ho),Ho=s(ho,"P",{});var ec=a(Ho);R2=i(ec,"The "),xd=s(ec,"A",{href:!0});var NF=a(xd);H2=i(NF,"TFElectraForMaskedLM"),NF.forEach(t),Q2=i(ec," forward method, overrides the "),vp=s(ec,"CODE",{});var IF=a(vp);B2=i(IF,"__call__"),IF.forEach(t),V2=i(ec," special method."),ec.forEach(t),K2=p(ho),y(cs.$$.fragment,ho),J2=p(ho),y(ps.$$.fragment,ho),G2=p(ho),y(hs.$$.fragment,ho),ho.forEach(t),mt.forEach(t),bu=p(o),Qo=s(o,"H2",{class:!0});var zf=a(Qo);ms=s(zf,"A",{id:!0,class:!0,href:!0});var SF=a(ms);Tp=s(SF,"SPAN",{});var DF=a(Tp);y(Gr.$$.fragment,DF),DF.forEach(t),SF.forEach(t),X2=p(zf),bp=s(zf,"SPAN",{});var WF=a(bp);Y2=i(WF,"TFElectraForSequenceClassification"),WF.forEach(t),zf.forEach(t),yu=p(o),Ke=s(o,"DIV",{class:!0});var Wt=a(Ke);y(Xr.$$.fragment,Wt),Z2=p(Wt),yp=s(Wt,"P",{});var UF=a(yp);eb=i(UF,`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
...
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Rf.forEach(t),Yy=p(ft),Wp=s(ft,"P",{});var $x=a(Wp);Zy=i($x,"Finally, this model supports inherent JAX features such as:"),$x.forEach(t),e1=p(ft),Qt=s(ft,"UL",{});var la=a(Qt);Up=s(la,"LI",{});var Fx=a(Up);$i=s(Fx,"A",{href:!0,rel:!0});var xx=a($i);t1=i(xx,"Just-In-Time (JIT) compilation"),xx.forEach(t),Fx.forEach(t),o1=p(la),Rp=s(la,"LI",{});var Mx=a(Rp);Fi=s(Mx,"A",{href:!0,rel:!0});var zx=a(Fi);n1=i(zx,"Automatic Differentiation"),zx.forEach(t),Mx.forEach(t),s1=p(la),Hp=s(la,"LI",{});var Cx=a(Hp);xi=s(Cx,"A",{href:!0,rel:!0});var jx=a(xi);a1=i(jx,"Vectorization"),jx.forEach(t),Cx.forEach(t),r1=p(la),Qp=s(la,"LI",{});var qx=a(Qp);Mi=s(qx,"A",{href:!0,rel:!0});var Px=a(Mi);i1=i(Px,"Parallelization"),Px.forEach(t),qx.forEach(t),la.forEach(t),l1=p(ft),Ct=s(ft,"DIV",{class:!0});var da=a(Ct);y(zi.$$.fragment,da),d1=p(da),tn=s(da,"P",{});var rc=a(tn);c1=i(rc,"The "),Bp=s(rc,"CODE",{});var Ax=a(Bp);p1=i(Ax,"FlaxElectraPreTrainedModel"),Ax.forEach(t),h1=i(rc," forward method, overrides the "),Vp=s(rc,"CODE",{});var Ox=a(Vp);m1=i(Ox,"__call__"),Ox.forEach(t),u1=i(rc," special method."),rc.forEach(t),f1=p(da),y(Ps.$$.fragment,da),g1=p(da),y(As.$$.fragment,da),da.forEach(t),ft.forEach(t),ju=p(o),on=s(o,"H2",{class:!0});var Hf=a(on);Os=s(Hf,"A",{id:!0,class:!0,href:!0});var Lx=a(Os);Kp=s(Lx,"SPAN",{});var Nx=a(Kp);y(Ci.$$.fragment,Nx),Nx.forEach(t),Lx.forEach(t),_1=p(Hf),Jp=s(Hf,"SPAN",{});var Ix=a(Jp);k1=i(Ix,"FlaxElectraForPreTraining"),Ix.forEach(t),Hf.forEach(t),qu=p(o),Pe=s(o,"DIV",{class:!0});var Xe=a(Pe);y(ji.$$.fragment,Xe),v1=p(Xe),Gp=s(Xe,"P",{});var Sx=a(Gp);T1=i(Sx,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),Sx.forEach(t),b1=p(Xe),Xp=s(Xe,"P",{});var Dx=a(Xp);y1=i(Dx,"It is recommended to load the discriminator checkpoint into that model."),Dx.forEach(t),E1=p(Xe),qi=s(Xe,"P",{});var Qf=a(qi);w1=i(Qf,"This model inherits from "),Nd=s(Qf,"A",{href:!0});var Wx=a(Nd);$1=i(Wx,"FlaxPreTrainedModel"),Wx.forEach(t),F1=i(Qf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Qf.forEach(t),x1=p(Xe),Pi=s(Xe,"P",{});var Bf=a(Pi);M1=i(Bf,`This model is also a Flax Linen
`),Ai=s(Bf,"A",{href:!0,rel:!0});var Ux=a(Ai);z1=i(Ux,"flax.nn.Module"),Ux.forEach(t),C1=i(Bf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Bf.forEach(t),j1=p(Xe),Yp=s(Xe,"P",{});var Rx=a(Yp);q1=i(Rx,"Finally, this model supports inherent JAX features such as:"),Rx.forEach(t),P1=p(Xe),Bt=s(Xe,"UL",{});var ca=a(Bt);Zp=s(ca,"LI",{});var Hx=a(Zp);Oi=s(Hx,"A",{href:!0,rel:!0});var Qx=a(Oi);A1=i(Qx,"Just-In-Time (JIT) compilation"),Qx.forEach(t),Hx.forEach(t),O1=p(ca),eh=s(ca,"LI",{});var Bx=a(eh);Li=s(Bx,"A",{href:!0,rel:!0});var Vx=a(Li);L1=i(Vx,"Automatic Differentiation"),Vx.forEach(t),Bx.forEach(t),N1=p(ca),th=s(ca,"LI",{});var Kx=a(th);Ni=s(Kx,"A",{href:!0,rel:!0});var Jx=a(Ni);I1=i(Jx,"Vectorization"),Jx.forEach(t),Kx.forEach(t),S1=p(ca),oh=s(ca,"LI",{});var Gx=a(oh);Ii=s(Gx,"A",{href:!0,rel:!0});var Xx=a(Ii);D1=i(Xx,"Parallelization"),Xx.forEach(t),Gx.forEach(t),ca.forEach(t),W1=p(Xe),jt=s(Xe,"DIV",{class:!0});var pa=a(jt);y(Si.$$.fragment,pa),U1=p(pa),nn=s(pa,"P",{});var ic=a(nn);R1=i(ic,"The "),nh=s(ic,"CODE",{});var Yx=a(nh);H1=i(Yx,"FlaxElectraPreTrainedModel"),Yx.forEach(t),Q1=i(ic," forward method, overrides the "),sh=s(ic,"CODE",{});var Zx=a(sh);B1=i(Zx,"__call__"),Zx.forEach(t),V1=i(ic," special method."),ic.forEach(t),K1=p(pa),y(Ls.$$.fragment,pa),J1=p(pa),y(Ns.$$.fragment,pa),pa.forEach(t),Xe.forEach(t),Pu=p(o),sn=s(o,"H2",{class:!0});var Vf=a(sn);Is=s(Vf,"A",{id:!0,class:!0,href:!0});var eM=a(Is);ah=s(eM,"SPAN",{});var tM=a(ah);y(Di.$$.fragment,tM),tM.forEach(t),eM.forEach(t),G1=p(Vf),rh=s(Vf,"SPAN",{});var oM=a(rh);X1=i(oM,"FlaxElectraForMaskedLM"),oM.forEach(t),Vf.forEach(t),Au=p(o),Se=s(o,"DIV",{class:!0});var gt=a(Se);y(Wi.$$.fragment,gt),Y1=p(gt),Ui=s(gt,"P",{});var Kf=a(Ui);Z1=i(Kf,"Electra Model with a "),ih=s(Kf,"CODE",{});var nM=a(ih);eE=i(nM,"language modeling"),nM.forEach(t),tE=i(Kf," head on top."),Kf.forEach(t),oE=p(gt),Ri=s(gt,"P",{});var Jf=a(Ri);nE=i(Jf,"This model inherits from "),Id=s(Jf,"A",{href:!0});var sM=a(Id);sE=i(sM,"FlaxPreTrainedModel"),sM.forEach(t),aE=i(Jf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Jf.forEach(t),rE=p(gt),Hi=s(gt,"P",{});var Gf=a(Hi);iE=i(Gf,`This model is also a Flax Linen
`),Qi=s(Gf,"A",{href:!0,rel:!0});var aM=a(Qi);lE=i(aM,"flax.nn.Module"),aM.forEach(t),dE=i(Gf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Gf.forEach(t),cE=p(gt),lh=s(gt,"P",{});var rM=a(lh);pE=i(rM,"Finally, this model supports inherent JAX features such as:"),rM.forEach(t),hE=p(gt),Vt=s(gt,"UL",{});var ha=a(Vt);dh=s(ha,"LI",{});var iM=a(dh);Bi=s(iM,"A",{href:!0,rel:!0});var lM=a(Bi);mE=i(lM,"Just-In-Time (JIT) compilation"),lM.forEach(t),iM.forEach(t),uE=p(ha),ch=s(ha,"LI",{});var dM=a(ch);Vi=s(dM,"A",{href:!0,rel:!0});var cM=a(Vi);fE=i(cM,"Automatic Differentiation"),cM.forEach(t),dM.forEach(t),gE=p(ha),ph=s(ha,"LI",{});var pM=a(ph);Ki=s(pM,"A",{href:!0,rel:!0});var hM=a(Ki);_E=i(hM,"Vectorization"),hM.forEach(t),pM.forEach(t),kE=p(ha),hh=s(ha,"LI",{});var mM=a(hh);Ji=s(mM,"A",{href:!0,rel:!0});var uM=a(Ji);vE=i(uM,"Parallelization"),uM.forEach(t),mM.forEach(t),ha.forEach(t),TE=p(gt),qt=s(gt,"DIV",{class:!0});var ma=a(qt);y(Gi.$$.fragment,ma),bE=p(ma),an=s(ma,"P",{});var lc=a(an);yE=i(lc,"The "),mh=s(lc,"CODE",{});var fM=a(mh);EE=i(fM,"FlaxElectraPreTrainedModel"),fM.forEach(t),wE=i(lc," forward method, overrides the "),uh=s(lc,"CODE",{});var gM=a(uh);$E=i(gM,"__call__"),gM.forEach(t),FE=i(lc," special method."),lc.forEach(t),xE=p(ma),y(Ss.$$.fragment,ma),ME=p(ma),y(Ds.$$.fragment,ma),ma.forEach(t),gt.forEach(t),Ou=p(o),rn=s(o,"H2",{class:!0});var Xf=a(rn);Ws=s(Xf,"A",{id:!0,class:!0,href:!0});var _M=a(Ws);fh=s(_M,"SPAN",{});var kM=a(fh);y(Xi.$$.fragment,kM),kM.forEach(t),_M.forEach(t),zE=p(Xf),gh=s(Xf,"SPAN",{});var vM=a(gh);CE=i(vM,"FlaxElectraForSequenceClassification"),vM.forEach(t),Xf.forEach(t),Lu=p(o),De=s(o,"DIV",{class:!0});var _t=a(De);y(Yi.$$.fragment,_t),jE=p(_t),_h=s(_t,"P",{});var TM=a(_h);qE=i(TM,`Electra Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),TM.forEach(t),PE=p(_t),Zi=s(_t,"P",{});var Yf=a(Zi);AE=i(Yf,"This model inherits from "),Sd=s(Yf,"A",{href:!0});var bM=a(Sd);OE=i(bM,"FlaxPreTrainedModel"),bM.forEach(t),LE=i(Yf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Yf.forEach(t),NE=p(_t),el=s(_t,"P",{});var Zf=a(el);IE=i(Zf,`This model is also a Flax Linen
`),tl=s(Zf,"A",{href:!0,rel:!0});var yM=a(tl);SE=i(yM,"flax.nn.Module"),yM.forEach(t),DE=i(Zf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Zf.forEach(t),WE=p(_t),kh=s(_t,"P",{});var EM=a(kh);UE=i(EM,"Finally, this model supports inherent JAX features such as:"),EM.forEach(t),RE=p(_t),Kt=s(_t,"UL",{});var ua=a(Kt);vh=s(ua,"LI",{});var wM=a(vh);ol=s(wM,"A",{href:!0,rel:!0});var $M=a(ol);HE=i($M,"Just-In-Time (JIT) compilation"),$M.forEach(t),wM.forEach(t),QE=p(ua),Th=s(ua,"LI",{});var FM=a(Th);nl=s(FM,"A",{href:!0,rel:!0});var xM=a(nl);BE=i(xM,"Automatic Differentiation"),xM.forEach(t),FM.forEach(t),VE=p(ua),bh=s(ua,"LI",{});var MM=a(bh);sl=s(MM,"A",{href:!0,rel:!0});var zM=a(sl);KE=i(zM,"Vectorization"),zM.forEach(t),MM.forEach(t),JE=p(ua),yh=s(ua,"LI",{});var CM=a(yh);al=s(CM,"A",{href:!0,rel:!0});var jM=a(al);GE=i(jM,"Parallelization"),jM.forEach(t),CM.forEach(t),ua.forEach(t),XE=p(_t),Pt=s(_t,"DIV",{class:!0});var fa=a(Pt);y(rl.$$.fragment,fa),YE=p(fa),ln=s(fa,"P",{});var dc=a(ln);ZE=i(dc,"The "),Eh=s(dc,"CODE",{});var qM=a(Eh);ew=i(qM,"FlaxElectraPreTrainedModel"),qM.forEach(t),tw=i(dc," forward method, overrides the "),wh=s(dc,"CODE",{});var PM=a(wh);ow=i(PM,"__call__"),PM.forEach(t),nw=i(dc," special method."),dc.forEach(t),sw=p(fa),y(Us.$$.fragment,fa),aw=p(fa),y(Rs.$$.fragment,fa),fa.forEach(t),_t.forEach(t),Nu=p(o),dn=s(o,"H2",{class:!0});var eg=a(dn);Hs=s(eg,"A",{id:!0,class:!0,href:!0});var AM=a(Hs);$h=s(AM,"SPAN",{});var OM=a($h);y(il.$$.fragment,OM),OM.forEach(t),AM.forEach(t),rw=p(eg),Fh=s(eg,"SPAN",{});var LM=a(Fh);iw=i(LM,"FlaxElectraForMultipleChoice"),LM.forEach(t),eg.forEach(t),Iu=p(o),We=s(o,"DIV",{class:!0});var kt=a(We);y(ll.$$.fragment,kt),lw=p(kt),xh=s(kt,"P",{});var NM=a(xh);dw=i(NM,`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),NM.forEach(t),cw=p(kt),dl=s(kt,"P",{});var tg=a(dl);pw=i(tg,"This model inherits from "),Dd=s(tg,"A",{href:!0});var IM=a(Dd);hw=i(IM,"FlaxPreTrainedModel"),IM.forEach(t),mw=i(tg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),tg.forEach(t),uw=p(kt),cl=s(kt,"P",{});var og=a(cl);fw=i(og,`This model is also a Flax Linen
`),pl=s(og,"A",{href:!0,rel:!0});var SM=a(pl);gw=i(SM,"flax.nn.Module"),SM.forEach(t),_w=i(og,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),og.forEach(t),kw=p(kt),Mh=s(kt,"P",{});var DM=a(Mh);vw=i(DM,"Finally, this model supports inherent JAX features such as:"),DM.forEach(t),Tw=p(kt),Jt=s(kt,"UL",{});var ga=a(Jt);zh=s(ga,"LI",{});var WM=a(zh);hl=s(WM,"A",{href:!0,rel:!0});var UM=a(hl);bw=i(UM,"Just-In-Time (JIT) compilation"),UM.forEach(t),WM.forEach(t),yw=p(ga),Ch=s(ga,"LI",{});var RM=a(Ch);ml=s(RM,"A",{href:!0,rel:!0});var HM=a(ml);Ew=i(HM,"Automatic Differentiation"),HM.forEach(t),RM.forEach(t),ww=p(ga),jh=s(ga,"LI",{});var QM=a(jh);ul=s(QM,"A",{href:!0,rel:!0});var BM=a(ul);$w=i(BM,"Vectorization"),BM.forEach(t),QM.forEach(t),Fw=p(ga),qh=s(ga,"LI",{});var VM=a(qh);fl=s(VM,"A",{href:!0,rel:!0});var KM=a(fl);xw=i(KM,"Parallelization"),KM.forEach(t),VM.forEach(t),ga.forEach(t),Mw=p(kt),At=s(kt,"DIV",{class:!0});var _a=a(At);y(gl.$$.fragment,_a),zw=p(_a),cn=s(_a,"P",{});var cc=a(cn);Cw=i(cc,"The "),Ph=s(cc,"CODE",{});var JM=a(Ph);jw=i(JM,"FlaxElectraPreTrainedModel"),JM.forEach(t),qw=i(cc," forward method, overrides the "),Ah=s(cc,"CODE",{});var GM=a(Ah);Pw=i(GM,"__call__"),GM.forEach(t),Aw=i(cc," special method."),cc.forEach(t),Ow=p(_a),y(Qs.$$.fragment,_a),Lw=p(_a),y(Bs.$$.fragment,_a),_a.forEach(t),kt.forEach(t),Su=p(o),pn=s(o,"H2",{class:!0});var ng=a(pn);Vs=s(ng,"A",{id:!0,class:!0,href:!0});var XM=a(Vs);Oh=s(XM,"SPAN",{});var YM=a(Oh);y(_l.$$.fragment,YM),YM.forEach(t),XM.forEach(t),Nw=p(ng),Lh=s(ng,"SPAN",{});var ZM=a(Lh);Iw=i(ZM,"FlaxElectraForTokenClassification"),ZM.forEach(t),ng.forEach(t),Du=p(o),Ae=s(o,"DIV",{class:!0});var Ye=a(Ae);y(kl.$$.fragment,Ye),Sw=p(Ye),Nh=s(Ye,"P",{});var ez=a(Nh);Dw=i(ez,"Electra model with a token classification head on top."),ez.forEach(t),Ww=p(Ye),Ih=s(Ye,"P",{});var tz=a(Ih);Uw=i(tz,"Both the discriminator and generator may be loaded into this model."),tz.forEach(t),Rw=p(Ye),vl=s(Ye,"P",{});var sg=a(vl);Hw=i(sg,"This model inherits from "),Wd=s(sg,"A",{href:!0});var oz=a(Wd);Qw=i(oz,"FlaxPreTrainedModel"),oz.forEach(t),Bw=i(sg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),sg.forEach(t),Vw=p(Ye),Tl=s(Ye,"P",{});var ag=a(Tl);Kw=i(ag,`This model is also a Flax Linen
`),bl=s(ag,"A",{href:!0,rel:!0});var nz=a(bl);Jw=i(nz,"flax.nn.Module"),nz.forEach(t),Gw=i(ag,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),ag.forEach(t),Xw=p(Ye),Sh=s(Ye,"P",{});var sz=a(Sh);Yw=i(sz,"Finally, this model supports inherent JAX features such as:"),sz.forEach(t),Zw=p(Ye),Gt=s(Ye,"UL",{});var ka=a(Gt);Dh=s(ka,"LI",{});var az=a(Dh);yl=s(az,"A",{href:!0,rel:!0});var rz=a(yl);e$=i(rz,"Just-In-Time (JIT) compilation"),rz.forEach(t),az.forEach(t),t$=p(ka),Wh=s(ka,"LI",{});var iz=a(Wh);El=s(iz,"A",{href:!0,rel:!0});var lz=a(El);o$=i(lz,"Automatic Differentiation"),lz.forEach(t),iz.forEach(t),n$=p(ka),Uh=s(ka,"LI",{});var dz=a(Uh);wl=s(dz,"A",{href:!0,rel:!0});var cz=a(wl);s$=i(cz,"Vectorization"),cz.forEach(t),dz.forEach(t),a$=p(ka),Rh=s(ka,"LI",{});var pz=a(Rh);$l=s(pz,"A",{href:!0,rel:!0});var hz=a($l);r$=i(hz,"Parallelization"),hz.forEach(t),pz.forEach(t),ka.forEach(t),i$=p(Ye),Ot=s(Ye,"DIV",{class:!0});var va=a(Ot);y(Fl.$$.fragment,va),l$=p(va),hn=s(va,"P",{});var pc=a(hn);d$=i(pc,"The "),Hh=s(pc,"CODE",{});var mz=a(Hh);c$=i(mz,"FlaxElectraPreTrainedModel"),mz.forEach(t),p$=i(pc," forward method, overrides the "),Qh=s(pc,"CODE",{});var uz=a(Qh);h$=i(uz,"__call__"),uz.forEach(t),m$=i(pc," special method."),pc.forEach(t),u$=p(va),y(Ks.$$.fragment,va),f$=p(va),y(Js.$$.fragment,va),va.forEach(t),Ye.forEach(t),Wu=p(o),mn=s(o,"H2",{class:!0});var rg=a(mn);Gs=s(rg,"A",{id:!0,class:!0,href:!0});var fz=a(Gs);Bh=s(fz,"SPAN",{});var gz=a(Bh);y(xl.$$.fragment,gz),gz.forEach(t),fz.forEach(t),g$=p(rg),Vh=s(rg,"SPAN",{});var _z=a(Vh);_$=i(_z,"FlaxElectraForQuestionAnswering"),_z.forEach(t),rg.forEach(t),Uu=p(o),Ue=s(o,"DIV",{class:!0});var vt=a(Ue);y(Ml.$$.fragment,vt),k$=p(vt),un=s(vt,"P",{});var hc=a(un);v$=i(hc,`ELECTRA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
