...
<p>Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
heads.`,name:"attentions"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L805"}}),Ka=new we({}),Ja=new I({props:{name:"class transformers.ElectraModel",anchor:"transformers.ElectraModel",parameters:[{name:"config",val:""}],parametersDescription:[{anchor:"transformers.ElectraModel.config",description:`<strong>config</strong> (<a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraConfig">ElectraConfig</a>) &#x2014; Model configuration class with all the parameters of the model.
...
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
...
elements depending on the configuration (<a
  href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraConfig"
...
</ul>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1069",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.models.electra.modeling_electra.ElectraForPreTrainingOutput"
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1554",returnDescription:`
<p>A <a
...
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1183",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L972",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1458",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1270",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_electra.py#L1352",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L931",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17212/en/main_classes/output#transformers.modeling_tf_outputs.TFBaseModelOutputWithPastAndCrossAttentions"
...
Whether or not to use the model in training mode (some modules like dropout modules have different
behaviors between training and evaluation).`,name:"training"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1029",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.models.electra.modeling_tf_electra.TFElectraForPreTrainingOutput"
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1159",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1271",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1365",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1482",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_tf_electra.py#L1564",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17212/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L894"}}),Si=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraModel.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"encoder_hidden_states",val:" = None"},{name:"encoder_attention_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"},{name:"past_key_values",val:": dict = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraModel.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17212/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Whether or not to return a <a href="/docs/transformers/pr_17212/en/main_classes/output#transformers.utils.ModelOutput">ModelOutput</a> instead of a plain tuple.`,name:"return_dict"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L746",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17212/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L1043"}}),Gi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForPreTraining.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"encoder_hidden_states",val:" = None"},{name:"encoder_attention_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"},{name:"past_key_values",val:": dict = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForPreTraining.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17212/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17212/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L1526"}}),rl=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForCausalLM.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"encoder_hidden_states",val:" = None"},{name:"encoder_attention_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"},{name:"past_key_values",val:": dict = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForCausalLM.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17212/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17212/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L980"}}),_l=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForMaskedLM.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"encoder_hidden_states",val:" = None"},{name:"encoder_attention_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"},{name:"past_key_values",val:": dict = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForMaskedLM.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17212/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17212/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17212/src/transformers/models/electra/modeling_flax_electra.py#L1353"}}),nd=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForQuestionAnswering.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"encoder_hidden_states",val:" = None"},{name:"encoder_attention_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"},{name:"past_key_values",val:": dict = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForQuestionAnswering.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17212/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17212/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17212/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),e2=c(),b(ps.$$.fragment),t2=c(),Ct=n("div"),b(ei.$$.fragment),o2=c(),Qo=n("p"),n2=r("The "),Kd=n("a"),s2=r("TFElectraForPreTraining"),a2=r(" forward method, overrides the "),Dp=n("code"),r2=r("__call__"),i2=r(" special method."),l2=c(),b(hs.$$.fragment),d2=c(),b(ms.$$.fragment),ef=c(),Vo=n("h2"),us=n("a"),Sp=n("span"),b(ti.$$.fragment),c2=c(),Wp=n("span"),p2=r("TFElectraForMaskedLM"),tf=c(),Le=n("div"),b(oi.$$.fragment),h2=c(),Up=n("p"),m2=r("Electra model with a language modeling head on top."),u2=c(),Rp=n("p"),f2=r(`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),g2=c(),ni=n("p"),_2=r("This model inherits from "),Jd=n("a"),k2=r("TFPreTrainedModel"),v2=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),T2=c(),si=n("p"),b2=r("This model is also a "),ai=n("a"),y2=r("tf.keras.Model"),E2=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),w2=c(),b(fs.$$.fragment),$2=c(),lt=n("div"),b(ri.$$.fragment),F2=c(),Ko=n("p"),x2=r("The "),Gd=n("a"),M2=r("TFElectraForMaskedLM"),z2=r(" forward method, overrides the "),Hp=n("code"),C2=r("__call__"),j2=r(" special method."),q2=c(),b(gs.$$.fragment),P2=c(),b(_s.$$.fragment),A2=c(),b(ks.$$.fragment),of=c(),Jo=n("h2"),vs=n("a"),Bp=n("span"),b(ii.$$.fragment),O2=c(),Qp=n("span"),L2=r("TFElectraForSequenceClassification"),nf=c(),Je=n("div"),b(li.$$.fragment),N2=c(),Vp=n("p"),I2=r(`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),D2=c(),di=n("p"),S2=r("This model inherits from "),Xd=n("a"),W2=r("TFPreTrainedModel"),U2=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),R2=c(),ci=n("p"),H2=r("This model is also a "),pi=n("a"),B2=r("tf.keras.Model"),Q2=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),V2=c(),b(Ts.$$.fragment),K2=c(),dt=n("div"),b(hi.$$.fragment),J2=c(),Go=n("p"),G2=r("The "),Yd=n("a"),X2=r("TFElectraForSequenceClassification"),Y2=r(" forward method, overrides the "),Kp=n("code"),Z2=r("__call__"),eb=r(" special method."),tb=c(),b(bs.$$.fragment),ob=c(),b(ys.$$.fragment),nb=c(),b(Es.$$.fragment),sf=c(),Xo=n("h2"),ws=n("a"),Jp=n("span"),b(mi.$$.fragment),sb=c(),Gp=n("span"),ab=r("TFElectraForMultipleChoice"),af=c(),Ge=n("div"),b(ui.$$.fragment),rb=c(),Xp=n("p"),ib=r(`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),lb=c(),fi=n("p"),db=r("This model inherits from "),Zd=n("a"),cb=r("TFPreTrainedModel"),pb=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),hb=c(),gi=n("p"),mb=r("This model is also a "),_i=n("a"),ub=r("tf.keras.Model"),fb=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),gb=c(),b($s.$$.fragment),_b=c(),jt=n("div"),b(ki.$$.fragment),kb=c(),Yo=n("p"),vb=r("The "),ec=n("a"),Tb=r("TFElectraForMultipleChoice"),bb=r(" forward method, overrides the "),Yp=n("code"),yb=r("__call__"),Eb=r(" special method."),wb=c(),b(Fs.$$.fragment),$b=c(),b(xs.$$.fragment),rf=c(),Zo=n("h2"),Ms=n("a"),Zp=n("span"),b(vi.$$.fragment),Fb=c(),eh=n("span"),xb=r("TFElectraForTokenClassification"),lf=c(),Ne=n("div"),b(Ti.$$.fragment),Mb=c(),th=n("p"),zb=r("Electra model with a token classification head on top."),Cb=c(),oh=n("p"),jb=r("Both the discriminator and generator may be loaded into this model."),qb=c(),bi=n("p"),Pb=r("This model inherits from "),tc=n("a"),Ab=r("TFPreTrainedModel"),Ob=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Lb=c(),yi=n("p"),Nb=r("This model is also a "),Ei=n("a"),Ib=r("tf.keras.Model"),Db=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Sb=c(),b(zs.$$.fragment),Wb=c(),ct=n("div"),b(wi.$$.fragment),Ub=c(),en=n("p"),Rb=r("The "),oc=n("a"),Hb=r("TFElectraForTokenClassification"),Bb=r(" forward method, overrides the "),nh=n("code"),Qb=r("__call__"),Vb=r(" special method."),Kb=c(),b(Cs.$$.fragment),Jb=c(),b(js.$$.fragment),Gb=c(),b(qs.$$.fragment),df=c(),tn=n("h2"),Ps=n("a"),sh=n("span"),b($i.$$.fragment),Xb=c(),ah=n("span"),Yb=r("TFElectraForQuestionAnswering"),cf=c(),Xe=n("div"),b(Fi.$$.fragment),Zb=c(),on=n("p"),ey=r(`Electra Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),rh=n("code"),ty=r("span start logits"),oy=r(" and "),ih=n("code"),ny=r("span end logits"),sy=r(")."),ay=c(),xi=n("p"),ry=r("This model inherits from "),nc=n("a"),iy=r("TFPreTrainedModel"),ly=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),dy=c(),Mi=n("p"),cy=r("This model is also a "),zi=n("a"),py=r("tf.keras.Model"),hy=r(` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),my=c(),b(As.$$.fragment),uy=c(),pt=n("div"),b(Ci.$$.fragment),fy=c(),nn=n("p"),gy=r("The "),sc=n("a"),_y=r("TFElectraForQuestionAnswering"),ky=r(" forward method, overrides the "),lh=n("code"),vy=r("__call__"),Ty=r(" special method."),by=c(),b(Os.$$.fragment),yy=c(),b(Ls.$$.fragment),Ey=c(),b(Ns.$$.fragment),pf=c(),sn=n("h2"),Is=n("a"),dh=n("span"),b(ji.$$.fragment),wy=c(),ch=n("span"),$y=r("FlaxElectraModel"),hf=c(),Ie=n("div"),b(qi.$$.fragment),Fy=c(),ph=n("p"),xy=r("The bare Electra Model transformer outputting raw hidden-states without any specific head on top."),My=c(),Pi=n("p"),zy=r("This model inherits from "),ac=n("a"),Cy=r("FlaxPreTrainedModel"),jy=r(`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),qy=c(),Ai=n("p"),Py=r(`This model is also a Flax Linen
`),Oi=n("a"),Ay=r("flax.nn.Module"),Oy=r(` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Ly=c(),hh=n("p"),Ny=r("Finally, this model supports inherent JAX features such as:"),Iy=c(),Kt=n("ul"),mh=n("li"),Li=n("a"),Dy=r("Just-In-Time (JIT) compilation"),Sy=c(),uh=n("li"),Ni=n("a"),Wy=r("Automatic Differentiation"),Uy=c(),fh=n("li"),Ii=n("a"),Ry=r("Vectorization"),Hy=c(),gh=n("li"),Di=n("a"),By=r("Parallelization"),Qy=c(),qt=n("div"),b(Si.$$.fragment),Vy=c(),an=n("p"),Ky=r("The "),_h=n("code"),Jy=r("FlaxElectraPreTrainedModel"),Gy=r(" forward method, overrides the "),kh=n("code"),Xy=r("__call__"),Yy=r(" special method."),Zy=c(),b(Ds.$$.fragment),eE=c(),b(Ss.$$.fragment),mf=c(),rn=n("h2"),Ws=n("a"),vh=n("span"),b(Wi.$$.fragment),tE=c(),Th=n("span"),oE=r("FlaxElectraForPreTraining"),uf=c(),Pe=n("div"),b(Ui.$$.fragment),nE=c(),bh=n("p"),sE=r("Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),aE=c(),yh=n("p"),rE=r("It is recommended to load the discriminator checkpoint into that model."),iE=c(),Ri=n("p"),lE=r("This model inherits from "),rc=n("a"),dE=r("FlaxPreTrainedModel"),cE=r(`. Check the superclass documentation for the generic methods the
...
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),rg.forEach(t),e2=p(mt),y(ps.$$.fragment,mt),t2=p(mt),Ct=s(mt,"DIV",{class:!0});var fa=a(Ct);y(ei.$$.fragment,fa),o2=p(fa),Qo=s(fa,"P",{});var Ec=a(Qo);n2=i(Ec,"The "),Kd=s(Ec,"A",{href:!0});var Rx=a(Kd);s2=i(Rx,"TFElectraForPreTraining"),Rx.forEach(t),a2=i(Ec," forward method, overrides the "),Dp=s(Ec,"CODE",{});var Hx=a(Dp);r2=i(Hx,"__call__"),Hx.forEach(t),i2=i(Ec," special method."),Ec.forEach(t),l2=p(fa),y(hs.$$.fragment,fa),d2=p(fa),y(ms.$$.fragment,fa),fa.forEach(t),mt.forEach(t),ef=p(o),Vo=s(o,"H2",{class:!0});var ig=a(Vo);us=s(ig,"A",{id:!0,class:!0,href:!0});var Bx=a(us);Sp=s(Bx,"SPAN",{});var Qx=a(Sp);y(ti.$$.fragment,Qx),Qx.forEach(t),Bx.forEach(t),c2=p(ig),Wp=s(ig,"SPAN",{});var Vx=a(Wp);p2=i(Vx,"TFElectraForMaskedLM"),Vx.forEach(t),ig.forEach(t),tf=p(o),Le=s(o,"DIV",{class:!0});var ut=a(Le);y(oi.$$.fragment,ut),h2=p(ut),Up=s(ut,"P",{});var Kx=a(Up);m2=i(Kx,"Electra model with a language modeling head on top."),Kx.forEach(t),u2=p(ut),Rp=s(ut,"P",{});var Jx=a(Rp);f2=i(Jx,`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),Jx.forEach(t),g2=p(ut),ni=s(ut,"P",{});var lg=a(ni);_2=i(lg,"This model inherits from "),Jd=s(lg,"A",{href:!0});var Gx=a(Jd);k2=i(Gx,"TFPreTrainedModel"),Gx.forEach(t),v2=i(lg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),lg.forEach(t),T2=p(ut),si=s(ut,"P",{});var dg=a(si);b2=i(dg,"This model is also a "),ai=s(dg,"A",{href:!0,rel:!0});var Xx=a(ai);y2=i(Xx,"tf.keras.Model"),Xx.forEach(t),E2=i(dg,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),dg.forEach(t),w2=p(ut),y(fs.$$.fragment,ut),$2=p(ut),lt=s(ut,"DIV",{class:!0});var go=a(lt);y(ri.$$.fragment,go),F2=p(go),Ko=s(go,"P",{});var wc=a(Ko);x2=i(wc,"The "),Gd=s(wc,"A",{href:!0});var Yx=a(Gd);M2=i(Yx,"TFElectraForMaskedLM"),Yx.forEach(t),z2=i(wc," forward method, overrides the "),Hp=s(wc,"CODE",{});var Zx=a(Hp);C2=i(Zx,"__call__"),Zx.forEach(t),j2=i(wc," special method."),wc.forEach(t),q2=p(go),y(gs.$$.fragment,go),P2=p(go),y(_s.$$.fragment,go),A2=p(go),y(ks.$$.fragment,go),go.forEach(t),ut.forEach(t),of=p(o),Jo=s(o,"H2",{class:!0});var cg=a(Jo);vs=s(cg,"A",{id:!0,class:!0,href:!0});var eM=a(vs);Bp=s(eM,"SPAN",{});var tM=a(Bp);y(ii.$$.fragment,tM),tM.forEach(t),eM.forEach(t),O2=p(cg),Qp=s(cg,"SPAN",{});var oM=a(Qp);L2=i(oM,"TFElectraForSequenceClassification"),oM.forEach(t),cg.forEach(t),nf=p(o),Je=s(o,"DIV",{class:!0});var Ht=a(Je);y(li.$$.fragment,Ht),N2=p(Ht),Vp=s(Ht,"P",{});var nM=a(Vp);I2=i(nM,`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),nM.forEach(t),D2=p(Ht),di=s(Ht,"P",{});var pg=a(di);S2=i(pg,"This model inherits from "),Xd=s(pg,"A",{href:!0});var sM=a(Xd);W2=i(sM,"TFPreTrainedModel"),sM.forEach(t),U2=i(pg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),pg.forEach(t),R2=p(Ht),ci=s(Ht,"P",{});var hg=a(ci);H2=i(hg,"This model is also a "),pi=s(hg,"A",{href:!0,rel:!0});var aM=a(pi);B2=i(aM,"tf.keras.Model"),aM.forEach(t),Q2=i(hg,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),hg.forEach(t),V2=p(Ht),y(Ts.$$.fragment,Ht),K2=p(Ht),dt=s(Ht,"DIV",{class:!0});var _o=a(dt);y(hi.$$.fragment,_o),J2=p(_o),Go=s(_o,"P",{});var $c=a(Go);G2=i($c,"The "),Yd=s($c,"A",{href:!0});var rM=a(Yd);X2=i(rM,"TFElectraForSequenceClassification"),rM.forEach(t),Y2=i($c," forward method, overrides the "),Kp=s($c,"CODE",{});var iM=a(Kp);Z2=i(iM,"__call__"),iM.forEach(t),eb=i($c," special method."),$c.forEach(t),tb=p(_o),y(bs.$$.fragment,_o),ob=p(_o),y(ys.$$.fragment,_o),nb=p(_o),y(Es.$$.fragment,_o),_o.forEach(t),Ht.forEach(t),sf=p(o),Xo=s(o,"H2",{class:!0});var mg=a(Xo);ws=s(mg,"A",{id:!0,class:!0,href:!0});var lM=a(ws);Jp=s(lM,"SPAN",{});var dM=a(Jp);y(mi.$$.fragment,dM),dM.forEach(t),lM.forEach(t),sb=p(mg),Gp=s(mg,"SPAN",{});var cM=a(Gp);ab=i(cM,"TFElectraForMultipleChoice"),cM.forEach(t),mg.forEach(t),af=p(o),Ge=s(o,"DIV",{class:!0});var Bt=a(Ge);y(ui.$$.fragment,Bt),rb=p(Bt),Xp=s(Bt,"P",{});var pM=a(Xp);ib=i(pM,`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),pM.forEach(t),lb=p(Bt),fi=s(Bt,"P",{});var ug=a(fi);db=i(ug,"This model inherits from "),Zd=s(ug,"A",{href:!0});var hM=a(Zd);cb=i(hM,"TFPreTrainedModel"),hM.forEach(t),pb=i(ug,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),ug.forEach(t),hb=p(Bt),gi=s(Bt,"P",{});var fg=a(gi);mb=i(fg,"This model is also a "),_i=s(fg,"A",{href:!0,rel:!0});var mM=a(_i);ub=i(mM,"tf.keras.Model"),mM.forEach(t),fb=i(fg,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),fg.forEach(t),gb=p(Bt),y($s.$$.fragment,Bt),_b=p(Bt),jt=s(Bt,"DIV",{class:!0});var ga=a(jt);y(ki.$$.fragment,ga),kb=p(ga),Yo=s(ga,"P",{});var Fc=a(Yo);vb=i(Fc,"The "),ec=s(Fc,"A",{href:!0});var uM=a(ec);Tb=i(uM,"TFElectraForMultipleChoice"),uM.forEach(t),bb=i(Fc," forward method, overrides the "),Yp=s(Fc,"CODE",{});var fM=a(Yp);yb=i(fM,"__call__"),fM.forEach(t),Eb=i(Fc," special method."),Fc.forEach(t),wb=p(ga),y(Fs.$$.fragment,ga),$b=p(ga),y(xs.$$.fragment,ga),ga.forEach(t),Bt.forEach(t),rf=p(o),Zo=s(o,"H2",{class:!0});var gg=a(Zo);Ms=s(gg,"A",{id:!0,class:!0,href:!0});var gM=a(Ms);Zp=s(gM,"SPAN",{});var _M=a(Zp);y(vi.$$.fragment,_M),_M.forEach(t),gM.forEach(t),Fb=p(gg),eh=s(gg,"SPAN",{});var kM=a(eh);xb=i(kM,"TFElectraForTokenClassification"),kM.forEach(t),gg.forEach(t),lf=p(o),Ne=s(o,"DIV",{class:!0});var ft=a(Ne);y(Ti.$$.fragment,ft),Mb=p(ft),th=s(ft,"P",{});var vM=a(th);zb=i(vM,"Electra model with a token classification head on top."),vM.forEach(t),Cb=p(ft),oh=s(ft,"P",{});var TM=a(oh);jb=i(TM,"Both the discriminator and generator may be loaded into this model."),TM.forEach(t),qb=p(ft),bi=s(ft,"P",{});var _g=a(bi);Pb=i(_g,"This model inherits from "),tc=s(_g,"A",{href:!0});var bM=a(tc);Ab=i(bM,"TFPreTrainedModel"),bM.forEach(t),Ob=i(_g,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),_g.forEach(t),Lb=p(ft),yi=s(ft,"P",{});var kg=a(yi);Nb=i(kg,"This model is also a "),Ei=s(kg,"A",{href:!0,rel:!0});var yM=a(Ei);Ib=i(yM,"tf.keras.Model"),yM.forEach(t),Db=i(kg,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),kg.forEach(t),Sb=p(ft),y(zs.$$.fragment,ft),Wb=p(ft),ct=s(ft,"DIV",{class:!0});var ko=a(ct);y(wi.$$.fragment,ko),Ub=p(ko),en=s(ko,"P",{});var xc=a(en);Rb=i(xc,"The "),oc=s(xc,"A",{href:!0});var EM=a(oc);Hb=i(EM,"TFElectraForTokenClassification"),EM.forEach(t),Bb=i(xc," forward method, overrides the "),nh=s(xc,"CODE",{});var wM=a(nh);Qb=i(wM,"__call__"),wM.forEach(t),Vb=i(xc," special method."),xc.forEach(t),Kb=p(ko),y(Cs.$$.fragment,ko),Jb=p(ko),y(js.$$.fragment,ko),Gb=p(ko),y(qs.$$.fragment,ko),ko.forEach(t),ft.forEach(t),df=p(o),tn=s(o,"H2",{class:!0});var vg=a(tn);Ps=s(vg,"A",{id:!0,class:!0,href:!0});var $M=a(Ps);sh=s($M,"SPAN",{});var FM=a(sh);y($i.$$.fragment,FM),FM.forEach(t),$M.forEach(t),Xb=p(vg),ah=s(vg,"SPAN",{});var xM=a(ah);Yb=i(xM,"TFElectraForQuestionAnswering"),xM.forEach(t),vg.forEach(t),cf=p(o),Xe=s(o,"DIV",{class:!0});var Qt=a(Xe);y(Fi.$$.fragment,Qt),Zb=p(Qt),on=s(Qt,"P",{});var Mc=a(on);ey=i(Mc,`Electra Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),rh=s(Mc,"CODE",{});var MM=a(rh);ty=i(MM,"span start logits"),MM.forEach(t),oy=i(Mc," and "),ih=s(Mc,"CODE",{});var zM=a(ih);ny=i(zM,"span end logits"),zM.forEach(t),sy=i(Mc,")."),Mc.forEach(t),ay=p(Qt),xi=s(Qt,"P",{});var Tg=a(xi);ry=i(Tg,"This model inherits from "),nc=s(Tg,"A",{href:!0});var CM=a(nc);iy=i(CM,"TFPreTrainedModel"),CM.forEach(t),ly=i(Tg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Tg.forEach(t),dy=p(Qt),Mi=s(Qt,"P",{});var bg=a(Mi);cy=i(bg,"This model is also a "),zi=s(bg,"A",{href:!0,rel:!0});var jM=a(zi);py=i(jM,"tf.keras.Model"),jM.forEach(t),hy=i(bg,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),bg.forEach(t),my=p(Qt),y(As.$$.fragment,Qt),uy=p(Qt),pt=s(Qt,"DIV",{class:!0});var vo=a(pt);y(Ci.$$.fragment,vo),fy=p(vo),nn=s(vo,"P",{});var zc=a(nn);gy=i(zc,"The "),sc=s(zc,"A",{href:!0});var qM=a(sc);_y=i(qM,"TFElectraForQuestionAnswering"),qM.forEach(t),ky=i(zc," forward method, overrides the "),lh=s(zc,"CODE",{});var PM=a(lh);vy=i(PM,"__call__"),PM.forEach(t),Ty=i(zc," special method."),zc.forEach(t),by=p(vo),y(Os.$$.fragment,vo),yy=p(vo),y(Ls.$$.fragment,vo),Ey=p(vo),y(Ns.$$.fragment,vo),vo.forEach(t),Qt.forEach(t),pf=p(o),sn=s(o,"H2",{class:!0});var yg=a(sn);Is=s(yg,"A",{id:!0,class:!0,href:!0});var AM=a(Is);dh=s(AM,"SPAN",{});var OM=a(dh);y(ji.$$.fragment,OM),OM.forEach(t),AM.forEach(t),wy=p(yg),ch=s(yg,"SPAN",{});var LM=a(ch);$y=i(LM,"FlaxElectraModel"),LM.forEach(t),yg.forEach(t),hf=p(o),Ie=s(o,"DIV",{class:!0});var gt=a(Ie);y(qi.$$.fragment,gt),Fy=p(gt),ph=s(gt,"P",{});var NM=a(ph);xy=i(NM,"The bare Electra Model transformer outputting raw hidden-states without any specific head on top."),NM.forEach(t),My=p(gt),Pi=s(gt,"P",{});var Eg=a(Pi);zy=i(Eg,"This model inherits from "),ac=s(Eg,"A",{href:!0});var IM=a(ac);Cy=i(IM,"FlaxPreTrainedModel"),IM.forEach(t),jy=i(Eg,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),Eg.forEach(t),qy=p(gt),Ai=s(gt,"P",{});var wg=a(Ai);Py=i(wg,`This model is also a Flax Linen
`),Oi=s(wg,"A",{href:!0,rel:!0});var DM=a(Oi);Ay=i(DM,"flax.nn.Module"),DM.forEach(t),Oy=i(wg,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),wg.forEach(t),Ly=p(gt),hh=s(gt,"P",{});var SM=a(hh);Ny=i(SM,"Finally, this model supports inherent JAX features such as:"),SM.forEach(t),Iy=p(gt),Kt=s(gt,"UL",{});var _a=a(Kt);mh=s(_a,"LI",{});var WM=a(mh);Li=s(WM,"A",{href:!0,rel:!0});var UM=a(Li);Dy=i(UM,"Just-In-Time (JIT) compilation"),UM.forEach(t),WM.forEach(t),Sy=p(_a),uh=s(_a,"LI",{});var RM=a(uh);Ni=s(RM,"A",{href:!0,rel:!0});var HM=a(Ni);Wy=i(HM,"Automatic Differentiation"),HM.forEach(t),RM.forEach(t),Uy=p(_a),fh=s(_a,"LI",{});var BM=a(fh);Ii=s(BM,"A",{href:!0,rel:!0});var QM=a(Ii);Ry=i(QM,"Vectorization"),QM.forEach(t),BM.forEach(t),Hy=p(_a),gh=s(_a,"LI",{});var VM=a(gh);Di=s(VM,"A",{href:!0,rel:!0});var KM=a(Di);By=i(KM,"Parallelization"),KM.forEach(t),VM.forEach(t),_a.forEach(t),Qy=p(gt),qt=s(gt,"DIV",{class:!0});var ka=a(qt);y(Si.$$.fragment,ka),Vy=p(ka),an=s(ka,"P",{});var Cc=a(an);Ky=i(Cc,"The "),_h=s(Cc,"CODE",{});var JM=a(_h);Jy=i(JM,"FlaxElectraPreTrainedModel"),JM.forEach(t),Gy=i(Cc," forward method, overrides the "),kh=s(Cc,"CODE",{});var GM=a(kh);Xy=i(GM,"__call__"),GM.forEach(t),Yy=i(Cc," special method."),Cc.forEach(t),Zy=p(ka),y(Ds.$$.fragment,ka),eE=p(ka),y(Ss.$$.fragment,ka),ka.forEach(t),gt.forEach(t),mf=p(o),rn=s(o,"H2",{class:!0});var $g=a(rn);Ws=s($g,"A",{id:!0,class:!0,href:!0});var XM=a(Ws);vh=s(XM,"SPAN",{});var YM=a(vh);y(Wi.$$.fragment,YM),YM.forEach(t),XM.forEach(t),tE=p($g),Th=s($g,"SPAN",{});var ZM=a(Th);oE=i(ZM,"FlaxElectraForPreTraining"),ZM.forEach(t),$g.forEach(t),uf=p(o),Pe=s(o,"DIV",{class:!0});var Ye=a(Pe);y(Ui.$$.fragment,Ye),nE=p(Ye),bh=s(Ye,"P",{});var ez=a(bh);sE=i(ez,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),ez.forEach(t),aE=p(Ye),yh=s(Ye,"P",{});var tz=a(yh);rE=i(tz,"It is recommended to load the discriminator checkpoint into that model."),tz.forEach(t),iE=p(Ye),Ri=s(Ye,"P",{});var Fg=a(Ri);lE=i(Fg,"This model inherits from "),rc=s(Fg,"A",{href:!0});var oz=a(rc);dE=i(oz,"FlaxPreTrainedModel"),oz.forEach(t),cE=i(Fg,`. Check the superclass documentation for the generic methods the
