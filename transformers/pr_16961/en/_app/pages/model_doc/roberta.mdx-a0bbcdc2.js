...
Optional second list of IDs for sequence pairs.`,name:"token_ids_1"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/tokenization_roberta.py#L340",returnDescription:`
...
Whether or not the token list is already formatted with special tokens for the model.`,name:"already_has_special_tokens"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/tokenization_roberta.py#L365",returnDescription:`
...
Optional second list of IDs for sequence pairs.`,name:"token_ids_1"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/tokenization_roberta.py#L392",returnDescription:`
<p>List of zeros.</p>
`,returnType:`
<p><code>List[int]</code></p>
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L737",returnDescription:`
<p>A <a
...
elements depending on the configuration (<a
  href="/docs/transformers/pr_16961/en/model_doc/roberta#transformers.RobertaConfig"
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L904",returnDescription:`
<p>A <a
...
Used to hide legacy arguments that have been deprecated.`,name:"kwargs"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L1059",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L1175",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L1274",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L1372",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_roberta.py#L1477",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L914",returnDescription:`
<p>A <a
...
Labels for computing the cross entropy classification loss. Indices should be in <code>[0, ..., config.vocab_size - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1171",returnDescription:`
<p>A <a
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1070",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1329",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1426",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1541",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/pr_16961/src/transformers/models/roberta/modeling_tf_roberta.py#L1627",returnDescription:`
<p>A <a
...
layers on top of the hidden-states output to compute `),Nc=s($d,"CODE",{});var o0=a(Nc);Gk=i(o0,"span start logits"),o0.forEach(t),Xk=i($d," and "),Ic=s($d,"CODE",{});var n0=a(Ic);Yk=i(n0,"span end logits"),n0.forEach(t),Zk=i($d,")."),$d.forEach(t),e1=h(ao),br=s(ao,"P",{});var jf=a(br);t1=i(jf,"This model inherits from "),Jl=s(jf,"A",{href:!0});var s0=a(Jl);o1=i(s0,"PreTrainedModel"),s0.forEach(t),n1=i(jf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),jf.forEach(t),s1=h(ao),kr=s(ao,"P",{});var Pf=a(kr);a1=i(Pf,"This model is also a PyTorch "),vr=s(Pf,"A",{href:!0,rel:!0});var a0=a(vr);r1=i(a0,"torch.nn.Module"),a0.forEach(t),i1=i(Pf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),Pf.forEach(t),l1=h(ao),lt=s(ao,"DIV",{class:!0});var ro=a(lt);y(Tr.$$.fragment,ro),d1=h(ro),qo=s(ro,"P",{});var Fd=a(qo);c1=i(Fd,"The "),Kl=s(Fd,"A",{href:!0});var r0=a(Kl);p1=i(r0,"RobertaForQuestionAnswering"),r0.forEach(t),h1=i(Fd," forward method, overrides the "),Dc=s(Fd,"CODE",{});var i0=a(Dc);m1=i(i0,"__call__"),i0.forEach(t),f1=i(Fd," special method."),Fd.forEach(t),u1=h(ro),y(Vn.$$.fragment,ro),g1=h(ro),y(Jn.$$.fragment,ro),_1=h(ro),y(Kn.$$.fragment,ro),ro.forEach(t),ao.forEach(t),zm=h(o),Co=s(o,"H2",{class:!0});var Lf=a(Co);Gn=s(Lf,"A",{id:!0,class:!0,href:!0});var l0=a(Gn);Sc=s(l0,"SPAN",{});var d0=a(Sc);y(yr.$$.fragment,d0),d0.forEach(t),l0.forEach(t),b1=h(Lf),Wc=s(Lf,"SPAN",{});var c0=a(Wc);k1=i(c0,"TFRobertaModel"),c0.forEach(t),Lf.forEach(t),qm=h(o),He=s(o,"DIV",{class:!0});var Pt=a(He);y(wr.$$.fragment,Pt),v1=h(Pt),Bc=s(Pt,"P",{});var p0=a(Bc);T1=i(p0,"The bare RoBERTa Model transformer outputting raw hidden-states without any specific head on top."),p0.forEach(t),y1=h(Pt),$r=s(Pt,"P",{});var Af=a($r);w1=i(Af,"This model inherits from "),Gl=s(Af,"A",{href:!0});var h0=a(Gl);$1=i(h0,"TFPreTrainedModel"),h0.forEach(t),F1=i(Af,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Af.forEach(t),x1=h(Pt),Fr=s(Pt,"P",{});var Of=a(Fr);R1=i(Of,"This model is also a "),xr=s(Of,"A",{href:!0,rel:!0});var m0=a(xr);M1=i(m0,"tf.keras.Model"),m0.forEach(t),E1=i(Of,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Of.forEach(t),z1=h(Pt),y(Xn.$$.fragment,Pt),q1=h(Pt),Ft=s(Pt,"DIV",{class:!0});var Js=a(Ft);y(Rr.$$.fragment,Js),C1=h(Js),jo=s(Js,"P",{});var xd=a(jo);j1=i(xd,"The "),Xl=s(xd,"A",{href:!0});var f0=a(Xl);P1=i(f0,"TFRobertaModel"),f0.forEach(t),L1=i(xd," forward method, overrides the "),Uc=s(xd,"CODE",{});var u0=a(Uc);A1=i(u0,"__call__"),u0.forEach(t),O1=i(xd," special method."),xd.forEach(t),N1=h(Js),y(Yn.$$.fragment,Js),I1=h(Js),y(Zn.$$.fragment,Js),Js.forEach(t),Pt.forEach(t),Cm=h(o),Po=s(o,"H2",{class:!0});var Nf=a(Po);es=s(Nf,"A",{id:!0,class:!0,href:!0});var g0=a(es);Hc=s(g0,"SPAN",{});var _0=a(Hc);y(Mr.$$.fragment,_0),_0.forEach(t),g0.forEach(t),D1=h(Nf),Qc=s(Nf,"SPAN",{});var b0=a(Qc);S1=i(b0,"TFRobertaForCausalLM"),b0.forEach(t),Nf.forEach(t),jm=h(o),Lo=s(o,"DIV",{class:!0});var If=a(Lo);y(Er.$$.fragment,If),W1=h(If),xt=s(If,"DIV",{class:!0});var Ks=a(xt);y(zr.$$.fragment,Ks),B1=h(Ks),Ao=s(Ks,"P",{});var Rd=a(Ao);U1=i(Rd,"The "),Yl=s(Rd,"A",{href:!0});var k0=a(Yl);H1=i(k0,"TFRobertaForCausalLM"),k0.forEach(t),Q1=i(Rd," forward method, overrides the "),Vc=s(Rd,"CODE",{});var v0=a(Vc);V1=i(v0,"__call__"),v0.forEach(t),J1=i(Rd," special method."),Rd.forEach(t),K1=h(Ks),y(ts.$$.fragment,Ks),G1=h(Ks),y(os.$$.fragment,Ks),Ks.forEach(t),If.forEach(t),Pm=h(o),Oo=s(o,"H2",{class:!0});var Df=a(Oo);ns=s(Df,"A",{id:!0,class:!0,href:!0});var T0=a(ns);Jc=s(T0,"SPAN",{});var y0=a(Jc);y(qr.$$.fragment,y0),y0.forEach(t),T0.forEach(t),X1=h(Df),Kc=s(Df,"SPAN",{});var w0=a(Kc);Y1=i(w0,"TFRobertaForMaskedLM"),w0.forEach(t),Df.forEach(t),Lm=h(o),Qe=s(o,"DIV",{class:!0});var Lt=a(Qe);y(Cr.$$.fragment,Lt),Z1=h(Lt),jr=s(Lt,"P",{});var Sf=a(jr);ev=i(Sf,"RoBERTa Model with a "),Gc=s(Sf,"CODE",{});var $0=a(Gc);tv=i($0,"language modeling"),$0.forEach(t),ov=i(Sf," head on top."),Sf.forEach(t),nv=h(Lt),Pr=s(Lt,"P",{});var Wf=a(Pr);sv=i(Wf,"This model inherits from "),Zl=s(Wf,"A",{href:!0});var F0=a(Zl);av=i(F0,"TFPreTrainedModel"),F0.forEach(t),rv=i(Wf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Wf.forEach(t),iv=h(Lt),Lr=s(Lt,"P",{});var Bf=a(Lr);lv=i(Bf,"This model is also a "),Ar=s(Bf,"A",{href:!0,rel:!0});var x0=a(Ar);dv=i(x0,"tf.keras.Model"),x0.forEach(t),cv=i(Bf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Bf.forEach(t),pv=h(Lt),y(ss.$$.fragment,Lt),hv=h(Lt),dt=s(Lt,"DIV",{class:!0});var io=a(dt);y(Or.$$.fragment,io),mv=h(io),No=s(io,"P",{});var Md=a(No);fv=i(Md,"The "),ed=s(Md,"A",{href:!0});var R0=a(ed);uv=i(R0,"TFRobertaForMaskedLM"),R0.forEach(t),gv=i(Md," forward method, overrides the "),Xc=s(Md,"CODE",{});var M0=a(Xc);_v=i(M0,"__call__"),M0.forEach(t),bv=i(Md," special method."),Md.forEach(t),kv=h(io),y(as.$$.fragment,io),vv=h(io),y(rs.$$.fragment,io),Tv=h(io),y(is.$$.fragment,io),io.forEach(t),Lt.forEach(t),Am=h(o),Io=s(o,"H2",{class:!0});var Uf=a(Io);ls=s(Uf,"A",{id:!0,class:!0,href:!0});var E0=a(ls);Yc=s(E0,"SPAN",{});var z0=a(Yc);y(Nr.$$.fragment,z0),z0.forEach(t),E0.forEach(t),yv=h(Uf),Zc=s(Uf,"SPAN",{});var q0=a(Zc);wv=i(q0,"TFRobertaForSequenceClassification"),q0.forEach(t),Uf.forEach(t),Om=h(o),Ve=s(o,"DIV",{class:!0});var At=a(Ve);y(Ir.$$.fragment,At),$v=h(At),ep=s(At,"P",{});var C0=a(ep);Fv=i(C0,`RoBERTa Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),C0.forEach(t),xv=h(At),Dr=s(At,"P",{});var Hf=a(Dr);Rv=i(Hf,"This model inherits from "),td=s(Hf,"A",{href:!0});var j0=a(td);Mv=i(j0,"TFPreTrainedModel"),j0.forEach(t),Ev=i(Hf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Hf.forEach(t),zv=h(At),Sr=s(At,"P",{});var Qf=a(Sr);qv=i(Qf,"This model is also a "),Wr=s(Qf,"A",{href:!0,rel:!0});var P0=a(Wr);Cv=i(P0,"tf.keras.Model"),P0.forEach(t),jv=i(Qf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Qf.forEach(t),Pv=h(At),y(ds.$$.fragment,At),Lv=h(At),ct=s(At,"DIV",{class:!0});var lo=a(ct);y(Br.$$.fragment,lo),Av=h(lo),Do=s(lo,"P",{});var Ed=a(Do);Ov=i(Ed,"The "),od=s(Ed,"A",{href:!0});var L0=a(od);Nv=i(L0,"TFRobertaForSequenceClassification"),L0.forEach(t),Iv=i(Ed," forward method, overrides the "),tp=s(Ed,"CODE",{});var A0=a(tp);Dv=i(A0,"__call__"),A0.forEach(t),Sv=i(Ed," special method."),Ed.forEach(t),Wv=h(lo),y(cs.$$.fragment,lo),Bv=h(lo),y(ps.$$.fragment,lo),Uv=h(lo),y(hs.$$.fragment,lo),lo.forEach(t),At.forEach(t),Nm=h(o),So=s(o,"H2",{class:!0});var Vf=a(So);ms=s(Vf,"A",{id:!0,class:!0,href:!0});var O0=a(ms);op=s(O0,"SPAN",{});var N0=a(op);y(Ur.$$.fragment,N0),N0.forEach(t),O0.forEach(t),Hv=h(Vf),np=s(Vf,"SPAN",{});var I0=a(np);Qv=i(I0,"TFRobertaForMultipleChoice"),I0.forEach(t),Vf.forEach(t),Im=h(o),Je=s(o,"DIV",{class:!0});var Ot=a(Je);y(Hr.$$.fragment,Ot),Vv=h(Ot),sp=s(Ot,"P",{});var D0=a(sp);Jv=i(D0,`Roberta Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),D0.forEach(t),Kv=h(Ot),Qr=s(Ot,"P",{});var Jf=a(Qr);Gv=i(Jf,"This model inherits from "),nd=s(Jf,"A",{href:!0});var S0=a(nd);Xv=i(S0,"TFPreTrainedModel"),S0.forEach(t),Yv=i(Jf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Jf.forEach(t),Zv=h(Ot),Vr=s(Ot,"P",{});var Kf=a(Vr);eT=i(Kf,"This model is also a "),Jr=s(Kf,"A",{href:!0,rel:!0});var W0=a(Jr);tT=i(W0,"tf.keras.Model"),W0.forEach(t),oT=i(Kf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Kf.forEach(t),nT=h(Ot),y(fs.$$.fragment,Ot),sT=h(Ot),Rt=s(Ot,"DIV",{class:!0});var Gs=a(Rt);y(Kr.$$.fragment,Gs),aT=h(Gs),Wo=s(Gs,"P",{});var zd=a(Wo);rT=i(zd,"The "),sd=s(zd,"A",{href:!0});var B0=a(sd);iT=i(B0,"TFRobertaForMultipleChoice"),B0.forEach(t),lT=i(zd," forward method, overrides the "),ap=s(zd,"CODE",{});var U0=a(ap);dT=i(U0,"__call__"),U0.forEach(t),cT=i(zd," special method."),zd.forEach(t),pT=h(Gs),y(us.$$.fragment,Gs),hT=h(Gs),y(gs.$$.fragment,Gs),Gs.forEach(t),Ot.forEach(t),Dm=h(o),Bo=s(o,"H2",{class:!0});var Gf=a(Bo);_s=s(Gf,"A",{id:!0,class:!0,href:!0});var H0=a(_s);rp=s(H0,"SPAN",{});var Q0=a(rp);y(Gr.$$.fragment,Q0),Q0.forEach(t),H0.forEach(t),mT=h(Gf),ip=s(Gf,"SPAN",{});var V0=a(ip);fT=i(V0,"TFRobertaForTokenClassification"),V0.forEach(t),Gf.forEach(t),Sm=h(o),Ke=s(o,"DIV",{class:!0});var Nt=a(Ke);y(Xr.$$.fragment,Nt),uT=h(Nt),lp=s(Nt,"P",{});var J0=a(lp);gT=i(J0,`RoBERTa Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
Named-Entity-Recognition (NER) tasks.`),J0.forEach(t),_T=h(Nt),Yr=s(Nt,"P",{});var Xf=a(Yr);bT=i(Xf,"This model inherits from "),ad=s(Xf,"A",{href:!0});var K0=a(ad);kT=i(K0,"TFPreTrainedModel"),K0.forEach(t),vT=i(Xf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Xf.forEach(t),TT=h(Nt),Zr=s(Nt,"P",{});var Yf=a(Zr);yT=i(Yf,"This model is also a "),ei=s(Yf,"A",{href:!0,rel:!0});var G0=a(ei);wT=i(G0,"tf.keras.Model"),G0.forEach(t),$T=i(Yf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Yf.forEach(t),FT=h(Nt),y(bs.$$.fragment,Nt),xT=h(Nt),pt=s(Nt,"DIV",{class:!0});var co=a(pt);y(ti.$$.fragment,co),RT=h(co),Uo=s(co,"P",{});var qd=a(Uo);MT=i(qd,"The "),rd=s(qd,"A",{href:!0});var X0=a(rd);ET=i(X0,"TFRobertaForTokenClassification"),X0.forEach(t),zT=i(qd," forward method, overrides the "),dp=s(qd,"CODE",{});var Y0=a(dp);qT=i(Y0,"__call__"),Y0.forEach(t),CT=i(qd," special method."),qd.forEach(t),jT=h(co),y(ks.$$.fragment,co),PT=h(co),y(vs.$$.fragment,co),LT=h(co),y(Ts.$$.fragment,co),co.forEach(t),Nt.forEach(t),Wm=h(o),Ho=s(o,"H2",{class:!0});var Zf=a(Ho);ys=s(Zf,"A",{id:!0,class:!0,href:!0});var Z0=a(ys);cp=s(Z0,"SPAN",{});var ex=a(cp);y(oi.$$.fragment,ex),ex.forEach(t),Z0.forEach(t),AT=h(Zf),pp=s(Zf,"SPAN",{});var tx=a(pp);OT=i(tx,"TFRobertaForQuestionAnswering"),tx.forEach(t),Zf.forEach(t),Bm=h(o),Ge=s(o,"DIV",{class:!0});var It=a(Ge);y(ni.$$.fragment,It),NT=h(It),Qo=s(It,"P",{});var Cd=a(Qo);IT=i(Cd,`RoBERTa Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
