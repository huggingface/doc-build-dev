...
- Be as easy and fast to use as possible:
  ...
  - On top of those three base classes, the library provides two APIs: [pipeline()](/docs/transformers/pr_15430/en/main_classes/pipelines#transformers.pipeline) for quickly
    using a model (plus its associated tokenizer and configuration) on a given task and
    [Trainer](/docs/transformers/pr_15430/en/main_classes/trainer#transformers.Trainer)/`Keras.fit` to quickly train or fine-tune a given model.
...
- **Model classes** such as [BertModel](/docs/transformers/pr_15430/en/model_doc/bert#transformers.BertModel), which are 30+ PyTorch models ([torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module)) or Keras models ([tf.keras.Model](https://www.tensorflow.org/api_docs/python/tf/keras/Model)) that work with the pretrained weights provided in the
  library.
- **Configuration classes** such as [BertConfig](/docs/transformers/pr_15430/en/model_doc/bert#transformers.BertConfig), which store all the parameters required to build
...
- **Tokenizer classes** such as [BertTokenizer](/docs/transformers/pr_15430/en/model_doc/bert#transformers.BertTokenizer), which store the vocabulary for each model and
