...
- SEWForCTC is fine-tuned using connectionist temporal classification (CTC) so the model output has to be decoded using
  [Wav2Vec2CTCTokenizer](/docs/transformers/pr_15731/en/model_doc/wav2vec2#transformers.Wav2Vec2CTCTokenizer).
...
- **final_dropout** (`float`, *optional*, defaults to 0.1) --
  The dropout probability for the final projection layer of [SEWForCTC](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWForCTC).
...
- **ctc_loss_reduction** (`str`, *optional*, defaults to `"sum"`) --
  ...
  instance of [SEWForCTC](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWForCTC).
- **ctc_zero_infinity** (`bool`, *optional*, defaults to `False`) --
  Whether to zero infinite losses and the associated gradients of `torch.nn.CTCLoss`. Infinite losses mainly
  occur when the inputs are too short to be aligned to the targets. Only relevant when training an instance
  of [SEWForCTC](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWForCTC).
...
This is the configuration class to store the configuration of a [SEWModel](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWModel). It is used to instantiate a SEW model
...
elements depending on the configuration ([SEWConfig](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWConfig)) and inputs.
...
The [SEWModel](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWModel) forward method, overrides the `__call__` special method.
...
- **attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
  heads.</retdesc></docstring>
The [SEWForCTC](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWForCTC) forward method, overrides the `__call__` special method.
...
- **attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights after the attention softmax, used to compute the weighted average in the self-attention
  heads.</retdesc></docstring>
The [SEWForSequenceClassification](/docs/transformers/pr_15731/en/model_doc/sew#transformers.SEWForSequenceClassification) forward method, overrides the `__call__` special method.
