...
<p>Indices can be obtained using <a href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17044/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
...
<p>A <a
  href="/docs/transformers/pr_17044/en/main_classes/output#transformers.modeling_outputs.BaseModelOutputWithCrossAttentions"
>transformers.modeling_outputs.BaseModelOutputWithCrossAttentions</a> or a tuple of
<code>torch.FloatTensor</code> (if <code>return_dict=False</code> is passed or when <code>config.return_dict=False</code>) comprising various
elements depending on the configuration (<a
  href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraConfig"
...
</ul>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1064",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.models.electra.modeling_electra.ElectraForPreTrainingOutput"
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1549",returnDescription:`
<p>A <a
...
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1178",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L967",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1453",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1265",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_electra.py#L1347",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L931",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17044/en/main_classes/output#transformers.modeling_tf_outputs.TFBaseModelOutputWithPastAndCrossAttentions"
...
Whether or not to use the model in training mode (some modules like dropout modules have different
behaviors between training and evaluation).`,name:"training"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1029",returnDescription:`
<p>A <a
  href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.models.electra.modeling_tf_electra.TFElectraForPreTrainingOutput"
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1159",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1271",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1365",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1482",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_tf_electra.py#L1564",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17044/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_flax_electra.py#L674"}}),zi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraModel.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraModel.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17044/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17044/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Whether or not to return a <a href="/docs/transformers/pr_17044/en/main_classes/output#transformers.utils.ModelOutput">ModelOutput</a> instead of a plain tuple.`,name:"return_dict"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_flax_electra.py#L576",returnDescription:`
<p>A <a
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17044/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_flax_electra.py#L823"}}),Si=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForPreTraining.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForPreTraining.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17044/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17044/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17044/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_flax_electra.py#L760"}}),Gi=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForMaskedLM.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForMaskedLM.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17044/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17044/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Initializing with a config file does not load the weights associated with the model, only the
configuration. Check out the <a href="/docs/transformers/pr_17044/en/main_classes/model#transformers.PreTrainedModel.from_pretrained">from_pretrained()</a> method to load the model weights.`,name:"config"}],source:"https://github.com/huggingface/transformers/blob/vr_17044/src/transformers/models/electra/modeling_flax_electra.py#L1133"}}),Ll=new I({props:{name:"__call__",anchor:"transformers.FlaxElectraForQuestionAnswering.__call__",parameters:[{name:"input_ids",val:""},{name:"attention_mask",val:" = None"},{name:"token_type_ids",val:" = None"},{name:"position_ids",val:" = None"},{name:"head_mask",val:" = None"},{name:"params",val:": dict = None"},{name:"dropout_rng",val:": PRNGKey = None"},{name:"train",val:": bool = False"},{name:"output_attentions",val:": typing.Optional[bool] = None"},{name:"output_hidden_states",val:": typing.Optional[bool] = None"},{name:"return_dict",val:": typing.Optional[bool] = None"}],parametersDescription:[{anchor:"transformers.FlaxElectraForQuestionAnswering.__call__.input_ids",description:`<strong>input_ids</strong> (<code>numpy.ndarray</code> of shape <code>(batch_size, sequence_length)</code>) &#x2014;
Indices of input sequence tokens in the vocabulary.</p>
<p>Indices can be obtained using <a href="/docs/transformers/pr_17044/en/model_doc/electra#transformers.ElectraTokenizer">ElectraTokenizer</a>. See <a href="/docs/transformers/pr_17044/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode">PreTrainedTokenizer.encode()</a> and
<a href="/docs/transformers/pr_17044/en/model_doc/wav2vec2_phoneme#transformers.Wav2Vec2PhonemeCTCTokenizer.__call__">PreTrainedTokenizer.<strong>call</strong>()</a> for details.</p>
...
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),mf.forEach(t),zv=p(io),Ft=s(io,"DIV",{class:!0});var sa=a(Ft);y(vr.$$.fragment,sa),Cv=p(sa),Po=s(sa,"P",{});var Kd=a(Po);jv=i(Kd,"The "),_d=s(Kd,"A",{href:!0});var V0=a(_d);qv=i(V0,"ElectraForMultipleChoice"),V0.forEach(t),Pv=i(Kd," forward method, overrides the "),Jc=s(Kd,"CODE",{});var K0=a(Jc);Av=i(K0,"__call__"),K0.forEach(t),Ov=i(Kd," special method."),Kd.forEach(t),Lv=p(sa),y(Hn.$$.fragment,sa),Nv=p(sa),y(Qn.$$.fragment,sa),sa.forEach(t),io.forEach(t),pu=p(o),Ao=s(o,"H2",{class:!0});var uf=a(Ao);Bn=s(uf,"A",{id:!0,class:!0,href:!0});var J0=a(Bn);Gc=s(J0,"SPAN",{});var G0=a(Gc);y(Tr.$$.fragment,G0),G0.forEach(t),J0.forEach(t),Iv=p(uf),Xc=s(uf,"SPAN",{});var X0=a(Xc);Sv=i(X0,"ElectraForTokenClassification"),X0.forEach(t),uf.forEach(t),hu=p(o),Be=s(o,"DIV",{class:!0});var St=a(Be);y(br.$$.fragment,St),Dv=p(St),Yc=s(St,"P",{});var Y0=a(Yc);Wv=i(Y0,"Electra model with a token classification head on top."),Y0.forEach(t),Uv=p(St),Zc=s(St,"P",{});var Z0=a(Zc);Rv=i(Z0,"Both the discriminator and generator may be loaded into this model."),Z0.forEach(t),Hv=p(St),yr=s(St,"P",{});var ff=a(yr);Qv=i(ff,"This model inherits from "),kd=s(ff,"A",{href:!0});var e4=a(kd);Bv=i(e4,"PreTrainedModel"),e4.forEach(t),Vv=i(ff,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),ff.forEach(t),Kv=p(St),Er=s(St,"P",{});var gf=a(Er);Jv=i(gf,"This model is also a PyTorch "),wr=s(gf,"A",{href:!0,rel:!0});var t4=a(wr);Gv=i(t4,"torch.nn.Module"),t4.forEach(t),Xv=i(gf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),gf.forEach(t),Yv=p(St),at=s(St,"DIV",{class:!0});var lo=a(at);y($r.$$.fragment,lo),Zv=p(lo),Oo=s(lo,"P",{});var Jd=a(Oo);eT=i(Jd,"The "),vd=s(Jd,"A",{href:!0});var o4=a(vd);tT=i(o4,"ElectraForTokenClassification"),o4.forEach(t),oT=i(Jd," forward method, overrides the "),ep=s(Jd,"CODE",{});var n4=a(ep);nT=i(n4,"__call__"),n4.forEach(t),sT=i(Jd," special method."),Jd.forEach(t),aT=p(lo),y(Vn.$$.fragment,lo),rT=p(lo),y(Kn.$$.fragment,lo),iT=p(lo),y(Jn.$$.fragment,lo),lo.forEach(t),St.forEach(t),mu=p(o),Lo=s(o,"H2",{class:!0});var _f=a(Lo);Gn=s(_f,"A",{id:!0,class:!0,href:!0});var s4=a(Gn);tp=s(s4,"SPAN",{});var a4=a(tp);y(Fr.$$.fragment,a4),a4.forEach(t),s4.forEach(t),lT=p(_f),op=s(_f,"SPAN",{});var r4=a(op);dT=i(r4,"ElectraForQuestionAnswering"),r4.forEach(t),_f.forEach(t),uu=p(o),nt=s(o,"DIV",{class:!0});var co=a(nt);y(xr.$$.fragment,co),cT=p(co),No=s(co,"P",{});var Gd=a(No);pT=i(Gd,`ELECTRA Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),np=s(Gd,"CODE",{});var i4=a(np);hT=i(i4,"span start logits"),i4.forEach(t),mT=i(Gd," and "),sp=s(Gd,"CODE",{});var l4=a(sp);uT=i(l4,"span end logits"),l4.forEach(t),fT=i(Gd,")."),Gd.forEach(t),gT=p(co),Mr=s(co,"P",{});var kf=a(Mr);_T=i(kf,"This model inherits from "),Td=s(kf,"A",{href:!0});var d4=a(Td);kT=i(d4,"PreTrainedModel"),d4.forEach(t),vT=i(kf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),kf.forEach(t),TT=p(co),zr=s(co,"P",{});var vf=a(zr);bT=i(vf,"This model is also a PyTorch "),Cr=s(vf,"A",{href:!0,rel:!0});var c4=a(Cr);yT=i(c4,"torch.nn.Module"),c4.forEach(t),ET=i(vf,` subclass.
Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage
and behavior.`),vf.forEach(t),wT=p(co),rt=s(co,"DIV",{class:!0});var po=a(rt);y(jr.$$.fragment,po),$T=p(po),Io=s(po,"P",{});var Xd=a(Io);FT=i(Xd,"The "),bd=s(Xd,"A",{href:!0});var p4=a(bd);xT=i(p4,"ElectraForQuestionAnswering"),p4.forEach(t),MT=i(Xd," forward method, overrides the "),ap=s(Xd,"CODE",{});var h4=a(ap);zT=i(h4,"__call__"),h4.forEach(t),CT=i(Xd," special method."),Xd.forEach(t),jT=p(po),y(Xn.$$.fragment,po),qT=p(po),y(Yn.$$.fragment,po),PT=p(po),y(Zn.$$.fragment,po),po.forEach(t),co.forEach(t),fu=p(o),So=s(o,"H2",{class:!0});var Tf=a(So);es=s(Tf,"A",{id:!0,class:!0,href:!0});var m4=a(es);rp=s(m4,"SPAN",{});var u4=a(rp);y(qr.$$.fragment,u4),u4.forEach(t),m4.forEach(t),AT=p(Tf),ip=s(Tf,"SPAN",{});var f4=a(ip);OT=i(f4,"TFElectraModel"),f4.forEach(t),Tf.forEach(t),gu=p(o),Ve=s(o,"DIV",{class:!0});var Dt=a(Ve);y(Pr.$$.fragment,Dt),LT=p(Dt),lp=s(Dt,"P",{});var g4=a(lp);NT=i(g4,"The bare Electra Model transformer outputting raw hidden-states without any specific head on top. Identical to the BERT model except that it uses an additional linear layer between the embedding layer and the encoder if the hidden size and embedding size are different. Both the generator and discriminator checkpoints may be loaded into this model."),g4.forEach(t),IT=p(Dt),Ar=s(Dt,"P",{});var bf=a(Ar);ST=i(bf,"This model inherits from "),yd=s(bf,"A",{href:!0});var _4=a(yd);DT=i(_4,"TFPreTrainedModel"),_4.forEach(t),WT=i(bf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),bf.forEach(t),UT=p(Dt),Or=s(Dt,"P",{});var yf=a(Or);RT=i(yf,"This model is also a "),Lr=s(yf,"A",{href:!0,rel:!0});var k4=a(Lr);HT=i(k4,"tf.keras.Model"),k4.forEach(t),QT=i(yf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),yf.forEach(t),BT=p(Dt),y(ts.$$.fragment,Dt),VT=p(Dt),xt=s(Dt,"DIV",{class:!0});var aa=a(xt);y(Nr.$$.fragment,aa),KT=p(aa),Do=s(aa,"P",{});var Yd=a(Do);JT=i(Yd,"The "),Ed=s(Yd,"A",{href:!0});var v4=a(Ed);GT=i(v4,"TFElectraModel"),v4.forEach(t),XT=i(Yd," forward method, overrides the "),dp=s(Yd,"CODE",{});var T4=a(dp);YT=i(T4,"__call__"),T4.forEach(t),ZT=i(Yd," special method."),Yd.forEach(t),eb=p(aa),y(os.$$.fragment,aa),tb=p(aa),y(ns.$$.fragment,aa),aa.forEach(t),Dt.forEach(t),_u=p(o),Wo=s(o,"H2",{class:!0});var Ef=a(Wo);ss=s(Ef,"A",{id:!0,class:!0,href:!0});var b4=a(ss);cp=s(b4,"SPAN",{});var y4=a(cp);y(Ir.$$.fragment,y4),y4.forEach(t),b4.forEach(t),ob=p(Ef),pp=s(Ef,"SPAN",{});var E4=a(pp);nb=i(E4,"TFElectraForPreTraining"),E4.forEach(t),Ef.forEach(t),ku=p(o),Oe=s(o,"DIV",{class:!0});var ht=a(Oe);y(Sr.$$.fragment,ht),sb=p(ht),hp=s(ht,"P",{});var w4=a(hp);ab=i(w4,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),w4.forEach(t),rb=p(ht),mp=s(ht,"P",{});var $4=a(mp);ib=i($4,`Even though both the discriminator and generator may be loaded into this model, the discriminator is the only model
of the two to have the correct classification head to be used for this model.`),$4.forEach(t),lb=p(ht),Dr=s(ht,"P",{});var wf=a(Dr);db=i(wf,"This model inherits from "),wd=s(wf,"A",{href:!0});var F4=a(wd);cb=i(F4,"TFPreTrainedModel"),F4.forEach(t),pb=i(wf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),wf.forEach(t),hb=p(ht),Wr=s(ht,"P",{});var $f=a(Wr);mb=i($f,"This model is also a "),Ur=s($f,"A",{href:!0,rel:!0});var x4=a(Ur);ub=i(x4,"tf.keras.Model"),x4.forEach(t),fb=i($f,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),$f.forEach(t),gb=p(ht),y(as.$$.fragment,ht),_b=p(ht),Mt=s(ht,"DIV",{class:!0});var ra=a(Mt);y(Rr.$$.fragment,ra),kb=p(ra),Uo=s(ra,"P",{});var Zd=a(Uo);vb=i(Zd,"The "),$d=s(Zd,"A",{href:!0});var M4=a($d);Tb=i(M4,"TFElectraForPreTraining"),M4.forEach(t),bb=i(Zd," forward method, overrides the "),up=s(Zd,"CODE",{});var z4=a(up);yb=i(z4,"__call__"),z4.forEach(t),Eb=i(Zd," special method."),Zd.forEach(t),wb=p(ra),y(rs.$$.fragment,ra),$b=p(ra),y(is.$$.fragment,ra),ra.forEach(t),ht.forEach(t),vu=p(o),Ro=s(o,"H2",{class:!0});var Ff=a(Ro);ls=s(Ff,"A",{id:!0,class:!0,href:!0});var C4=a(ls);fp=s(C4,"SPAN",{});var j4=a(fp);y(Hr.$$.fragment,j4),j4.forEach(t),C4.forEach(t),Fb=p(Ff),gp=s(Ff,"SPAN",{});var q4=a(gp);xb=i(q4,"TFElectraForMaskedLM"),q4.forEach(t),Ff.forEach(t),Tu=p(o),Le=s(o,"DIV",{class:!0});var mt=a(Le);y(Qr.$$.fragment,mt),Mb=p(mt),_p=s(mt,"P",{});var P4=a(_p);zb=i(P4,"Electra model with a language modeling head on top."),P4.forEach(t),Cb=p(mt),kp=s(mt,"P",{});var A4=a(kp);jb=i(A4,`Even though both the discriminator and generator may be loaded into this model, the generator is the only model of
the two to have been trained for the masked language modeling task.`),A4.forEach(t),qb=p(mt),Br=s(mt,"P",{});var xf=a(Br);Pb=i(xf,"This model inherits from "),Fd=s(xf,"A",{href:!0});var O4=a(Fd);Ab=i(O4,"TFPreTrainedModel"),O4.forEach(t),Ob=i(xf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),xf.forEach(t),Lb=p(mt),Vr=s(mt,"P",{});var Mf=a(Vr);Nb=i(Mf,"This model is also a "),Kr=s(Mf,"A",{href:!0,rel:!0});var L4=a(Kr);Ib=i(L4,"tf.keras.Model"),L4.forEach(t),Sb=i(Mf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Mf.forEach(t),Db=p(mt),y(ds.$$.fragment,mt),Wb=p(mt),it=s(mt,"DIV",{class:!0});var ho=a(it);y(Jr.$$.fragment,ho),Ub=p(ho),Ho=s(ho,"P",{});var ec=a(Ho);Rb=i(ec,"The "),xd=s(ec,"A",{href:!0});var N4=a(xd);Hb=i(N4,"TFElectraForMaskedLM"),N4.forEach(t),Qb=i(ec," forward method, overrides the "),vp=s(ec,"CODE",{});var I4=a(vp);Bb=i(I4,"__call__"),I4.forEach(t),Vb=i(ec," special method."),ec.forEach(t),Kb=p(ho),y(cs.$$.fragment,ho),Jb=p(ho),y(ps.$$.fragment,ho),Gb=p(ho),y(hs.$$.fragment,ho),ho.forEach(t),mt.forEach(t),bu=p(o),Qo=s(o,"H2",{class:!0});var zf=a(Qo);ms=s(zf,"A",{id:!0,class:!0,href:!0});var S4=a(ms);Tp=s(S4,"SPAN",{});var D4=a(Tp);y(Gr.$$.fragment,D4),D4.forEach(t),S4.forEach(t),Xb=p(zf),bp=s(zf,"SPAN",{});var W4=a(bp);Yb=i(W4,"TFElectraForSequenceClassification"),W4.forEach(t),zf.forEach(t),yu=p(o),Ke=s(o,"DIV",{class:!0});var Wt=a(Ke);y(Xr.$$.fragment,Wt),Zb=p(Wt),yp=s(Wt,"P",{});var U4=a(yp);ey=i(U4,`ELECTRA Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),U4.forEach(t),ty=p(Wt),Yr=s(Wt,"P",{});var Cf=a(Yr);oy=i(Cf,"This model inherits from "),Md=s(Cf,"A",{href:!0});var R4=a(Md);ny=i(R4,"TFPreTrainedModel"),R4.forEach(t),sy=i(Cf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Cf.forEach(t),ay=p(Wt),Zr=s(Wt,"P",{});var jf=a(Zr);ry=i(jf,"This model is also a "),ei=s(jf,"A",{href:!0,rel:!0});var H4=a(ei);iy=i(H4,"tf.keras.Model"),H4.forEach(t),ly=i(jf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),jf.forEach(t),dy=p(Wt),y(us.$$.fragment,Wt),cy=p(Wt),lt=s(Wt,"DIV",{class:!0});var mo=a(lt);y(ti.$$.fragment,mo),py=p(mo),Bo=s(mo,"P",{});var tc=a(Bo);hy=i(tc,"The "),zd=s(tc,"A",{href:!0});var Q4=a(zd);my=i(Q4,"TFElectraForSequenceClassification"),Q4.forEach(t),uy=i(tc," forward method, overrides the "),Ep=s(tc,"CODE",{});var B4=a(Ep);fy=i(B4,"__call__"),B4.forEach(t),gy=i(tc," special method."),tc.forEach(t),_y=p(mo),y(fs.$$.fragment,mo),ky=p(mo),y(gs.$$.fragment,mo),vy=p(mo),y(_s.$$.fragment,mo),mo.forEach(t),Wt.forEach(t),Eu=p(o),Vo=s(o,"H2",{class:!0});var qf=a(Vo);ks=s(qf,"A",{id:!0,class:!0,href:!0});var V4=a(ks);wp=s(V4,"SPAN",{});var K4=a(wp);y(oi.$$.fragment,K4),K4.forEach(t),V4.forEach(t),Ty=p(qf),$p=s(qf,"SPAN",{});var J4=a($p);by=i(J4,"TFElectraForMultipleChoice"),J4.forEach(t),qf.forEach(t),wu=p(o),Je=s(o,"DIV",{class:!0});var Ut=a(Je);y(ni.$$.fragment,Ut),yy=p(Ut),Fp=s(Ut,"P",{});var G4=a(Fp);Ey=i(G4,`ELECTRA Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),G4.forEach(t),wy=p(Ut),si=s(Ut,"P",{});var Pf=a(si);$y=i(Pf,"This model inherits from "),Cd=s(Pf,"A",{href:!0});var X4=a(Cd);Fy=i(X4,"TFPreTrainedModel"),X4.forEach(t),xy=i(Pf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Pf.forEach(t),My=p(Ut),ai=s(Ut,"P",{});var Af=a(ai);zy=i(Af,"This model is also a "),ri=s(Af,"A",{href:!0,rel:!0});var Y4=a(ri);Cy=i(Y4,"tf.keras.Model"),Y4.forEach(t),jy=i(Af,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Af.forEach(t),qy=p(Ut),y(vs.$$.fragment,Ut),Py=p(Ut),zt=s(Ut,"DIV",{class:!0});var ia=a(zt);y(ii.$$.fragment,ia),Ay=p(ia),Ko=s(ia,"P",{});var oc=a(Ko);Oy=i(oc,"The "),jd=s(oc,"A",{href:!0});var Z4=a(jd);Ly=i(Z4,"TFElectraForMultipleChoice"),Z4.forEach(t),Ny=i(oc," forward method, overrides the "),xp=s(oc,"CODE",{});var eF=a(xp);Iy=i(eF,"__call__"),eF.forEach(t),Sy=i(oc," special method."),oc.forEach(t),Dy=p(ia),y(Ts.$$.fragment,ia),Wy=p(ia),y(bs.$$.fragment,ia),ia.forEach(t),Ut.forEach(t),$u=p(o),Jo=s(o,"H2",{class:!0});var Of=a(Jo);ys=s(Of,"A",{id:!0,class:!0,href:!0});var tF=a(ys);Mp=s(tF,"SPAN",{});var oF=a(Mp);y(li.$$.fragment,oF),oF.forEach(t),tF.forEach(t),Uy=p(Of),zp=s(Of,"SPAN",{});var nF=a(zp);Ry=i(nF,"TFElectraForTokenClassification"),nF.forEach(t),Of.forEach(t),Fu=p(o),Ne=s(o,"DIV",{class:!0});var ut=a(Ne);y(di.$$.fragment,ut),Hy=p(ut),Cp=s(ut,"P",{});var sF=a(Cp);Qy=i(sF,"Electra model with a token classification head on top."),sF.forEach(t),By=p(ut),jp=s(ut,"P",{});var aF=a(jp);Vy=i(aF,"Both the discriminator and generator may be loaded into this model."),aF.forEach(t),Ky=p(ut),ci=s(ut,"P",{});var Lf=a(ci);Jy=i(Lf,"This model inherits from "),qd=s(Lf,"A",{href:!0});var rF=a(qd);Gy=i(rF,"TFPreTrainedModel"),rF.forEach(t),Xy=i(Lf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Lf.forEach(t),Yy=p(ut),pi=s(ut,"P",{});var Nf=a(pi);Zy=i(Nf,"This model is also a "),hi=s(Nf,"A",{href:!0,rel:!0});var iF=a(hi);e1=i(iF,"tf.keras.Model"),iF.forEach(t),t1=i(Nf,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Nf.forEach(t),o1=p(ut),y(Es.$$.fragment,ut),n1=p(ut),dt=s(ut,"DIV",{class:!0});var uo=a(dt);y(mi.$$.fragment,uo),s1=p(uo),Go=s(uo,"P",{});var nc=a(Go);a1=i(nc,"The "),Pd=s(nc,"A",{href:!0});var lF=a(Pd);r1=i(lF,"TFElectraForTokenClassification"),lF.forEach(t),i1=i(nc," forward method, overrides the "),qp=s(nc,"CODE",{});var dF=a(qp);l1=i(dF,"__call__"),dF.forEach(t),d1=i(nc," special method."),nc.forEach(t),c1=p(uo),y(ws.$$.fragment,uo),p1=p(uo),y($s.$$.fragment,uo),h1=p(uo),y(Fs.$$.fragment,uo),uo.forEach(t),ut.forEach(t),xu=p(o),Xo=s(o,"H2",{class:!0});var If=a(Xo);xs=s(If,"A",{id:!0,class:!0,href:!0});var cF=a(xs);Pp=s(cF,"SPAN",{});var pF=a(Pp);y(ui.$$.fragment,pF),pF.forEach(t),cF.forEach(t),m1=p(If),Ap=s(If,"SPAN",{});var hF=a(Ap);u1=i(hF,"TFElectraForQuestionAnswering"),hF.forEach(t),If.forEach(t),Mu=p(o),Ge=s(o,"DIV",{class:!0});var Rt=a(Ge);y(fi.$$.fragment,Rt),f1=p(Rt),Yo=s(Rt,"P",{});var sc=a(Yo);g1=i(sc,`Electra Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
layers on top of the hidden-states output to compute `),Op=s(sc,"CODE",{});var mF=a(Op);_1=i(mF,"span start logits"),mF.forEach(t),k1=i(sc," and "),Lp=s(sc,"CODE",{});var uF=a(Lp);v1=i(uF,"span end logits"),uF.forEach(t),T1=i(sc,")."),sc.forEach(t),b1=p(Rt),gi=s(Rt,"P",{});var Sf=a(gi);y1=i(Sf,"This model inherits from "),Ad=s(Sf,"A",{href:!0});var fF=a(Ad);E1=i(fF,"TFPreTrainedModel"),fF.forEach(t),w1=i(Sf,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
etc.)`),Sf.forEach(t),$1=p(Rt),_i=s(Rt,"P",{});var Df=a(_i);F1=i(Df,"This model is also a "),ki=s(Df,"A",{href:!0,rel:!0});var gF=a(ki);x1=i(gF,"tf.keras.Model"),gF.forEach(t),M1=i(Df,` subclass. Use it
as a regular TF 2.0 Keras Model and refer to the TF 2.0 documentation for all matter related to general usage and
behavior.`),Df.forEach(t),z1=p(Rt),y(Ms.$$.fragment,Rt),C1=p(Rt),ct=s(Rt,"DIV",{class:!0});var fo=a(ct);y(vi.$$.fragment,fo),j1=p(fo),Zo=s(fo,"P",{});var ac=a(Zo);q1=i(ac,"The "),Od=s(ac,"A",{href:!0});var _F=a(Od);P1=i(_F,"TFElectraForQuestionAnswering"),_F.forEach(t),A1=i(ac," forward method, overrides the "),Np=s(ac,"CODE",{});var kF=a(Np);O1=i(kF,"__call__"),kF.forEach(t),L1=i(ac," special method."),ac.forEach(t),N1=p(fo),y(zs.$$.fragment,fo),I1=p(fo),y(Cs.$$.fragment,fo),S1=p(fo),y(js.$$.fragment,fo),fo.forEach(t),Rt.forEach(t),zu=p(o),en=s(o,"H2",{class:!0});var Wf=a(en);qs=s(Wf,"A",{id:!0,class:!0,href:!0});var vF=a(qs);Ip=s(vF,"SPAN",{});var TF=a(Ip);y(Ti.$$.fragment,TF),TF.forEach(t),vF.forEach(t),D1=p(Wf),Sp=s(Wf,"SPAN",{});var bF=a(Sp);W1=i(bF,"FlaxElectraModel"),bF.forEach(t),Wf.forEach(t),Cu=p(o),Ie=s(o,"DIV",{class:!0});var ft=a(Ie);y(bi.$$.fragment,ft),U1=p(ft),Dp=s(ft,"P",{});var yF=a(Dp);R1=i(yF,"The bare Electra Model transformer outputting raw hidden-states without any specific head on top."),yF.forEach(t),H1=p(ft),yi=s(ft,"P",{});var Uf=a(yi);Q1=i(Uf,"This model inherits from "),Ld=s(Uf,"A",{href:!0});var EF=a(Ld);B1=i(EF,"FlaxPreTrainedModel"),EF.forEach(t),V1=i(Uf,`. Check the superclass documentation for the generic methods the
...
`),wi=s(Rf,"A",{href:!0,rel:!0});var wF=a(wi);G1=i(wF,"flax.nn.Module"),wF.forEach(t),X1=i(Rf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Rf.forEach(t),Y1=p(ft),Wp=s(ft,"P",{});var $F=a(Wp);Z1=i($F,"Finally, this model supports inherent JAX features such as:"),$F.forEach(t),eE=p(ft),Qt=s(ft,"UL",{});var la=a(Qt);Up=s(la,"LI",{});var FF=a(Up);$i=s(FF,"A",{href:!0,rel:!0});var xF=a($i);tE=i(xF,"Just-In-Time (JIT) compilation"),xF.forEach(t),FF.forEach(t),oE=p(la),Rp=s(la,"LI",{});var MF=a(Rp);Fi=s(MF,"A",{href:!0,rel:!0});var zF=a(Fi);nE=i(zF,"Automatic Differentiation"),zF.forEach(t),MF.forEach(t),sE=p(la),Hp=s(la,"LI",{});var CF=a(Hp);xi=s(CF,"A",{href:!0,rel:!0});var jF=a(xi);aE=i(jF,"Vectorization"),jF.forEach(t),CF.forEach(t),rE=p(la),Qp=s(la,"LI",{});var qF=a(Qp);Mi=s(qF,"A",{href:!0,rel:!0});var PF=a(Mi);iE=i(PF,"Parallelization"),PF.forEach(t),qF.forEach(t),la.forEach(t),lE=p(ft),Ct=s(ft,"DIV",{class:!0});var da=a(Ct);y(zi.$$.fragment,da),dE=p(da),tn=s(da,"P",{});var rc=a(tn);cE=i(rc,"The "),Bp=s(rc,"CODE",{});var AF=a(Bp);pE=i(AF,"FlaxElectraPreTrainedModel"),AF.forEach(t),hE=i(rc," forward method, overrides the "),Vp=s(rc,"CODE",{});var OF=a(Vp);mE=i(OF,"__call__"),OF.forEach(t),uE=i(rc," special method."),rc.forEach(t),fE=p(da),y(Ps.$$.fragment,da),gE=p(da),y(As.$$.fragment,da),da.forEach(t),ft.forEach(t),ju=p(o),on=s(o,"H2",{class:!0});var Hf=a(on);Os=s(Hf,"A",{id:!0,class:!0,href:!0});var LF=a(Os);Kp=s(LF,"SPAN",{});var NF=a(Kp);y(Ci.$$.fragment,NF),NF.forEach(t),LF.forEach(t),_E=p(Hf),Jp=s(Hf,"SPAN",{});var IF=a(Jp);kE=i(IF,"FlaxElectraForPreTraining"),IF.forEach(t),Hf.forEach(t),qu=p(o),Pe=s(o,"DIV",{class:!0});var Xe=a(Pe);y(ji.$$.fragment,Xe),vE=p(Xe),Gp=s(Xe,"P",{});var SF=a(Gp);TE=i(SF,"Electra model with a binary classification head on top as used during pretraining for identifying generated tokens."),SF.forEach(t),bE=p(Xe),Xp=s(Xe,"P",{});var DF=a(Xp);yE=i(DF,"It is recommended to load the discriminator checkpoint into that model."),DF.forEach(t),EE=p(Xe),qi=s(Xe,"P",{});var Qf=a(qi);wE=i(Qf,"This model inherits from "),Nd=s(Qf,"A",{href:!0});var WF=a(Nd);$E=i(WF,"FlaxPreTrainedModel"),WF.forEach(t),FE=i(Qf,`. Check the superclass documentation for the generic methods the
...
`),Ai=s(Bf,"A",{href:!0,rel:!0});var UF=a(Ai);zE=i(UF,"flax.nn.Module"),UF.forEach(t),CE=i(Bf,` subclass. Use it as a
regular Flax Module and refer to the Flax documentation for all matter related to general usage and behavior.`),Bf.forEach(t),jE=p(Xe),Yp=s(Xe,"P",{});var RF=a(Yp);qE=i(RF,"Finally, this model supports inherent JAX features such as:"),RF.forEach(t),PE=p(Xe),Bt=s(Xe,"UL",{});var ca=a(Bt);Zp=s(ca,"LI",{});var HF=a(Zp);Oi=s(HF,"A",{href:!0,rel:!0});var QF=a(Oi);AE=i(QF,"Just-In-Time (JIT) compilation"),QF.forEach(t),HF.forEach(t),OE=p(ca),eh=s(ca,"LI",{});var BF=a(eh);Li=s(BF,"A",{href:!0,rel:!0});var VF=a(Li);LE=i(VF,"Automatic Differentiation"),VF.forEach(t),BF.forEach(t),NE=p(ca),th=s(ca,"LI",{});var KF=a(th);Ni=s(KF,"A",{href:!0,rel:!0});var JF=a(Ni);IE=i(JF,"Vectorization"),JF.forEach(t),KF.forEach(t),SE=p(ca),oh=s(ca,"LI",{});var GF=a(oh);Ii=s(GF,"A",{href:!0,rel:!0});var XF=a(Ii);DE=i(XF,"Parallelization"),XF.forEach(t),GF.forEach(t),ca.forEach(t),WE=p(Xe),jt=s(Xe,"DIV",{class:!0});var pa=a(jt);y(Si.$$.fragment,pa),UE=p(pa),nn=s(pa,"P",{});var ic=a(nn);RE=i(ic,"The "),nh=s(ic,"CODE",{});var YF=a(nh);HE=i(YF,"FlaxElectraPreTrainedModel"),YF.forEach(t),QE=i(ic," forward method, overrides the "),sh=s(ic,"CODE",{});var ZF=a(sh);BE=i(ZF,"__call__"),ZF.forEach(t),VE=i(ic," special method."),ic.forEach(t),KE=p(pa),y(Ls.$$.fragment,pa),JE=p(pa),y(Ns.$$.fragment,pa),pa.forEach(t),Xe.forEach(t),Pu=p(o),sn=s(o,"H2",{class:!0});var Vf=a(sn);Is=s(Vf,"A",{id:!0,class:!0,href:!0});var ex=a(Is);ah=s(ex,"SPAN",{});var tx=a(ah);y(Di.$$.fragment,tx),tx.forEach(t),ex.forEach(t),GE=p(Vf),rh=s(Vf,"SPAN",{});var ox=a(rh);XE=i(ox,"FlaxElectraForMaskedLM"),ox.forEach(t),Vf.forEach(t),Au=p(o),Se=s(o,"DIV",{class:!0});var gt=a(Se);y(Wi.$$.fragment,gt),YE=p(gt),Ui=s(gt,"P",{});var Kf=a(Ui);ZE=i(Kf,"Electra Model with a "),ih=s(Kf,"CODE",{});var nx=a(ih);ew=i(nx,"language modeling"),nx.forEach(t),tw=i(Kf," head on top."),Kf.forEach(t),ow=p(gt),Ri=s(gt,"P",{});var Jf=a(Ri);nw=i(Jf,"This model inherits from "),Id=s(Jf,"A",{href:!0});var sx=a(Id);sw=i(sx,"FlaxPreTrainedModel"),sx.forEach(t),aw=i(Jf,`. Check the superclass documentation for the generic methods the
