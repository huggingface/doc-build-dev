...
Optional second list of IDs for sequence pairs.`,name:"token_ids_1"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/tokenization_roberta.py#L340",returnDescription:`
...
Whether or not the token list is already formatted with special tokens for the model.`,name:"already_has_special_tokens"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/tokenization_roberta.py#L365",returnDescription:`
...
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L741",returnDescription:`
<p>A <a
...
elements depending on the configuration (<a
  href="/docs/transformers/pr_16925/en/model_doc/roberta#transformers.RobertaConfig"
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>).`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L908",returnDescription:`
<p>A <a
...
Used to hide legacy arguments that have been deprecated.`,name:"kwargs"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L1063",returnDescription:`
<p>A <a
...
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L1179",returnDescription:`
<p>A <a
...
<code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L1278",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L1376",returnDescription:`
<p>A <a
...
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_roberta.py#L1481",returnDescription:`
<p>A <a
...
If set to <code>True</code>, <code>past_key_values</code> key value states are returned and can be used to speed up decoding (see
<code>past_key_values</code>). Set to <code>False</code> during training, <code>True</code> during generation`,name:"use_cache"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L915",returnDescription:`
<p>A <a
...
Labels for computing the cross entropy classification loss. Indices should be in <code>[0, ..., config.vocab_size - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1174",returnDescription:`
<p>A <a
...
Labels for computing the masked language modeling loss. Indices should be in <code>[-100, 0, ..., config.vocab_size]</code> (see <code>input_ids</code> docstring) Tokens with indices set to <code>-100</code> are ignored (masked), the
loss is only computed for the tokens with labels in <code>[0, ..., config.vocab_size]</code>`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1072",returnDescription:`
<p>A <a
...
Labels for computing the sequence classification/regression loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>. If <code>config.num_labels == 1</code> a regression loss is computed (Mean-Square loss), If
<code>config.num_labels &gt; 1</code> a classification loss is computed (Cross-Entropy).`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1333",returnDescription:`
<p>A <a
...
where <code>num_choices</code> is the size of the second dimension of the input tensors. (See <code>input_ids</code> above)`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1431",returnDescription:`
<p>A <a
...
Labels for computing the token classification loss. Indices should be in <code>[0, ..., config.num_labels - 1]</code>.`,name:"labels"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1547",returnDescription:`
<p>A <a
...
Labels for position (index) of the end of the labelled span for computing the token classification loss.
Positions are clamped to the length of the sequence (<code>sequence_length</code>). Position outside of the sequence
are not taken into account for computing the loss.`,name:"end_positions"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_tf_roberta.py#L1634",returnDescription:`
<p>A <a
...
Whether or not to return a <a href="/docs/transformers/pr_16925/en/main_classes/output#transformers.utils.ModelOutput">ModelOutput</a> instead of a plain tuple.`,name:"return_dict"}],source:"https://github.com/huggingface/transformers/blob/pr_16925/src/transformers/models/roberta/modeling_flax_roberta.py#L608",returnDescription:`
<p>A <a
...
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),Gw=p(),Mp=n("p"),Xw=r("Finally, this model supports inherent JAX features such as:"),Yw=p(),St=n("ul"),Ep=n("li"),wi=n("a"),Zw=r("Just-In-Time (JIT) compilation"),e1=p(),zp=n("li"),$i=n("a"),t1=r("Automatic Differentiation"),o1=p(),qp=n("li"),Fi=n("a"),n1=r("Vectorization"),s1=p(),Cp=n("li"),xi=n("a"),a1=r("Parallelization"),r1=p(),Et=n("div"),y(Ri.$$.fragment),i1=p(),Go=n("p"),l1=r("The "),jp=n("code"),d1=r("FlaxRobertaPreTrainedModel"),c1=r(" forward method, overrides the "),Pp=n("code"),p1=r("__call__"),h1=r(" special method."),m1=p(),y(qs.$$.fragment),f1=p(),y(Cs.$$.fragment),Vm=p(),Xo=n("h2"),js=n("a"),Lp=n("span"),y(Mi.$$.fragment),u1=p(),Ap=n("span"),g1=r("FlaxRobertaForSequenceClassification"),Jm=p(),Ie=n("div"),y(Ei.$$.fragment),_1=p(),Op=n("p"),b1=r(`Roberta Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),k1=p(),zi=n("p"),T1=r("This model inherits from "),pd=n("a"),y1=r("FlaxPreTrainedModel"),v1=r(`. Check the superclass documentation for the generic methods the
...
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),R1=p(),Np=n("p"),M1=r("Finally, this model supports inherent JAX features such as:"),E1=p(),Wt=n("ul"),Ip=n("li"),ji=n("a"),z1=r("Just-In-Time (JIT) compilation"),q1=p(),Dp=n("li"),Pi=n("a"),C1=r("Automatic Differentiation"),j1=p(),Sp=n("li"),Li=n("a"),P1=r("Vectorization"),L1=p(),Wp=n("li"),Ai=n("a"),A1=r("Parallelization"),O1=p(),zt=n("div"),y(Oi.$$.fragment),N1=p(),Yo=n("p"),I1=r("The "),Up=n("code"),D1=r("FlaxRobertaPreTrainedModel"),S1=r(" forward method, overrides the "),Bp=n("code"),W1=r("__call__"),U1=r(" special method."),B1=p(),y(Ps.$$.fragment),H1=p(),y(Ls.$$.fragment),Km=p(),Zo=n("h2"),As=n("a"),Hp=n("span"),y(Ni.$$.fragment),Q1=p(),Qp=n("span"),V1=r("FlaxRobertaForMultipleChoice"),Gm=p(),De=n("div"),y(Ii.$$.fragment),J1=p(),Vp=n("p"),K1=r(`Roberta Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
...
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),s$=p(),Jp=n("p"),a$=r("Finally, this model supports inherent JAX features such as:"),r$=p(),Ut=n("ul"),Kp=n("li"),Ui=n("a"),i$=r("Just-In-Time (JIT) compilation"),l$=p(),Gp=n("li"),Bi=n("a"),d$=r("Automatic Differentiation"),c$=p(),Xp=n("li"),Hi=n("a"),p$=r("Vectorization"),h$=p(),Yp=n("li"),Qi=n("a"),m$=r("Parallelization"),f$=p(),qt=n("div"),y(Vi.$$.fragment),u$=p(),en=n("p"),g$=r("The "),Zp=n("code"),_$=r("FlaxRobertaPreTrainedModel"),b$=r(" forward method, overrides the "),eh=n("code"),k$=r("__call__"),T$=r(" special method."),y$=p(),y(Os.$$.fragment),v$=p(),y(Ns.$$.fragment),Xm=p(),tn=n("h2"),Is=n("a"),th=n("span"),y(Ji.$$.fragment),w$=p(),oh=n("span"),$$=r("FlaxRobertaForTokenClassification"),Ym=p(),Se=n("div"),y(Ki.$$.fragment),F$=p(),nh=n("p"),x$=r(`Roberta Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
...
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),L$=p(),sh=n("p"),A$=r("Finally, this model supports inherent JAX features such as:"),O$=p(),Bt=n("ul"),ah=n("li"),Zi=n("a"),N$=r("Just-In-Time (JIT) compilation"),I$=p(),rh=n("li"),el=n("a"),D$=r("Automatic Differentiation"),S$=p(),ih=n("li"),tl=n("a"),W$=r("Vectorization"),U$=p(),lh=n("li"),ol=n("a"),B$=r("Parallelization"),H$=p(),Ct=n("div"),y(nl.$$.fragment),Q$=p(),on=n("p"),V$=r("The "),dh=n("code"),J$=r("FlaxRobertaPreTrainedModel"),K$=r(" forward method, overrides the "),ch=n("code"),G$=r("__call__"),X$=r(" special method."),Y$=p(),y(Ds.$$.fragment),Z$=p(),y(Ss.$$.fragment),Zm=p(),nn=n("h2"),Ws=n("a"),ph=n("span"),y(sl.$$.fragment),e2=p(),hh=n("span"),t2=r("FlaxRobertaForQuestionAnswering"),ef=p(),We=n("div"),y(al.$$.fragment),o2=p(),sn=n("p"),n2=r(`Roberta Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
...
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),ru.forEach(t),Gw=h(gt),Mp=s(gt,"P",{});var Rx=a(Mp);Xw=i(Rx,"Finally, this model supports inherent JAX features such as:"),Rx.forEach(t),Yw=h(gt),St=s(gt,"UL",{});var Zs=a(St);Ep=s(Zs,"LI",{});var Mx=a(Ep);wi=s(Mx,"A",{href:!0,rel:!0});var Ex=a(wi);Zw=i(Ex,"Just-In-Time (JIT) compilation"),Ex.forEach(t),Mx.forEach(t),e1=h(Zs),zp=s(Zs,"LI",{});var zx=a(zp);$i=s(zx,"A",{href:!0,rel:!0});var qx=a($i);t1=i(qx,"Automatic Differentiation"),qx.forEach(t),zx.forEach(t),o1=h(Zs),qp=s(Zs,"LI",{});var Cx=a(qp);Fi=s(Cx,"A",{href:!0,rel:!0});var jx=a(Fi);n1=i(jx,"Vectorization"),jx.forEach(t),Cx.forEach(t),s1=h(Zs),Cp=s(Zs,"LI",{});var Px=a(Cp);xi=s(Px,"A",{href:!0,rel:!0});var Lx=a(xi);a1=i(Lx,"Parallelization"),Lx.forEach(t),Px.forEach(t),Zs.forEach(t),r1=h(gt),Et=s(gt,"DIV",{class:!0});var ea=a(Et);v(Ri.$$.fragment,ea),i1=h(ea),Go=s(ea,"P",{});var Ld=a(Go);l1=i(Ld,"The "),jp=s(Ld,"CODE",{});var Ax=a(jp);d1=i(Ax,"FlaxRobertaPreTrainedModel"),Ax.forEach(t),c1=i(Ld," forward method, overrides the "),Pp=s(Ld,"CODE",{});var Ox=a(Pp);p1=i(Ox,"__call__"),Ox.forEach(t),h1=i(Ld," special method."),Ld.forEach(t),m1=h(ea),v(qs.$$.fragment,ea),f1=h(ea),v(Cs.$$.fragment,ea),ea.forEach(t),gt.forEach(t),Vm=h(o),Xo=s(o,"H2",{class:!0});var iu=a(Xo);js=s(iu,"A",{id:!0,class:!0,href:!0});var Nx=a(js);Lp=s(Nx,"SPAN",{});var Ix=a(Lp);v(Mi.$$.fragment,Ix),Ix.forEach(t),Nx.forEach(t),u1=h(iu),Ap=s(iu,"SPAN",{});var Dx=a(Ap);g1=i(Dx,"FlaxRobertaForSequenceClassification"),Dx.forEach(t),iu.forEach(t),Jm=h(o),Ie=s(o,"DIV",{class:!0});var _t=a(Ie);v(Ei.$$.fragment,_t),_1=h(_t),Op=s(_t,"P",{});var Sx=a(Op);b1=i(Sx,`Roberta Model transformer with a sequence classification/regression head on top (a linear layer on top of the
pooled output) e.g. for GLUE tasks.`),Sx.forEach(t),k1=h(_t),zi=s(_t,"P",{});var lu=a(zi);T1=i(lu,"This model inherits from "),pd=s(lu,"A",{href:!0});var Wx=a(pd);y1=i(Wx,"FlaxPreTrainedModel"),Wx.forEach(t),v1=i(lu,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),lu.forEach(t),w1=h(_t),qi=s(_t,"P",{});var du=a(qi);$1=i(du,"This model is also a Flax Linen "),Ci=s(du,"A",{href:!0,rel:!0});var Ux=a(Ci);F1=i(Ux,"flax.linen.Module"),Ux.forEach(t),x1=i(du,`
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),du.forEach(t),R1=h(_t),Np=s(_t,"P",{});var Bx=a(Np);M1=i(Bx,"Finally, this model supports inherent JAX features such as:"),Bx.forEach(t),E1=h(_t),Wt=s(_t,"UL",{});var ta=a(Wt);Ip=s(ta,"LI",{});var Hx=a(Ip);ji=s(Hx,"A",{href:!0,rel:!0});var Qx=a(ji);z1=i(Qx,"Just-In-Time (JIT) compilation"),Qx.forEach(t),Hx.forEach(t),q1=h(ta),Dp=s(ta,"LI",{});var Vx=a(Dp);Pi=s(Vx,"A",{href:!0,rel:!0});var Jx=a(Pi);C1=i(Jx,"Automatic Differentiation"),Jx.forEach(t),Vx.forEach(t),j1=h(ta),Sp=s(ta,"LI",{});var Kx=a(Sp);Li=s(Kx,"A",{href:!0,rel:!0});var Gx=a(Li);P1=i(Gx,"Vectorization"),Gx.forEach(t),Kx.forEach(t),L1=h(ta),Wp=s(ta,"LI",{});var Xx=a(Wp);Ai=s(Xx,"A",{href:!0,rel:!0});var Yx=a(Ai);A1=i(Yx,"Parallelization"),Yx.forEach(t),Xx.forEach(t),ta.forEach(t),O1=h(_t),zt=s(_t,"DIV",{class:!0});var oa=a(zt);v(Oi.$$.fragment,oa),N1=h(oa),Yo=s(oa,"P",{});var Ad=a(Yo);I1=i(Ad,"The "),Up=s(Ad,"CODE",{});var Zx=a(Up);D1=i(Zx,"FlaxRobertaPreTrainedModel"),Zx.forEach(t),S1=i(Ad," forward method, overrides the "),Bp=s(Ad,"CODE",{});var eR=a(Bp);W1=i(eR,"__call__"),eR.forEach(t),U1=i(Ad," special method."),Ad.forEach(t),B1=h(oa),v(Ps.$$.fragment,oa),H1=h(oa),v(Ls.$$.fragment,oa),oa.forEach(t),_t.forEach(t),Km=h(o),Zo=s(o,"H2",{class:!0});var cu=a(Zo);As=s(cu,"A",{id:!0,class:!0,href:!0});var tR=a(As);Hp=s(tR,"SPAN",{});var oR=a(Hp);v(Ni.$$.fragment,oR),oR.forEach(t),tR.forEach(t),Q1=h(cu),Qp=s(cu,"SPAN",{});var nR=a(Qp);V1=i(nR,"FlaxRobertaForMultipleChoice"),nR.forEach(t),cu.forEach(t),Gm=h(o),De=s(o,"DIV",{class:!0});var bt=a(De);v(Ii.$$.fragment,bt),J1=h(bt),Vp=s(bt,"P",{});var sR=a(Vp);K1=i(sR,`Roberta Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
softmax) e.g. for RocStories/SWAG tasks.`),sR.forEach(t),G1=h(bt),Di=s(bt,"P",{});var pu=a(Di);X1=i(pu,"This model inherits from "),hd=s(pu,"A",{href:!0});var aR=a(hd);Y1=i(aR,"FlaxPreTrainedModel"),aR.forEach(t),Z1=i(pu,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),pu.forEach(t),e$=h(bt),Si=s(bt,"P",{});var hu=a(Si);t$=i(hu,"This model is also a Flax Linen "),Wi=s(hu,"A",{href:!0,rel:!0});var rR=a(Wi);o$=i(rR,"flax.linen.Module"),rR.forEach(t),n$=i(hu,`
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),hu.forEach(t),s$=h(bt),Jp=s(bt,"P",{});var iR=a(Jp);a$=i(iR,"Finally, this model supports inherent JAX features such as:"),iR.forEach(t),r$=h(bt),Ut=s(bt,"UL",{});var na=a(Ut);Kp=s(na,"LI",{});var lR=a(Kp);Ui=s(lR,"A",{href:!0,rel:!0});var dR=a(Ui);i$=i(dR,"Just-In-Time (JIT) compilation"),dR.forEach(t),lR.forEach(t),l$=h(na),Gp=s(na,"LI",{});var cR=a(Gp);Bi=s(cR,"A",{href:!0,rel:!0});var pR=a(Bi);d$=i(pR,"Automatic Differentiation"),pR.forEach(t),cR.forEach(t),c$=h(na),Xp=s(na,"LI",{});var hR=a(Xp);Hi=s(hR,"A",{href:!0,rel:!0});var mR=a(Hi);p$=i(mR,"Vectorization"),mR.forEach(t),hR.forEach(t),h$=h(na),Yp=s(na,"LI",{});var fR=a(Yp);Qi=s(fR,"A",{href:!0,rel:!0});var uR=a(Qi);m$=i(uR,"Parallelization"),uR.forEach(t),fR.forEach(t),na.forEach(t),f$=h(bt),qt=s(bt,"DIV",{class:!0});var sa=a(qt);v(Vi.$$.fragment,sa),u$=h(sa),en=s(sa,"P",{});var Od=a(en);g$=i(Od,"The "),Zp=s(Od,"CODE",{});var gR=a(Zp);_$=i(gR,"FlaxRobertaPreTrainedModel"),gR.forEach(t),b$=i(Od," forward method, overrides the "),eh=s(Od,"CODE",{});var _R=a(eh);k$=i(_R,"__call__"),_R.forEach(t),T$=i(Od," special method."),Od.forEach(t),y$=h(sa),v(Os.$$.fragment,sa),v$=h(sa),v(Ns.$$.fragment,sa),sa.forEach(t),bt.forEach(t),Xm=h(o),tn=s(o,"H2",{class:!0});var mu=a(tn);Is=s(mu,"A",{id:!0,class:!0,href:!0});var bR=a(Is);th=s(bR,"SPAN",{});var kR=a(th);v(Ji.$$.fragment,kR),kR.forEach(t),bR.forEach(t),w$=h(mu),oh=s(mu,"SPAN",{});var TR=a(oh);$$=i(TR,"FlaxRobertaForTokenClassification"),TR.forEach(t),mu.forEach(t),Ym=h(o),Se=s(o,"DIV",{class:!0});var kt=a(Se);v(Ki.$$.fragment,kt),F$=h(kt),nh=s(kt,"P",{});var yR=a(nh);x$=i(yR,`Roberta Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
Named-Entity-Recognition (NER) tasks.`),yR.forEach(t),R$=h(kt),Gi=s(kt,"P",{});var fu=a(Gi);M$=i(fu,"This model inherits from "),md=s(fu,"A",{href:!0});var vR=a(md);E$=i(vR,"FlaxPreTrainedModel"),vR.forEach(t),z$=i(fu,`. Check the superclass documentation for the generic methods the
library implements for all its model (such as downloading, saving and converting weights from PyTorch models)`),fu.forEach(t),q$=h(kt),Xi=s(kt,"P",{});var uu=a(Xi);C$=i(uu,"This model is also a Flax Linen "),Yi=s(uu,"A",{href:!0,rel:!0});var wR=a(Yi);j$=i(wR,"flax.linen.Module"),wR.forEach(t),P$=i(uu,`
subclass. Use it as a regular Flax linen Module and refer to the Flax documentation for all matter related to
general usage and behavior.`),uu.forEach(t),L$=h(kt),sh=s(kt,"P",{});var $R=a(sh);A$=i($R,"Finally, this model supports inherent JAX features such as:"),$R.forEach(t),O$=h(kt),Bt=s(kt,"UL",{});var aa=a(Bt);ah=s(aa,"LI",{});var FR=a(ah);Zi=s(FR,"A",{href:!0,rel:!0});var xR=a(Zi);N$=i(xR,"Just-In-Time (JIT) compilation"),xR.forEach(t),FR.forEach(t),I$=h(aa),rh=s(aa,"LI",{});var RR=a(rh);el=s(RR,"A",{href:!0,rel:!0});var MR=a(el);D$=i(MR,"Automatic Differentiation"),MR.forEach(t),RR.forEach(t),S$=h(aa),ih=s(aa,"LI",{});var ER=a(ih);tl=s(ER,"A",{href:!0,rel:!0});var zR=a(tl);W$=i(zR,"Vectorization"),zR.forEach(t),ER.forEach(t),U$=h(aa),lh=s(aa,"LI",{});var qR=a(lh);ol=s(qR,"A",{href:!0,rel:!0});var CR=a(ol);B$=i(CR,"Parallelization"),CR.forEach(t),qR.forEach(t),aa.forEach(t),H$=h(kt),Ct=s(kt,"DIV",{class:!0});var ra=a(Ct);v(nl.$$.fragment,ra),Q$=h(ra),on=s(ra,"P",{});var Nd=a(on);V$=i(Nd,"The "),dh=s(Nd,"CODE",{});var jR=a(dh);J$=i(jR,"FlaxRobertaPreTrainedModel"),jR.forEach(t),K$=i(Nd," forward method, overrides the "),ch=s(Nd,"CODE",{});var PR=a(ch);G$=i(PR,"__call__"),PR.forEach(t),X$=i(Nd," special method."),Nd.forEach(t),Y$=h(ra),v(Ds.$$.fragment,ra),Z$=h(ra),v(Ss.$$.fragment,ra),ra.forEach(t),kt.forEach(t),Zm=h(o),nn=s(o,"H2",{class:!0});var gu=a(nn);Ws=s(gu,"A",{id:!0,class:!0,href:!0});var LR=a(Ws);ph=s(LR,"SPAN",{});var AR=a(ph);v(sl.$$.fragment,AR),AR.forEach(t),LR.forEach(t),e2=h(gu),hh=s(gu,"SPAN",{});var OR=a(hh);t2=i(OR,"FlaxRobertaForQuestionAnswering"),OR.forEach(t),gu.forEach(t),ef=h(o),We=s(o,"DIV",{class:!0});var Tt=a(We);v(al.$$.fragment,Tt),o2=h(Tt),sn=s(Tt,"P",{});var Id=a(sn);n2=i(Id,`Roberta Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear
