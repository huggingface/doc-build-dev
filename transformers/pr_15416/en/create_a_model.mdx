...
Get a closer look at [DistilBERT](model_doc/distilbert) by accessing [DistilBertConfig](/docs/transformers/pr_15416/en/model_doc/distilbert#transformers.DistilBertConfig) to inspect it's attributes:
...
Pretrained model attributes can be modified in the [from_pretrained()](/docs/transformers/pr_15416/en/main_classes/configuration#transformers.PretrainedConfig.from_pretrained) function:

```py
>>> my_config = DistilBertConfig.from_pretrained("distilbert-base-uncased", activation="relu", attention_dropout=0.4)
```

Once you are satisfied with your model configuration, you can save it with [save_pretrained()](/docs/transformers/pr_15416/en/main_classes/configuration#transformers.PretrainedConfig.save_pretrained). Your configuration file is stored as a JSON file in the specified save directory:

```py
>>> my_config.save_pretrained(save_directory="./your_model_save_path")
```

To reuse the configuration file, load it with [from_pretrained()](/docs/transformers/pr_15416/en/main_classes/configuration#transformers.PretrainedConfig.from_pretrained):
...
The next step is to create a [model](main_classes/models). The model - also loosely referred to as the architecture - defines what each layer is doing and what operations are happening. Attributes like `num_hidden_layers` from the configuration are used to define the architecture. Every model shares the base class [PreTrainedModel](/docs/transformers/pr_15416/en/main_classes/model#transformers.PreTrainedModel) and a few common methods like resizing input embeddings and pruning self-attention heads. In addition, all models are also either a [`torch.nn.Module`](https://pytorch.org/docs/stable/generated/torch.nn.Module.html), [`tf.keras.Model`](https://www.tensorflow.org/api_docs/python/tf/keras/Model) or [`flax.linen.Module`](https://flax.readthedocs.io/en/latest/flax.linen.html#module) subclass. This means models are compatible with each of their respective framework's usage.
...
Create a pretrained model with [from_pretrained()](/docs/transformers/pr_15416/en/main_classes/model#transformers.PreTrainedModel.from_pretrained):
...
For example, [DistilBertForSequenceClassification](/docs/transformers/pr_15416/en/model_doc/distilbert#transformers.DistilBertForSequenceClassification) is a base DistilBERT model with a sequence classification head. The sequence classification head is a linear layer on top of the pooled outputs.
...
- [PreTrainedTokenizer](/docs/transformers/pr_15416/en/main_classes/tokenizer#transformers.PreTrainedTokenizer): a Python implementation of a tokenizer.
- [PreTrainedTokenizerFast](/docs/transformers/pr_15416/en/main_classes/tokenizer#transformers.PreTrainedTokenizerFast): a tokenizer from our Rust-based [🤗 Tokenizer](https://huggingface.co/docs/tokenizers/python/latest/) library. This tokenizer type is significantly faster - especially during batch tokenization - due to it's Rust implementation. The fast tokenizer also offers additional methods like *offset mapping* which maps tokens to their original words or characters.
...
Create a fast tokenizer with the [DistilBertTokenizerFast](/docs/transformers/pr_15416/en/model_doc/distilbert#transformers.DistilBertTokenizerFast) class:
...
By default, [AutoTokenizer](/docs/transformers/pr_15416/en/model_doc/auto#transformers.AutoTokenizer) will try to load a fast tokenizer. You can disable this behavior by setting `use_fast=False` in `from_pretrained`.
...
Modify any of the [ViTFeatureExtractor](/docs/transformers/pr_15416/en/model_doc/vit#transformers.ViTFeatureExtractor) parameters to create your custom feature extractor:
...
For audio inputs, you can create a [Wav2Vec2FeatureExtractor](/docs/transformers/pr_15416/en/model_doc/wav2vec2#transformers.Wav2Vec2FeatureExtractor) and customize the parameters in a similar way:
...
Combine the feature extractor and tokenizer in [Wav2Vec2Processor](/docs/transformers/pr_15416/en/model_doc/wav2vec2#transformers.Wav2Vec2Processor):
